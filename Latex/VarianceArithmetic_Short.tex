...
The value of bound moment  $\zeta(2n, \kappa)$ for Uniform distribution is also provided as an comparison.
}
...
In $\delta^x x = \zeta(2, \kappa) (\delta x)^2$, the result variance $\delta^x$ is less than input variance $(\delta x)^2$ due to linear leakage.
...
The unbiased bounding range $[\kappa_{min}, \kappa_{max}]$ of Uniform distribution is estimated as $1 + \frac{1}{N}$ times of the minimal and maximal values in sampled data set \cite{Uniform_Distrobution Estimator}.
The bounding leakage $\epsilon$ is the percentage of $[\kappa_{min}, \kappa_{max}]$ outside the true bounding range $[\mu - \sqrt{3}\sigma, \mu + \sqrt{3}\sigma]$.
Because $\zeta(2n)$ for Uniform distribution increases much slower than $\zeta(2n, 5)$ for Gaussian distribution, Figure \ref{fig: Uniform_Function} shows that for the selected functions, ideal leakage equals linear leakage for Uniform input uncertainty, and both decrease linearly with sample count $N$.
