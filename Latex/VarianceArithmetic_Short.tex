...
R_1 &\equiv \frac{1}{N} \sum_i^N \big( y_i - f[0] - f[1] x_i \big); \Rightarrow R_1 = (\delta y)^2 - \frac{\nu(x, y)^2}{(\delta x)^2} ; \\
\label{eqn: set slope}
&f[1] = \frac{\nu(x,y)}{(\delta x)^2}; \\
...
Formula \eqref{eqn: multi slope cov} is the slope variance, which shows that slopes from different dimensions may be mutually dependent. 
...
R_M &\equiv \frac{1}{N} \sum_i^N \left(Y_i - f[0] - \sum_{m=1}^{M} f[m]\, X_{m,i}\right)^2 
...
R_P &\equiv \frac{1}{N} \sum_i^N \left(Y_ij - \sum_{p=0}^{P} f[p]\, X_i^p\right)^2; \\
