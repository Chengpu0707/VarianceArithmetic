...
It is desirable that each value from library functions be accomplished by a uncertainty deviation.  
\end{abstract}
...
The probability of $\tilde{z} \not \in [\varrho, \kappa]$ is defined as the \emph{bounding leakage} $\epsilon(\kappa) = 1 - \zeta(0, \kappa)$.
...
\end{itemize}
The trends of uncertainty biases $\overline{f} - f$ versus sample count $N$ are exactly as those in Figure \ref{fig: Normal_Function} and \ref{fig: Uniform_Function} respectively, except that  $\overline{x} - x = 0$.
...
These observations suggest that bounding leakage is the reason for $\delta^2 f < \widehat{\delta^2} f$ and it is justified to infer the confidential interval range $\kappa$ from bounding leakage $\epsilon(\kappa_s) = 1  - \zeta(0, \kappa_s)$ in Figure \ref{fig: Normal_Bounding_Leakage},  or more precisely $1  - \zeta(2, \kappa_s)$ for $\delta^2 f$.

Define the stable variance when the sample count $N$ is sufficiently large as the \emph{ideal variance} $\widehat{\delta^2} f$.
...
Conceptually, the upper-bound for $\delta x$ should be less than $\pi/2$ to make the result meaningful, which Figure \ref{fig: Sin_Conv_Edge} satisfies.
\ifdefined\Verbose
...
For example, because mathematical divergence seems to correspond to distributional zero, it is even tempting to postulate if a localized experiment changes $\hat{\kappa}$ to produce distributional zero and the quantum result as ``waveform collapse". 
Both resemblances are worth further investigation.

It is also worthwhile investigating if uncertainty bias produces Lamb shift.
