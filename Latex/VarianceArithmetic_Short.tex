...
For example, because mathematical divergence seems to correspond to distributional zero, it is even tempting to postulate if a localized experiment changes $\hat{\kappa}$ to produce distributional zero and the quantum result as ``waveform collapse" \cite{Quantum Field}. 
Both resemblances are worth further investigation.

It is also worthwhile investigating if uncertainty bias produces Lamb shift \cite{Quantum Field}.
