...
When there is no systematic error \cite{Statistical_Methods}\cite{Precisions_Physical_Measurements} among the inputs, the uncertainties of the inputs are uncorrelated of each other, even though there may be significant correlations among the inputs.
...
\item \emph{positive}: The variance must be positive.
...
For example, after the respective upper bound $P(x)$ for $(x \pm \delta x)^c$ are normalized by the corresponding $\hat{\kappa}$, they shows almost the same trends of increasing with $c$.
...
Similar trends holds when the input uncertainty is uniform but with larger upper bound $\delta x$.
...
Achieving ideal coverage serves as a necessary verification step to ensure that statistical Taylor expansion have been applied correctly within the given context.
...
Formula \eqref{eqn: determinant mean} and \eqref{eqn: determinant variance} gives the statistical Taylor expansion of the determinant of a a square matrix $\mathbf{M}$ of size $n$, when the uncertainties of matrix elements are all independent of each other.
...
Because the the Quart sine function strictly preserves the symmetry and periodicity of sine function, it provides superior numerical accuracy compared to Library function.
\begin{itemize}
...
When the FFT order $L$ is less than 8, the error deviations deviates from $1$ due to insufficient sample count of $2^L$.

Figure \ref{fig: FFT_SinCos_Clean_Histo_Indexed} shows that the error distribution of Sin/Cos signal resemble Gaussian distribution, with an additional Delta-like distribution at zero.
...
Such resonance of numerical errors seem to have not been discussed before in the literature.
...
In other areas, both transformation achieve proper coverage with error values around $1$.

When $\delta x \rightarrow 0$, the error distribution of reverse transformation in Figure \ref{fig: FFT_Linear_Clean_Histo_Indexed} is narrower than that in Figure \ref{fig: FFT_SinCos_Clean_Histo_Lib}.
The corresponding error deviations are $1.5 < 6.2$, respectively.

The error deviations for the round-trip transformation resembles Figure \ref{fig: Forward_Error_vs_Size_Noise} but with FFT order as the specific dimension.
...
\begin{itemize}
\item As expected, the result uncertainty deviations for the forward transformations increase with FFT order $L$ as $\sqrt{2}^L$.

\item As expected, the result uncertainty deviations for the reverse transformations decrease with FFT order $L$ as $1/\sqrt{2}^L$.

\item As expected, the result uncertainty deviations for the round-trip transformations remains equal to the corresponding input uncertainties of $10^{-3}$.

\item As expected, the result error deviations for the forward and reverse transformations remain constant at $1$.

\item As expected, the result error deviation for the round-trip transformations is far less than $1$ but increases with FFT order $L$ exponentially due to increasing calculation.
\end{itemize}
