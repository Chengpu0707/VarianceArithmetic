...
If $N_j$ is unknown, and if \emph{the data set is assumed to be independent and identically distributed (i.i.d.)}, then $1/(\overline{x_j} - \overline{x})^2$ can replace $N_j/N$ as the weight, because bootstrapping the underlying distribution mean as $\overline{x}$ gives $(\overline{x_j} - \overline{x})^2 \propto 1/N_j$ according to the central limit theorem \cite{Probability_Statistics}.
...
(\delta x)^2 &\simeq \frac{J + \sum_{j} \frac{(\delta x_j)^2}{(\overline{x_j} - \overline{x})^2}}
		{\sum_{j} \frac{1}{(\overline{x_j} - \overline{x})^2}};
\end{align}
...
Furthermore, using $1/(\delta x_j)^2$ as the weight prevents mixing of precise and imprecise values, whereas Formulas \eqref{eqn: set mean}, \eqref{eqn: set variance}, \eqref{eqn: set mean approx}, and \eqref{eqn: set variance approx} all permit such mixing.


\subsection{First Order Regression}
...
$(\delta f[1])^2$ decreases with larger $(\delta x)^2$, while $(\delta f[0])^2$ decreases with larger $(\delta x)^2$ relative to $\overline{x}^2$ and reaches its lower bound $R_1/N$ as $\overline{x} \to 0$.
\begin{align}
...
\end{align}
Because of its statistical rigor, one usage of Formula \eqref{eqn: set cov} is to exclude data from a data set to improve the fitting result according to Formula \eqref{eqn: set slope precision} in cluster analysis.
