...
\nu\bigl(f[k],\, f[l]\bigr) &= \frac{R_K}{N}\, \bigl(\mathbf{C}^{-1}\bigr)_{kl}; \\
...
Formulas \eqref{eqn: multi cov partition} and \eqref{eqn: multi cross cov partition} extend Formula \eqref{eqn: set cov} component-wise to assemble $\mathbf{C}$ and $\boldsymbol{\nu}$ from subset statistics, with the same conditions as in the previous subsection: within-subset covariances may be zero, but all $N_j$ are required.
\begin{align}
\label{eqn: multi cov partition}
\nu(x_k, x_l) &= \sum_j \frac{N_j}{N} \Bigl( \nu(x_k, x_l)_j + \bigl(\overline{x_{k,j}} - \overline{x_k}\bigr)\bigl(\overline{x_{l,j}} - \overline{x_l}\bigr) \Bigr), \\
...
Formulas \eqref{eqn: poly C} and \eqref{eqn: poly nu} give the corresponding covariance matrix and covariance vector, respectively.
\begin{align}
\label{eqn: poly C}
C_{kl} &= \nu(x^k, x^l) = \overline{x^{k+l}} - \overline{x^k}\,\overline{x^l}, \quad k,\, l = 1, \dots, P; \\
\label{eqn: poly nu}
\nu_k &= \nu(x^k, y) = \overline{x^k y} - \overline{x^k}\,\overline{y}, \quad k = 1, \dots, P;
\end{align}
Conversely, the multi-dimensional regression of Formulas \eqref{eqn: poly C} and \eqref{eqn: poly nu} also yields Formula \eqref{eqn: poly normal}, confirming that the two methods are equivalent.
\iffalse
...
\end{align}
\fi
This equivalence allows applying Formulas \eqref{eqn: multi cov partition} and \eqref{eqn: multi cross cov partition} for polynomial regression also.
...
Even when $\overline{x} \rightarrow 0$, $f[m]$ inherently depends on others, unless between odd and even parities when $x$ is symmetrically distributed.
