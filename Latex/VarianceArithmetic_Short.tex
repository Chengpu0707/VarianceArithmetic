...
Statistical Taylor expansion is a rigorous extension of conventional Taylor expansion that replaces each precise input variable with a random variable of known distribution and sample count, then computes the mean, deviation, and reliability of every result.
By tracking the propagation of input uncertainties through all intermediate steps, it renders the final result path-independent, and it validates that result either as a $+1$ error deviation or a $-1$ error slope.
This path-independence sets it fundamentally apart from conventional numerical approaches, which are path-dependent, and may make statistical Taylor expansion a unified standard for the numerical computation of analytic expressions.
To this end, this study presents an implementation called variance arithmetic and demonstrates its performance across diverse mathematical applications.

This study also reveals the potentially substantial impact of numerical errors in library functions. 
\ifdefined\Verbose
...
The direct application of Taylor expansion in interval arithmetic over-estimates result intervals, so that it has to be treated as a polynomial \cite{Taylor_Models}\cite{Taylor_Forms_Limits}, and needs interval partition and other techniques to reduce such over-estimation \cite{Interval_Analysis}\cite{Interval_Analysis_Theory_Applications}\cite{Interval_Global_Optimization}.
The subdivision can cause branched results \cite{Interval_Analysis}\cite{Interval_Analysis_Theory_Applications}, such as calculating square root for an input interval containing zero.
...
In contrast, statistical Taylor expansion is precise statistically, and it diverges when the input precision is not fine enough.
...
It also compares interval arithmetic and statistical Taylor expansion for their different goals.

\item Section \ref{sec: recursion} showcases variance arithmetic in catching catastrophic cancellation in a recursion algorithm.
...
\captionof{figure}{
$\gamma$ vs $P$ for different $\gamma_P$ in Formula \eqref{eqn: uncertainty correlation}.
If $\gamma_P$ is the maximal allowed correlation at the uncertainty level for the uncorrelated uncertainty assumption to hold, then $\gamma$ is the maximal allowed correlation for the two signals to hold.
...
The uncorrelated uncertainty condition \cite{Prev_Precision_Arithmetic} keeps the uncertainty-level correlation less than the signal-level correlation when the precision is less than $1$.
...
\end{itemize}
The above conclusion agrees with the common experiences that, below the noise level of measured signals, noise rather than true signals dominates each bit.  

Similarly, while the correlated portion between two values has the same magnitude at every bit position, the ratio of uncorrelated portion to correlated portion doubles for each bit position below the MSB. 
...
&\sum_{j=1}^{2n+2-1} \begin{pmatrix} c \\ j \end{pmatrix} \begin{pmatrix} c \\ 2n - j + 2 \end{pmatrix} = 
		...
		&\qquad + \left(\frac{c^2}{2} + \frac{c - 2n}{2n + 1} c \right) \begin{pmatrix} c \\ 2n \end{pmatrix}; \\
...
\iffalse
Validate Formula \eqref{eqn: exponential asymptotic moment} of the tex using Formula \eqref{eqn: mean-reverting bounding}:

Validation result

Formula eqn: exponential asymptotic moment is valid as a leading-order asymptote, and the mean-reverting condition is exactly what licenses its clean form. Here is the derivation, and the precise sub-leading caveat the numerics expose.

What must be shown

For the standardized Exponential, $\rho(\tilde z)=e^{-(1+\tilde z)}$ on $\tilde z\ge -1$ (I verified mean $0$, variance $1$ — correctly standardized). The bound moment is the conditional moment
$$\zeta(n,\kappa)=\frac{I_n}{D},\qquad I_n=\int_\varrho^\kappa \tilde z^n\rho(\tilde z),d\tilde z,\qquad D=\int_\varrho^\kappa\rho(\tilde z),d\tilde z=e^{-(1+\varrho)}-e^{-(1+\kappa)}.$$

The derivation (the paper's route)

For large $n$, $\tilde z^n$ concentrates the mass at the upper endpoint $\kappa$ (the integrand $e^{n\ln\tilde z-(1+\tilde z)}$ is increasing for $\tilde z<n$, so on $[\varrho,\kappa]$ its max is at $\kappa$). Approximating $\rho(\tilde z)\approx\rho(\kappa)$ there and integrating the monomial exactly:
$$I_n\approx \rho(\kappa)\int_\varrho^\kappa \tilde z^n,d\tilde z=\rho(\kappa),\frac{\kappa^{n+1}-\varrho^{,n+1}}{n+1}.$$

Now invoke eqn: mean-reverting bounding. The condition $\zeta(1,\kappa)=0$ reduces (as the text states) to
$$(1+\varrho),e^{-(1+\varrho)}=(1+\kappa),e^{-(1+\kappa)}.$$
As $\kappa$ grows, the right side $\to 0$, forcing $1+\varrho\to 0^+$, i.e. $\varrho\to -1^+$ (the support edge). This does two things at once:

1. Kills the lower endpoint: $|\varrho|\le 1<\kappa$, so $\varrho^{,n+1}$ is negligible against $\kappa^{n+1}$ — drop it.
2. Normalizes the denominator: $D=e^{-(1+\varrho)}-e^{-(1+\kappa)}\to e^{0}-0 = 1$.

Together these give exactly
$$\zeta(n,\kappa)=\frac{I_n}{D}\ \longrightarrow\ \rho(\kappa),\frac{\kappa^{n+1}}{n+1}. \qquad\checkmark$$

This is also structurally consistent with eqn: symmetric asymptotic moment: the symmetric case integrates $\tilde z^{2n}$ over both endpoints $\pm\kappa$ (factor 2), whereas the one-sided Exponential is dominated by the single endpoint $\kappa$ (factor 1) — which is exactly the difference between the two formulas.

The numbers (and the honest caveat)

A rigorous endpoint (Watson/Laplace) analysis sharpens the numerator to $I_n\sim\rho(\kappa),\kappa^{n+1}/n$, so the exact asymptote is
$$\zeta(n,\kappa)\ \sim\ \frac{\rho(\kappa)}{D}\cdot\frac{\kappa^{n+1}}{n}.$$
This differs from the stated RHS only in two sub-leading ways: the denominator is $n$ rather than $n{+}1$ (ratio $\to1$), and there is a normalization factor $1/D$ (which $\to1$ under mean-reverting). The high-precision check confirms both readings: the stated form converges to $1/D$, the sharpened form converges cleanly to $1$, each as $O(\kappa/n)$:


Verdict

Valid. Formula eqn: exponential asymptotic moment correctly captures the dominant behavior $\zeta(n,\kappa)\sim\rho(\kappa),\kappa^{n+1}/n$, and the mean-reverting condition eqn: mean-reverting bounding is precisely what makes the clean stated form hold: by pinning $\varrho\to-1$ it both removes the lower-endpoint term and sends the truncation factor $D\to1$. The only imprecisions are sub-leading — the $n{+}1$ vs $n$ denominator and the residual $1/D\to1$ — and neither affects the formula's purpose: the exponential growth rate $\kappa^{n}$ is what drives the convergence test in eqn: variance asymptotic, and that rate is exactly right.

One small suggestion: since eqn: symmetric asymptotic moment and this one are leading-order asymptotic equivalences (the RHS depends on $n$), the "$\lim_{n\to\infty}\zeta = \ldots$" notation is slightly loose — "$\zeta(n,\kappa)\sim\ldots$ as $n\to\infty$" would be more precise. Want me to adjust that notation in both formulas?
...
The value of moment  $\zeta(2n)$ for Uniform distribution is also provided as an comparison.
}
...
\rho(\tilde{z}) = \frac{1}{2\sqrt{3}}, |\tilde{z}| \leq \sqrt{3}&: \zeta(2n, \kappa) = \frac{\kappa^{2n}}{2n + 1}; \\
\label{eqn: symmetric asymptotic moment}
\rho(-\tilde{z}) =\rho(\tilde{z})&: \lim_{n \rightarrow +\infty} \zeta(2n, \kappa) \simeq \frac{\kappa^{2n}}{2n}; \\
\label{eqn: exponential asymptotic moment}
|\varrho| < \infty&: \lim_{n \rightarrow +\infty} \zeta(n, \kappa) \simeq \frac{\kappa^{n}}{n}; \\
\label{eqn: variance asymptotic}
...
Formula \eqref{eqn: exponential asymptotic moment} states the corresponding asymptote for any asymmetric $\rho(\tilde{z})$ under the mean-reverting bounding condition of Formula \eqref{eqn: mean-reverting bounding} when both $\varrho$ and $\zeta(2, \kappa)$ are bounded.
\ifdefined\Verbose
Figure \ref{fig: Gaussian_5_Moment} shows $\zeta(2n)$ for the Uniform distribution, $\zeta(2n, 5)$ for the Normal distribution, and the corresponding asymptote for the Normal distribution.
\fi
...
\item Formula \eqref{eqn: log precision} for $\log(x \pm \delta x)$ can be approximated by Formula \eqref{eqn: log convergence}, which converges when $P(x) \leq 1/\kappa$.
...
\begin{align}
\label{eqn: log convergence}
\rho(-\tilde{z}) =\rho(\tilde{z})&: \delta^2 \log(x \pm \delta x) \simeq \sum_{n = 1}^{+\infty} (P(x) \kappa)^{2n}\frac{\log(n)}{n}; \\
\label{eqn: pow convergence}
\rho(-\tilde{z}) =\rho(\tilde{z})&: \frac{\delta^2 (x \pm \delta x)^c}{(x^c)^2} \simeq \sum_{n = 1}^{+\infty} (P(x) \kappa)^{2n} \frac{\begin{pmatrix} 2c \\ 2n \end{pmatrix}}{2n};
\end{align}
...
Define \emph{bounding leakage} $\epsilon(\kappa) \equiv 1 - \zeta(0, \kappa)$ which is the probability for the distribution to be outside the bounding range.
Ideally, a bounding leakage should be as small as possible, and the corresponding bound range $\hat{\kappa}$ and variance $\hat{\delta^2} f(x)$ are the \emph{ideal bounding range} and \emph{ideal variance}, respectively.
For a Uniform distribution, by definition $\hat{\kappa} = \sqrt{3}$.
For a Gaussian distribution, when $\kappa \rightarrow \infty$, the convergence range of Formulas \eqref{eqn: log convergence} and \eqref{eqn: pow convergence} approaches zero, such that the choice of $\hat{\kappa}$ is a compromise.  
According to the 5-$\sigma$ rule for determining the statistical significance of an experimental result \cite{Statistical_Methods}\cite{Precisions_Physical_Measurements}\cite{Probability_Statistics},  $\hat{\kappa} = 5$ by default.
...
In $\delta^2 x = \zeta(2, \kappa) (\delta x)^2$, the result variance $\delta^2 x$ is less than input variance $(\delta x)^2$ due to linear leakage.
...
\epsilon(\kappa) &= \omega(\kappa) + 2 \rho(\kappa) \kappa; \\
...
\varepsilon(x y) &= \frac{2\omega_x P(x)^2 + 2 \omega_y P(y)^2 - \omega_x P(x)^2 \omega_y P(y)^2}{P(x)^2 + P(y)^2 + P(x)^2 P(y)^2};
\end{align}
Define \emph{ideal leakage} as $\varepsilon \equiv 1 - \delta^2 f/\widehat{\delta^2} f \simeq \omega$, which quantifies the reliability of $\hat{\delta^2} f$ from the underlying distributions and the corresponding sample counts of all inputs, with Formulas \eqref{eqn: sum leakage} and \eqref{eqn: product leakage} as two examples.
Figure \ref{fig: Normal_Function} compares ideal leakage $\varepsilon$ and linear leakage $\omega$ for the selected functions when the input uncertainty distribution is Gaussian, and shows that the example stable ideal leakages are less than $10^{-4}$ when $\hat{\kappa} = 5$, which is small enough for most applications. 

The unbiased bounding range $[\varrho, \kappa]$ of Uniform distribution is estimated as $1 + \frac{1}{N}$ times the minimal and maximal values in the sampled data set \cite{Uniform_Distrobution Estimator}.
The bounding leakage $\epsilon$ is the percentage of $[\varrho, \kappa]$ outside the true bounding range $[\mu - \sqrt{3}\sigma, \mu + \sqrt{3}\sigma]$.
...
Because Formula \eqref{eqn: sum variance} comes from variance definition, Formula \eqref{eqn: Taylor 1d variance} does not contain dependence problem, in contrast to Taylor expansion using interval arithmetic \cite{Interval_Analysis}\cite{Taylor_Models}\cite{Taylor_Forms_Limits}.
...
For example, if $x^2 - x$ is calculated as $x^2 - x$, $x(x - 1)$, and $(x - \frac{1}{2})^2 - \frac{1}{4}$, only $(x - \frac{1}{2})^2 - \frac{1}{4}$ gives the correct result, while the other two give incorrect results due to false independence assumptions between $x^2$ and $x$, or between $x - 1$ and $x$, respectively.
...
\item \emph{stable}: To avoid truncation error \cite{Numerical_Recipes}, the absolute value of the last expansion term must be less than $\epsilon$ times both the result deviation and the result absolute value, where $\epsilon \simeq 5.73\;10^{-7}$ is the bounding leakage for Gaussian distribution with $\hat{\kappa} = 5$.
...
\end{itemize}
\ifdefined\Verbose
A similar trend holds when the input uncertainty follows other distributions.
...
It has nearly the same trend of upper bounds $P(x)$ increasing with $c$ as shown in Figure \ref{fig: Pow_Conv_Edge_Uniform}, but with different y-scaling: $1/\sqrt{3}$ instead of $1/5$.
\else
A similar trend holds near $\hat{\kappa} = \sqrt{3}$ when the input uncertainty follows Uniform distribution.
\fi 
...
In addition to convergence, the variance expansion may yield negative results when the expansion is terminated arbitrarily, as in Formula \eqref{eqn: sin precision} for $sin(x \pm \delta x)$, such that an upper bound for $\delta x$ is needed to guarantee non-negative variance at any expansion order.
...
Unless the least 20 bits of the significand of $x$ are all zero, $\delta x$ is assumed to be $1/\sqrt{3}$ times the ULP (Unit in the Last Place \cite{Floating_Point_Standard}) of $x$, because rounding errors are shown to be uniformly distributed within the ULP \cite{Prev_Precision_Arithmetic}.
...
For the value test, the \emph{error slope}---defined as the slope of error deviation versus input noise in a linear regression---should be exactly $-1$.
...
The calculated uncertainty bounds the residual error effectively for all $x$, with an error deviation of $2.60$ when the expansion order is less than $224$.
...
For example, when $c$ is a natural number $n$, $(x \pm \delta x)^c$ becomes a polynomial with no upper bound on $\delta x$, in contrast to when $c$ is not a natural number as shown in Figure \ref{fig: Pow_Conv_Edge}.
...
The legend shows the size of the matrix, the deviation of added noise, as well as the type of the matrix as \textit{Random} for randomly generated matrix, and \textit{Hilbert} as the Hilbert matrix.
The noise of a Hilbert matrix matrix is the ULP of each of its element, which is slightly less than $10^{-16}$ as expected.
}
...
|\widetilde{\mathbf{M}}| &\equiv \sum_{[j_1\dots j_n]_n} \$ [j_1\dots j_n]_n  \prod_{i=1 \dots n} (x_{i,j_i} + \tilde{x}_{i,j_{i}}) \\
...
\delta^2 |\mathbf{M}| &= \sum_{m=1}^{n} \sum_{<i_1 \dots i_m>_n} \sum_{[j_1 \dots j_m]_n}
  	|\mathbf{M}_{<i_1 \dots i_m>_n, <[j_1 \dots j_m]>_n}|^2 \prod _{k=1 \dots m} \zeta_{i,j}(2, \kappa_{i,j})(\delta x_{i_k, j_k})^2; 
\end{align}
Let $<j_1, j_{2} \dots j_m>_n$ denote a selection of $m$ numbers from the sequence of natural numbers $1,2 \dots n$ in sequence, which is named as the \emph{ordered permutation}.
...
\delta^2 \left( \begin{matrix} w, x \\ y, z \end{matrix} \right)^{-1} \simeq&\; \frac{
	 	\left( \begin{matrix} z^4, x^2 z^2 \\ y^2 z^2,  x^2 y^2 \end{matrix} \right) \zeta_w(2) (\delta w)^2 +
	 	\left( \begin{matrix} y^2 z^2, w^2 z^2 \\ y^4, w^2 y^2 \end{matrix} \right)\zeta_x(2)  (\delta x)^2}{(w z - x y)^4} + \\
	&\; \frac{\left( \begin{matrix} x^2 z^2, x^4 \\ w^2 z^2, w^2 x^2 \end{matrix} \right) \zeta_y(2) (\delta y)^2 +
	 	\left( \begin{matrix} x^2 y^2, w^2 x^2 \\ w^2 y^2, w^4 \end{matrix} \right) \zeta_z(2) (\delta z)^2
	}{(w z - x y)^4}; \nonumber
\end{align}
...
Because an element of the original matrix $\mathbf{M}$ appears multiple times in Formula \eqref{eqn: inverse matrix}, the variance of that element, as obtained using Formula \eqref{eqn: Taylor 2d variance}, is very complicated. 
...
In Figure \ref{fig: Matrix_Determinant_Prec_vs_Condition}, adding noise to a matrix does not change its conditional number, while increases result precision because conceptually a matrix with larger uncertainty is less stable.  
In fact, Formula \eqref{eqn: inverse matrix 2 variance} shows the stability of each matrix element, which may be a better characterization of matrix stability than the conditional number.
...
\hline 
Uncertainty      & $\delta x \in [10^{-15}, 1]$ & $P(x) \in [10^{-15}x, 0.2 x]$ & $P(x) \in [10^{-15}, 0.2]$ & $\delta x \in [10^{-15}, 1]$     \\
\hline 
...
For each calculation window, Formulas \eqref{eqn: time-series linear regression 0} and \eqref{eqn: time-series linear regression 1} present the least-squares line-fit of $Y = \alpha + \beta j$ for a window width $2H + 1$ before the time index $j$ \cite{Prev_Precision_Arithmetic}.
...
\delta^2 \beta_{j} &= \sum_{X=-H}^{H} X^2 (\delta Y_{j-H+X})^2;
\end{align}
When the time series contains uncertainty, directly applying Formulas \eqref{eqn: moving-window linear regression 0} and \eqref{eqn: moving-window linear regression 1} results in a loss of precision because both formulas reuse each input multiple times, thereby accumulating the variance of that input with every reuse.
...
Formula \eqref{eqn: moving-window linear regression variance 1} is not progressive because a progressive occurrence of $\delta^2 \beta_j$ would be more expensive to compute than direct application of Formula \eqref{eqn: moving-window linear regression variance 1}.
...
The fitted values of $\alpha$ and $\beta$ follow the expected behavior, exhibiting a characteristic delay of $H$ in $j$.
...
Although mathematically self-consistent, DFT implies a periodically repeats in the time domain \cite{Prev_Precision_Arithmetic}.
...
\end{itemize}
The forward and reverse transformations are identical except for a sign difference, implying that they are essentially the same algorithm, and any observed difference arises mainly from the input data. 
...
\item The reverse transformation spreads the precise frequency-domain spectrum (where most values are zero) into a time-domain sine or cosine signal, causing its uncertainties to grow more slowly. 
\end{itemize}
The question is whether variance arithmetic can work effectively in these two contrary cases.
...
Even though the data for forward and reverse transformations are drastically different, variance arithmetic works effectively in both cases.
...
\captionof{figure}{
Error distributions of normalized errors of Sin/Cos signal without added input noise for forward, reverse and round-trip transformations (legend) computed using the Library sine function.
The FFT order is $18$.
}
...
Figure \ref{fig: Sin_Diff} indicates that the numerical errors of $sin(x)$ obtained using the Library sine function increase with a periodicity of $\pi$, which can resonate with a signal whose periodicity is an integer multiply of $\pi$, producing the resonant pattern shown in Figure \ref{fig: FFT_Sin_Clean_6_3_Wave_Lib}.
...
Figure \ref{fig: FFT_Linear_Clean_Histo_Lib} shows that the reverse error distribution when $\delta x = 0$ is no longer bounded.
...
\captionof{figure}{
Error distributions for Linear signals with $10^{-3}$ input noise for forward, reverse and roundtrip transformations (legend) computed using the Library sine function.
...
\begin{tabular}{|c|c|c|c|c|} 
\hline 
Signal     & Sine             & Forward       & Reverse       & Roundtrip    \\ 
\hline 
...
\captionof{table}{
The measured average error deviations for forward and reverse transformations and the measured error slopes for the roundtrip transformations for difference signal and sine functions.
...
Table \ref{tbl: fft error stats} shows the measured average error deviation for forward and reverse transformations and the measured error slope for roundtrip transformations for different signals and sine functions.
...
As a consequence, Figure \ref{fig: FFT_Reverse_Uncertainty_Ratio} shows that the ratio of of the maximal value error to the interval range decreases linearly with FFT order.
Such trend of uncertainty over-estimation is independent of input noise type (Gaussian vs Uniform), the choice of interval range (use prefixed fold of deviation or use actual input range), the transformation (forward, reverse, or round-trip), the signal under test (Sin, Cos, or Linear), and the sine function (Quart, or Library).
While statistical Taylor expansion tracks the output value errors precisely with error deviation of $1$ as shown in Table \ref{tbl: ideal coverage}, exactly the same calculation using interval arithmetic results in over-estimation of uncertainty which is linearly proportional to the amount of calculation.
Interval arithmetic tracks the worse-case of output range, thus it is unsuitable to track random uncertainty with order-of-magnitude over-estimation which is difficult to quantify in general. 
Thus, statistical Taylor expansion has much wider applicable range because the uncertainties in most applications are statistical in nature \cite{Statistical_Methods}\cite{Precisions_Physical_Measurements}.
...
\captionof{figure}{
The resulting precision (left y-axis) and uncertainties (right y-axis) of $\sin(\pi j/2^{18})$ versus index $j$ (x-axis) using either Quart or recursive sine function (legend).
}
...
\end{align}
Formulas \eqref{eqn: phase sin} and \eqref{eqn: phase cos} compute $\sin(j \pi/2^L), \cos(j \pi/2^L), j = 0 \dots 2^{L - 2}$ recursively for regression order $L = 0 \dots 17$ starting from Formula \eqref{eqn: phase boundary}.
Formula \eqref{eqn: regr error} shows that such regression guarantees both $\sin(x)^2 + \cos(x)^2 = 1$ and $\sin(2x) = 2\sin(x) \cos(x)$, such that value errors will not accumulate when the recursion order $L$ increases.
...
Unlike the hidden catastrophic cancellation in floating-point arithmetic, variance arithmetic uses coarser precision to demonstrate where and by how much the regression algorithm becomes unfit to compute $\sin(x)$.
...
Suppose a data set contains $N$ samples $X_j, j = 1 \dots N$, to fit with a parameter $c_0$ such that the residual $R_0 \equiv \sum_{j} (X_j - c_0)^2 /N$ is minimized.
...
\end{align}
Suppose the data set of $N$ samples is partitioned into $J$ sub sets, where each $j$-th sub set has samples $N_j$, mean $\overline{x_j}$, and variance $(\delta x_j)^2$. Formulas \eqref{eqn: set mean} and \eqref{eqn: set variance} give the set mean and variance, respectively, in which $N_j/N$ is the weight of the $j$-th sub set.
...
\iffalse
For $n = 0$, the intercept Formula \eqref{eqn: multi intercept} with $x_m = x^m$ rearranges to $\sum_{p=0}^{P} c_p\, \overline{x^p} = \overline{y}$, which is Formula \eqref{eqn: poly normal} at $n = 0$ (using $\overline{x^0} = 1$).
For $n = 1, \dots, P$, splitting the $p = 0$ term from the left side of Formula \eqref{eqn: poly normal}, substituting that intercept, and applying the slope solution \eqref{eqn: multi slope} with the polynomial covariances of Formula \eqref{eqn: poly nu} recovers Formula \eqref{eqn: poly normal}, as Formula \eqref{eqn: poly normal from multi} shows:
\begin{align}
\label{eqn: poly nu}
C_{np} &\equiv \nu(x^n, x^p) = \overline{x^{n+p}} - \overline{x^n}\,\overline{x^p}, \qquad
\nu_n \equiv \nu(x^n, y) = \overline{x^n y} - \overline{x^n}\,\overline{y}; \\
\label{eqn: poly normal from multi}
\sum_{p=0}^{P} c_p\, \overline{x^{p+n}}
&= \overline{x^n}\,\overline{y} + \sum_{p=1}^{P} c_p\, C_{np}
= \overline{x^n}\,\overline{y} + \nu_n
= \overline{x^n y}, \quad n = 1, \dots, P;
\end{align}
\fi
...
When the data set is partitioned into subsets, the uncorrelated uncertainty condition cannot hold between $x^m$ and $x^n$.
...
However, because Equation \eqref{eqn: poly perp foot} has no general solution when the polynomial power $P$ is larger than $2$, Equation \eqref{eqn: poly normal} is used for general polynomial regression.
...
\begin{itemize}
\item For a distribution test, the error distribution should be Normal, with error deviation being $1$.
\item For a value test, the error distribution should be Delta, with error slope being $-1$.
\end{itemize}

Variance arithmetic simplifies statistical Taylor expansion by introducing numerical rules that eliminate invalid results, such as divergent, negative variance, unstable, infinite, or unreliable.
...
The measured error slope rates so far are slightly but significantly more than $-1$.
...
If the sign bit of the floating type can be repurposed as an exponent bit in a new unsigned floating-point representation, the range of the deviations will be identical to that of the values.
...
Figure \ref{fig: Normal_Bounding_Leakage} shows that linear leakage converges toward $0$ with increasing sample count $N$ faster when $\kappa_s$ is larger.
It indicates that the ideal bound range $\hat{\kappa}$ should be as large as possible just to ensure convergence.
However, such an approach is valid only if the statistical meaning of the ideal leakage for the selection of ideal bounding becomes clear.
...
Unrelated to this work, the author is very grateful for Prof. Lizhi Fang (Arizona University) and Prof. Paul Hough (Brookhaven National Lab) for their life-long lead in adventures in science.
...
All data sets used in this study are all generated in the open-source project at \url{https://github.com/Chengpu0707/VarianceArithmetic}. 
