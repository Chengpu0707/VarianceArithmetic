...
Statistical Taylor expansion brings not only numerical enhancement, but also conceptual extension to the conventional mathematical concepts.
For example, sample count should be ubiquitous in analysis, and this study suggests that the conventional least-squares regression \cite{Statistical_Methods}\cite{Probability_Statistics}\cite{Numerical_Recipes} is defective without using sample counts.
...
$f[0] = \overline{x}$ and $R_0 = (\delta x)^2$ where $\overline{x}$ and $(\delta x)^2$ are the mean and variance of the data set.
...
$(\delta x)^2$ contains both the sub set variance $(\delta x_j)^2$ and the inter sub set variance $(\overline{x_j} - \overline{x})^2$.
\begin{align}
\label{eqn: set mean}
...
If $N_j$ is unknown, and if further \emph{the data set is assumed to be independent and identically distributed}, $1/(\overline{x_j} - \overline{x})^2$ can replace $N_j/N$ as the weight because after bootstrapping the underlying distribution mean as $\overline{x}$, $(\overline{x_j} - \overline{x})^2$ is proportional to $1/N_j$ according to central limit theorem \cite{Probability_Statistics}.
The resulting Equation \eqref{eqn: set mean approx} for $\overline{x}$ includes a non-zero correction $\varpi$ because $(\overline{x_j} - \overline{x})^2 \sim 1/N_j$ holds only in expectation, and the solution with the least $|\varpi|$ is the most likely.
The $\overline{x}$ solution yields $(\delta x)^2$ in Formula \eqref{eqn: set variance approx}.
Equation \eqref{eqn: set mean approx} requires $J > 3$ for sufficient information; for example, with $J = 3$ and $\overline{x_j} \in \{2, 3, 5\}$, setting $\varpi = 0$ yields two solutions $\overline{x} \in \{\frac{10 - \sqrt{7}}{3}, \frac{10 + \sqrt{7}}{3}\}$, which are non-unique due to lack of information.
...
The common practice to use $1/(\delta x_j)^2$ as the sub set weight \cite{Statistical_Methods}\cite{Probability_Statistics}\cite{Numerical_Recipes} for regression seems to have no statistical ground.
In addition, using $1/(\delta x_j)^2$ as weight prevents the mixing of precise and imprecise values in the data set, while Formulas \eqref{eqn: set mean}, \eqref{eqn: set variance}, \eqref{eqn: set mean approx} and \eqref{eqn: set variance approx} all permit such mixture.


\subsection{First Order Regression}

Suppose a data set contains $N$ samples $(X_j, Y_j), j = 1 \dots N$, to fit with two parameters $Y_j = f[0] + f[1] X_j$ such that the residual $R_1 \equiv \sum_{j} (Y_j - f[1] X_j - f[0])^2 /N$ is minimized, as in Formula \eqref{eqn: set residual}, in which $\nu(x,y) \equiv \overline{x y} - \overline{x}\;\overline{y}$ is defined as the co-variance between x and y.
...
Suppose the data set of $N$ samples is partitioned into $J$ sub sets, where each $j$-th sub set has samples $N_j$, mean $\overline{x_j}$, $\overline{y_j}$, variance $(\delta x_j)^2$,  $(\delta y_j)^2$, and co-variance $\nu(x,y)_j$. 
Formula \eqref{eqn: set mean} provides set $\overline{x}$ and $\overline{y}$, Formula \eqref{eqn: set variance} yields set $(\delta x)^2$ and $(\delta y)^2$, while Formula \eqref{eqn: set cov} gives the set co-variance in which the first term is sub set co-variance and the second term is inter sub set co-variance.
\begin{align}
\label{eqn: set cov}
\nu(x,y) = \sum_j \frac{N_j}{N} \big( \nu(x,y)_j + (\overline{x_j} - \overline{x})(\overline{y_j} - \overline{y}) \big);
\end{align}
Conventional regression \cite{Statistical_Methods}\cite{Probability_Statistics}\cite{Numerical_Recipes} contains two defects: ignoring vital information in regression formulas such as $N_j$, $\delta x_j$, $\delta y_j$, and $\nu(x,y)_j$, and ad hoc usage of either $1/(\delta x_j)^2$ or $1/(\delta y_j)^2$ or $1/(\delta x_j)^2 \; 1/(\delta y_j)^2$ as weights.
Both defects stem from patching $\delta x_j$ and $\delta y_j$ into the original least-squares analysis that used precise values only.
