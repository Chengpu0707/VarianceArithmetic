...
A smaller $P(x)$ indicates higher measurement quality of $x \pm \delta x$.
...
For example, Normal distribution is the normalized form of Gaussian distributions.
...
Although Formula \eqref{eqn: Taylor 2d variance} is only 2-dimensional, it can be extended easily to any higher dimensions.
...
As a trade-off, the choice of $\kappa$ to calculate the stable $f(x) - \overline{f(x)}$ and $\delta^2 f(x)$ is defined as the \emph{ideal bound range} $\hat{\kappa}$, and the result is the  \emph{ideal variance} $\hat{\delta^2} f(x)$.
...
Figure \ref{fig: Normal_Bounding_Leakage} shows that $\omega(\kappa_s, N + 1) < \omega(\kappa_s, N)$, and $\lim_{N \rightarrow \infty} \omega(\kappa_s, N) = \omega(\kappa_s)$ along the y-axis direction, and that $\kappa(\kappa_s, N + 1) > \kappa(\kappa_s, N)$ and $\lim_{N \rightarrow \infty} \kappa(\kappa_s, N) = \kappa_s$ along the x-axis direction. 
...
Dependency tracing requires applying statistical Taylor expansion to an analytic form of the function in one step.
...
\end{itemize}
\ifdefined\Verbose
A similar trend holds when the input uncertainty follows other distribution.
...
For the value test, the \emph{error noise ratio} which is the slope between the error deviation and the input noise of a linear regression, should be exactly $-1$,
...
\end{align}
Because the maximal expansion term using Formula \eqref{eqn: polynomial Taylor} is $\tilde{x}^{2N}$, $N$ in Formula \eqref{eqn: polynomial Taylor} can reach only half of the maximal expansion order of Formula \eqref{eqn: Taylor 1d variance}, for example, $2j=224 = 448/2$ for $\zeta(2n,5)$ of Gaussian input uncertainty.
...
For example, when $c$ is a natural number $n$, $(x \pm \delta x)^c$ becomes a polynomial which has no upper bound for $\delta x$, in contrast to when $c$ is not a natural number as shown in Figure \ref{fig: Pow_Conv_Edge}.
...
This abnormality is caused by floating-point rounding errors at this particular point, resulting in an error deviation of $3.2$. 
...
\delta^2 \left( \begin{matrix} w, x \\ y, z \end{matrix} \right)^{-1} \simeq&\; \frac{
	 	\left( \begin{matrix} z^4, x^2 z^2 \\ y^2 z^2,  x^2 y^2 \end{matrix} \right) \zeta(2) (\delta w)^2 +
	 	\left( \begin{matrix} y^2 z^2, w^2 z^2 \\ y^4, w^2 y^2 \end{matrix} \right)\zeta(2)  (\delta x)^2}{(w z - x y)^4} + \\
	&\; \frac{\left( \begin{matrix} x^2 z^2, x^4 \\ w^2 z^2, w^2 x^2 \end{matrix} \right) \zeta(2) (\delta y)^2 +
	 	\left( \begin{matrix} x^2 y^2, w^2 x^2 \\ w^2 y^2, w^4 \end{matrix} \right) \zeta(2) (\delta z)^2
	}{(w z - x y)^4}; \nonumber
\end{align}
...
Figure \ref{fig: Forward_Error_vs_Size_Noise} shows the error deviation of a typical value test: the error deviation decreases linearly with increasing input uncertainty deviation $\delta x$ with a error noise ratio of $-0.985 \pm 0.003$.
...
Contrary to conventional approach, statistical Taylor expansion uses Formula \eqref{eqn: inverse matrix} for matrix inversion because logically, the result should be symmetric for all matrix elements as demonstrated by Formula \eqref{eqn: inverse matrix 2 variance}, instead of the more conventional Gaussian elimination \cite{Numerical_Recipes}.
...
For each calculation window, Formulas \eqref{eqn: time-series linear regression 0} and \eqref{eqn: time-series linear regression 1} present the least-square line-fit of $Y = \alpha + \beta j$ for a window width $2H + 1$ before the time index $j$ \cite{Prev_Precision_Arithmetic}.
...
\item For a value test, the error distribution should be Delta, with error deviation noise ration being $-1$.
\end{itemize}
...
However, such approach is valid only if the statistical meaning of the ideal leakage for the selection of ideal bounding becomes clear.
