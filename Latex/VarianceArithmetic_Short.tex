...
\ifdefined\Verbose
Formula \eqref{eqn: uniform moment} gives $\zeta(2n, \kappa)$ for Uniform distributions, Formula \eqref{eqn: symetric asymptotic moment} shows the asymptoticity for any symmetric distribution $\rho(\tilde{z})$.
Figure \ref{fig: Gaussian_5_Moment} shows $\zeta(2n)$ for Uniform distribution, $\zeta(2n, 5)$ for Normal distribution. and the asymptote for Normal distribution.
\fi
Formula \eqref{eqn: variance asymptotic} shows that the asymptotic behavior of $\zeta(n, \kappa)$ as $n \rightarrow \infty$ determines whether Formula \eqref{eqn: Taylor 1d variance} will converge.
\begin{itemize}
...
\end{align}
Because the maximal expansion term using Formula \eqref{eqn: polynomial Taylor} is $\tilde{x}^{2N}$, $N$ in Formula \eqref{eqn: polynomial Taylor} can reach only half of the maximal expansion order of Formula \eqref{eqn: Taylor 1d variance}, for example, $N = 224 = 448/2$ for $\zeta(2n,5)$ of Gaussian input uncertainty.
...
Figure \ref{fig: Adjugate_Error_vs_Size_Noise} shows that the error deviations of $\widetilde{\mathbf{M}^A} - \mathbf{M}^A$ are very close to $1$, as $0.986 \pm 0.051$ on average.

Figure \ref{fig: Adjugate_Error_vs_Size_Noise} contains an abnormal error deviation of $3.2$ when the matrix size is $8$ and the input uncertainty is $\delta x = 10^{-17}$.
Without added noise, when the matrix size is $8$, error distribution transforms from Delta distribution to Gaussian distribution due to the emergence of rounding errors.
With added noise less than $10^{-15}$, when the matrix size is less than $8$, error distribution is Gaussian with a Delta-like spike.
...
Figure \ref{fig: Forward_Error_vs_Size_Noise} shows the error deviation of a typical value test: the error deviation decreases linearly with increasing input uncertainty deviation $\delta x$ with error slopes very close to $-1$, as $-0.985 \pm 0.003$ on average.
...
$(\delta f[1])^2$ decreases with larger $(\delta x)^2$, while $(\delta f[0])^2$ decreases with larger $(\delta x)^2 /\overline{x}^2$ and reaches its lower bound $R_1/N$ as $\overline{x} \to 0$.
...
\end{align}
Formula \eqref{eqn: set slope precision} shows that $P(f[1])$ is the product of the central limit theorem factor $1/\sqrt{N}$ and the residual-to-signal ratio, while Formula \eqref{eqn: set R2} shows that the conventional goodness-of-fit $R^2$ depends only on the residual-to-signal ratio and not on $N$.
...
\iffalse
Formulas \eqref{eqn: set residual} to \eqref{eqn: set R2} is not invariant under swapping x and y. 
...
\iffalse
Suppose a data set contains $N$ samples $(X_{1j}, \dots, X_{Kj}, Y_j)$, $j = 1 \dots N$, to fit with $K+1$ parameters $Y_j = f[0] + \sum_{k=1}^{K} f[k]\, X_{kj}$ such that the residual $R_K$ in Formula \eqref{eqn: multi residual def} is minimized.
Stacking the $K$ $\nu(x_k, y)$ into a covariance vector $\boldsymbol{\nu}$ and the $K \times K$ covariances $\nu(x_k, x_l)$ into a covariance matrix $\mathbf{C}$, Formulas \eqref{eqn: multi slope} to \eqref{eqn: multi intercept variance} generalize Formulas \eqref{eqn: set slope} to \eqref{eqn: set intercept variance}.
\begin{align}
\label{eqn: multi residual def}
R_K &\equiv \frac{1}{N} \sum_j \left(Y_j - f[0] - \sum_{k=1}^{K} f[k]\, X_{kj}\right)^2; \\
\label{eqn: multi slope}
\bigl(f[1], \dots, f[K]\bigr)^T &= \mathbf{C}^{-1} \boldsymbol{\nu}; \\
\label{eqn: multi intercept}
f[0] &= \overline{y} - \sum_{k=1}^{K} f[k]\, \overline{x_k}; \\
\label{eqn: multi residual}
R_K &= (\delta y)^2 - \boldsymbol{\nu}^T \mathbf{C}^{-1} \boldsymbol{\nu}; \\
\label{eqn: multi slope var}
(\delta f[k])^2 &= \frac{R_K}{N}\, \bigl(\mathbf{C}^{-1}\bigr)_{kk}; \\
\label{eqn: multi intercept variance}
(\delta f[0])^2 &= \frac{R_K}{N} \left(1 + \overline{\mathbf{x}}^{\,T} \mathbf{C}^{-1} \overline{\mathbf{x}} \right); \\
\label{eqn: multi cross cov partition}
\nu(x_k, y) &= \sum_j \frac{N_j}{N} \Bigl( \nu(x_k, y)_j + \bigl(\overline{x_{k,j}} - \overline{x_k}\bigr)\bigl(\overline{y_j} - \overline{y}\bigr) \Bigr);
\end{align}
The off-diagonal entries $\nu(f[k], f[l])$ quantify collinearity: large $(\mathbf{C}^{-1})_{kl}$ makes individual slope estimates unreliable even when $R_K$ is small.
Conventional multi-dimensional regression is defective for the same reason as the one-dimensional case.
\fi
...
Formula \eqref{eqn: multi slope cov} shows that slopes from different dimensions may be mutually dependent, in which $\mathbf{C}$ is the covariance matrix with $(\mathbf{C})_{m,n} = \nu(x_m, x_n)$, and $R_M$ is the residual.
Such dependency makes slopes sensitive to small changes in input data.
This dependency can be avoided by rotating the sample space onto its principal axes (perhaps with dimension reduced) \cite{Linear_Algebra}, so it is not an inherent dependency problem for multi-dimensional linear regression.
\begin{align}
\label{eqn: multi slope cov}
\nu(f[m],\, f[n]) &= \frac{1}{N} R_M (\mathbf{C}^{-1})_{m,n} \quad m, n = 1, \dots, M;
\end{align}

When $\mathbf{C}$ is assembled from subsets, conventional multi-dimensional regression is defective for the same reason as the one-dimensional case.
...
R_P &\equiv \frac{1}{N} \sum_j \left(Y_j - \sum_{p=0}^{P} f[p]\, X_j^p\right)^2; \\
...
Formulas \eqref{eqn: poly C} and \eqref{eqn: poly nu} give the corresponding covariance matrix $\mathbf{C}$ and covariance vector $\boldsymbol{\nu}$, respectively.
...
\begin{align}
\label{eqn: poly C}
C_{m,n} & = \overline{x^{m+n}} - \overline{x^m}\,\overline{x^n}, \quad m,n = 1, \dots, P; \\
\label{eqn: poly nu}
\nu_k &= \nu(x^k, y) = \overline{x^k y} - \overline{x^k}\,\overline{y}, \quad k = 1, \dots, P; \\
\label{eqn: multi slope}
\big(f[1], \dots, f[P]\big)^T &= \mathbf{C}^{-1} \boldsymbol{\nu};
\end{align}
