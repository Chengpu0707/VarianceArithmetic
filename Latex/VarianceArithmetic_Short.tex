...
Differentiating $R_P$ with respect to each $f[n]$ and setting $\partial R_P / \partial f[n] = 0$ for $n = 0, 1, \dots, P$ yields Equation \eqref{eqn: poly normal}.
\begin{align}
...
It can also be applied as a multi-dimensional linear regression with $x^p$ for each dimension, and its residual is also Formula \eqref{eqn: poly residual def}.
...
Even when $\overline{x} \rightarrow 0$, $f[m]$ inherently depends on others, except between odd and even parities when $x$ is symmetrically distributed.
