...
Stacking the $K$ $\nu(x_k, y)$ into a covariance vector $\boldsymbol{\nu}$ and the $K \times K$ covariances $\nu(x_k, x_l)$ into a covariance matrix $\mathbf{C}$, Formulas \eqref{eqn: multi slope} to \eqref{eqn: multi intercept variance} generalize Formulas \eqref{eqn: set slope} to \eqref{eqn: set intercept variance} to $K$ predictors.
\begin{align}
...
\end{align}
where $\overline{\mathbf{x}} = (\overline{x_1}, \dots, \overline{x_K})^T$.
$R_K$ is non-negative because $\mathbf{C}$ is positive semi-definite, and decreases monotonically with $K$.
...
Polynomial regression fits $N$ samples $(X_j, Y_j)$ to $\hat{Y}_j = \sum_{p=0}^{P} f[p]\, X_j^p$ by minimizing the residual in Formula \eqref{eqn: poly residual def}.
...
Equivalently, polynomial regression can be applied as a multi-dimensional linear regression with $x^p$ for each dimension, and the result residual is also Formula \eqref{eqn: poly residual def}.
Formulas \eqref{eqn: poly C} and \eqref{eqn: poly nu} gives the corresponding covariance matrix and covariance vector, respectively.
\begin{align}
\label{eqn: poly C}
...
Because $x^m$ and $x_n$ are not independent of each other, $|(\mathbf{C}^{-1})_{mn}|$ are generally significant.
For example, when $(\delta x)/\overline{x} \rightarrow 0$, $\nu(x^m, x^n) \rightarrow (\delta x^m) (\delta x^m)$ making $\mathbf{C}$ near-singular and $|(\mathbf{C}^{-1})_{mn}|$ large and unstable.
Even when $\overline{x} \rightarrow 0$, $f[m]$ for large $m$ inherently depends on others.
Thus only power regression for $y = f[0] + f[P] x^P$ is absolutely reliable.
