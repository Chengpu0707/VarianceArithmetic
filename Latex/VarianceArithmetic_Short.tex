...
A full expansion illuminates the convergence of the result.
...
Because $f$ contains an unknown amount of rounding error \cite{Rounding_Error}\cite{Precise_Numerical_Methods}\cite{Algorithms_Accuracy}, a 32-bit floating-point representation with $10^{-7}$ resolution may not be sufficient for inputs with $10^{-2}$ to $10^{-6}$ precision.
However, statistical Taylor expansion can account for rounding errors as part of the result deviation $\delta f$.

The bounding range in interval arithmetic \cite{Interval_Analysis}\cite{Worst_Case_Error_Bounds}\cite{Interval_Analysis_Theory_Applications}\cite{Interval_Arithmetic}\cite{Interval_Analysis_Notations} is inconsistent with the statistical nature of an $x \pm \delta x$ pair and it tends to grossly over-estimate the result uncertainty because of its worst-case assumption \cite{Prev_Precision_Arithmetic}.
Even the interval Taylor methods that bound the Taylor remainder with interval arithmetic, such as the Taylor model \cite{Taylor_Models} and the Taylor form \cite{Taylor_Forms_Limits}, inherit this over-estimation.
Partitioning each input interval into $n$ pieces and taking the union of the per-piece results reduces the over-estimation of the natural interval extension at a linear rate of $O(1/n)$ \cite{Interval_Analysis}\cite{Interval_Analysis_Theory_Applications}.
However, for an expression in $d$ variables this requires $O(n^d)$ evaluations \cite{Interval_Global_Optimization}.
The subdivision can also cause branched results, such as calculating square root for an input interval containing zero.
In contrast, statistical Taylor expansion is precise statistically, and it diverges when the input precision is not fine enough, such as requiring the input precision to be finer than $1/\sqrt{3}$ for square root when the input is Uniform distributed.
...
When inputs contain no systematic error \cite{Statistical_Methods}\cite{Precisions_Physical_Measurements}, their uncertainties are uncorrelated even though their values may be significantly correlated.
...
It also distinguishes statistical Taylor expansion from conventional theory of propagation of uncertainty \cite{Uncertainty Propagation}\cite{Uncertainties Library}.
...
\begin{itemize}
\item 
A \emph{distributional pole} occurs when $y^{(1)}_x=0 \rightarrow \rho(\tilde{y}, \mu_y, \sigma_y) = \infty$.
For example, $(\mu \pm 1)^2$ has a distributional pole at $\tilde{x} = 0$, as shown in Figure \ref{fig: Square_Distribution}.

\item 
A \emph{distributional zero} occurs when $y^{(1)}_x=\infty \rightarrow \rho(\tilde{y}, \mu_y, \sigma_y) = 0$.
For example, $\sqrt{\mu \pm 1}$ has a distributional zero at $\tilde{x} = 0$, as shown in Figure \ref{fig: Square_Root_Distribution}.

\end{itemize}

In both Figures \ref{fig: Square_Distribution} and \ref{fig: Square_Root_Distribution}, $\rho(\tilde{y}, \mu_y, \sigma_y)$ closely resembles $\rho(\tilde{x}, \mu, \sigma)$ when the mode lies sufficiently far from either a distributional pole or zero, as in $(5 \pm 1)^2$ and $\sqrt{5 \pm 1}$.
In such cases, the output $f(x)$ can be fully characterized by its mean $\overline{f(x)}$ and deviation $\delta f(x)$. 
...
\begin{align}
\label{eqn: Taylor 2d}
f(x + \tilde{x}, y + \tilde{y}) &= f(x, y) + \sum_{m=0, n=0}^{m+n \geq 1} \frac{f^{(m,n)}_{(x,y)}}{m! n!} \tilde{x}^m \tilde{y}^n; \\
\label{eqn: Taylor 2d mean}
\overline{f(x,y)} &= f(x, y) + \sum_{m=0, n=0}^{m+n \geq 2} (\delta x)^m (\delta y)^n \frac{f^{(m,n)}_{(x,y)}}{m!\;n!} \zeta_x(m, \kappa_x) \zeta_y(n, \kappa_y);  \\
\label{eqn: Taylor 2d variance}
\delta^2 f(x, y) &= \sum_{m=0, n=0}^{m+n \geq 2} (\delta x)^m (\delta y)^n \sum_{i=0}^{m} \sum_{j=0}^{n}
...
\begin{align}
\label{eqn: sin mean}
\overline{\sin(x)} &= \sin(x) + \sum_{n=2}^{\infty} (\delta x)^n \sin(x)_x^{(n)} \frac{\zeta(n, \kappa)}{n!}; \\
\label{eqn: sin precision}
\delta^2 \sin(x) =& \sum_{n=2}^{\infty} (\delta x)^n \sum_{j=1}^{n-1} \sin(x)_x^{(j)}\sin(x)_x^{(n-j)} \frac{\zeta(n, \kappa) - \zeta(j, \kappa) \zeta(n - j, \kappa)}{j! (n-j)!};
\end{align}
...
This is also structurally consistent with eqn: symetric asymptotic moment: the symmetric case integrates $\tilde z^{2n}$ over both endpoints $\pm\kappa$ (factor 2), whereas the one-sided Exponential is dominated by the single endpoint $\kappa$ (factor 1) — which is exactly the difference between the two formulas.
...
One small suggestion: since eqn: symetric asymptotic moment and this one are leading-order asymptotic equivalences (the RHS depends on $n$), the "$\lim_{n\to\infty}\zeta = \ldots$" notation is slightly loose — "$\zeta(n,\kappa)\sim\ldots$ as $n\to\infty$" would be more precise. Want me to adjust that notation in both formulas?
...
\rho(\tilde{z}) = \frac{1}{2\sqrt{3}}, |\tilde{z}| \leq \sqrt{3}&:
		\zeta(2n, \kappa) = \int_{-\kappa}^{+\kappa} \tilde{z}^{2n} \frac{1}{2 \sqrt{3}} d \tilde{z}
			= 2 \rho(\kappa) \frac{\kappa^{2n+1}}{2n + 1}; \\
\label{eqn: symetric asymptotic moment}
\rho(-\tilde{z}) =\rho(\tilde{z})&: \lim_{n \rightarrow +\infty} \zeta(2n, \kappa) = 2 \rho(\kappa) \frac{\kappa^{2n+1}}{2n+1}; \\
\label{eqn: exponential asymptotic moment}
\rho(\tilde{z}) = e^{-(1+\tilde{z})}, \tilde{z} \geq -1&: \lim_{n \rightarrow +\infty} \zeta(n, \kappa) = \rho(\kappa) \frac{\kappa^{n+1}}{n+1}; \\
\label{eqn: variance asymptotic}
0 < j < n&: \lim_{n \rightarrow +\infty} \zeta(n, \kappa) - \zeta(j, \kappa) \zeta(n-j, \kappa) \simeq \frac{\kappa^{n + 1}}{n + 1};
\end{align}
Formula \eqref{eqn: uniform moment} gives the closed form of $\zeta(2n, \kappa)$ for the Uniform distribution.
Formula \eqref{eqn: symetric asymptotic moment} states the large-$n$ asymptote for any symmetric $\rho(\tilde{z})$.
Formula \eqref{eqn: exponential asymptotic moment} states the corresponding asymptote for the standardized Exponential distribution under the mean-reverting bounding condition of Formula \eqref{eqn: mean-reverting bounding} which reduces to $(1+\varrho)\,e^{-(1+\varrho)} = (1+\kappa)\,e^{-(1+\kappa)}, \varrho < 0, \kappa > 0$.
\ifdefined\Verbose
...
As $\kappa \rightarrow +\infty$, $\zeta(2, \kappa) \rightarrow 1$ so that $\delta^2 f(x)$ stabilizes, but the convergence range of $\delta^2 f(x)$ shrinks toward zero for Formulas \eqref{eqn: log precision} and \eqref{eqn: power precision}.
The value of $\kappa$ that yields stable $f(x) - \overline{f(x)}$ and $\delta^2 f(x)$ is defined as the \emph{ideal bound range} $\hat{\kappa}$, and the corresponding result is the \emph{ideal variance} $\hat{\delta^2} f(x)$.
For a Uniform distribution, by definition $\hat{\kappa} = \sqrt{3}$.
For a Gaussian distribution, according to the 5-$\sigma$ rule for determining the statistical significance of an experimental result \cite{Statistical_Methods}\cite{Precisions_Physical_Measurements}\cite{Probability_Statistics},  $\hat{\kappa} = 5$ by default.
...
Because Formula \eqref{eqn: sum variance} comes from variance definition, Formula \eqref{eqn: Taylor 1d variance} does not contain dependence problem, in contrast to Taylor expansion using interval arithmetic \cite{Interval_Analysis}\cite{Taylor_Forms_Limits}.
...
\end{align}
\fi
This equivalence suggests that Formula \eqref{eqn: poly normal} implies that $x^m$ and $x^n$ obey the uncorrelated uncertainty condition, which is incorrect conceptually.
