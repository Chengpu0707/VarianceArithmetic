...
\item Section \ref{sec: Math Library} discusses the evaluation of variance arithmetic on common mathematical library functions, showing the effect of distribution pole.
...
\end{align}
Under the uncorrelated uncertainty condition, Formulas \eqref{eqn: Taylor 2d mean} and \eqref{eqn: Taylor 2d variance} compute the mean and variance of the Taylor expansion given in Formula \eqref{eqn: Taylor 2d}, where $\zeta_x(m)$ and $\zeta_y(n)$ denote the bound moments for $x$ and $y$, respectively.
...
\begin{align}
\label{eqn: exp mean}
\frac{\overline{e^x}}{e^x}  &= 1 + \sum_{n=1}^{\infty} (\delta x)^n \zeta(n) \frac{1}{n!}; \\
\label{eqn: exp precision}
\frac{\delta^2 e^x}{(e^x)^2} &= \sum_{n=2}^{\infty} (\delta x)^n \sum_{j=1}^{n-1} \frac{\zeta(n) - \zeta(j) \zeta(n - j)}{j!\;(n - j)!};
\end{align}
...
\sin(x + \tilde{x}) &= \sum_{n=0}^{\infty} \eta(n, x) \frac{\tilde{x}^{n}}{n!};
	...
	\end{cases} \\
\label{eqn: sin mean}
\overline{\sin(x)} =& \sum_{n=0}^{\infty} (\delta x)^n \eta(n, x) \frac{\zeta(n)}{n!}; \\
...
Figure \ref{fig: Bounding_Factor_Leakage} sections Figure \ref{fig: Normal_Bounding_Leakage} along the y-axis direction for $\kappa_s=4,5,6$, and shows that to reach the stable variance for a $\kappa_s$, the required sample count is $N \gtrsim 10^3$, whereas the stable result bounding leakage is smaller for larger bounding range $\kappa$.
It also shows that linear leakage converges to $0$ with increasing sample count $N$ faster when $\hat{\kappa}$ is larger.
...
Figure \ref{fig: Normal_Function} shows that the ideal leakages for the selected functions are less than $10^{-4}$, which is small enough for most applications. 
...
For a reversible transformation such as matrix reversion or FFT (Fast Fourier Transformation), after a \emph{round-trip transformation} which is a forward transformation followed by a reverse transformation, the original input should be restored.
...
Consequently, approximation strategies should prioritize variances before values.
\fi
\end{itemize}
...
Fourmula \eqref{eqn: determinant Taylor expansion} presents the Taylor expansion of the determinant for matrix $\mathbf{M}$ when the uncertainties of matrix elements are all independent of each other, whereas Formulas \eqref{eqn: determinant mean} and \eqref{eqn: determinant variance} gives the statistical Taylor expansion of the determinant $|\mathbf{M}|$ \cite{Linear_Algebra}.
...
The choice of ideal bounding range $\hat{\kappa}$ should be extended to other distributions, perhaps by using the same value of $1 - \zeta(2, \kappa)$.
The procedure to find the bound moments for each sample count should be developed for discrete distributions.
