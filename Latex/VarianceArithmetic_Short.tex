...
It fundamentally differs from common approaches in applied mathematics that optimize computational path for each calculation.
...
It is desirable each value from library functions should be accomplished by a uncertainty deviation.  
\end{abstract}
...
Statistical Taylor expansion determines the result $f \pm \delta f$ of a general analytic expression $f(x, \dots)$. 
\begin{itemize}
\item 
...
Conventional floating-point arithmetic \cite{Computer_Architecture}\cite{Floating_Point_Arithmetic}\cite{Floating_Point_Standard} can only calculate the result $f$, while suffering from rounding error during calculation \cite{Rounding_Error}\cite{Precise_Numerical_Methods}\cite{Algorithms_Accuracy}, so that its $10^{-16}$ representation resolution may not be enough for inputs with $10^{-5}$ to $10^{-6}$ precision for heave calculation.
...
On the other hand, statistical Taylor expansion is precise in statistics.
...
\item Section \ref{sec: polynomial} illustrates variance arithmetic in computing polynomial, demonstrating the capability of variance arithmetic in tracing floating-point rounding errors.
...
\item 
A \emph{distributional zero} of the uncertainty distribution occurs when $f^{(1)}_x=\infty \rightarrow \rho(\tilde{y}, \mu_y, \sigma_y) = 0$.
...
However, as shown later in this paper, $\delta^2 \sin(x \pm \delta x)$ can become negative for large $\delta x$, which imposes an upper-bound constraint on the input $\delta x$.
...
Figure \ref{fig: Bounding_Factor_Leakage} further demonstrates that for smaller $\kappa_s$, $\kappa(\kappa_s, N)$ approaches $\kappa_s$ more rapidly as $N$ increases, but converges to a larger stable bounding leakage $\epsilon(\kappa_s)$.
...
\begin{itemize}
\item
When the underlying uncertainty distribution is Gaussian and $\kappa_s = 5$, Figure \ref{fig: Normal_Function} shows that the resulting variances $\delta^2 f$ for a selected functions rise with $N$ until reaching the corresponding stable values $\widehat{\delta^2} f$ when $N \geq 50$.
...
All functions approach their corresponding stable variances with exactly the same trend in each figure.
These observations suggest that bounding leakage is the reason for $\delta^2 f < \widehat{\delta^2} f$ and it is justified to infer the confidential interval range $\kappa$ from bounding leakage $\epsilon(N, \kappa)$ in Figure \ref{fig: Normal_Bounding_Leakage}.
...
Define the \emph{ideal ratio} as $\alpha \equiv \delta^2 f/\widehat{\delta^2} f$, which is $0$ when $N = 1$ and $1$ when $N$ is sufficiently large.
...
Beyond this upper bound, the result deviation is no longer positive.
Conceptually, the upper-bound for $\delta x$ should be less than $\pi$ to make the result meaningful, which Figure \ref{fig: Sin_Conv_Edge} satisfies.
\ifdefined\Verbose
...
\item \emph{Normalized error}: the ratio of a value error to the corresponding result deviation.
...
Once input error from every source is accounted precisely, \emph{ideal coverage} is achieved:
\begin{itemize}
\item The error deviation is exactly $1$, or $0$ near a distributional pole (where input uncertainty is suppressed for the result).

\item \emph{Distribution Test}: The error distribution is Normal when verifying the result mean and deviation.

\item \emph{Value Test}: The error distribution is Delta when verifying the result values.
...
\end{align}
Because the maximal expansion term using Formula \eqref{eqn: polynomial Taylor} is $\tilde{x}^{2N}$, Formula \eqref{eqn: polynomial Taylor} has only half of the maximal expansion order of Formula \eqref{eqn: Taylor 1d variance}.
...
This abnormality is caused by floating-point rounding errors when $\delta x \rightarrow 0$.
Variance arithmetic can only provide proper coverage for floating-point rounding errors. 
...
$\tilde{\mathbf{M}} \times \tilde{\mathbf{M}}^A - |\tilde{\mathbf{M}}| \mathbf{I}$ tests each value so that it is a value test.
Figure \ref{fig: Forward_Error_vs_Size_Noise} shows the error deviation of a typical value test: the error deviation decreases with increasing input uncertainty deviation $\delta x$, because the value should match unless at $\delta x \rightarrow 0$ floating-point rounding errors dominate.
...
\captionof{table}{
The result error deviation of selected basic functions using variance arithmetic, with $\delta x > 10^{-15}$.
...
The result error deviation is similar to Figure \ref{fig: Adjugate_Error_vs_Size_Noise} but with the specific dimension as $c$ in $(1 + \delta x)^c$, or $x$ in $e^{x \pm \delta x}$ and $\log(x \pm \delta x)$.
The coverage is no longer ideal when $ \delta x < 10^{-15}$.
...
The error deviations resemble Figure \ref{fig: Adjugate_Error_vs_Size_Noise} but with FFT order as the specific dimension, independent of Sin or Cos signals, or frequency of the signals.
...
It demonstrates that the reverse transformation is more sensitive to imperfections in Quart sine function, because its error distribution is structured on top of the Gaussian distribution.
...
The error deviations resemble Figure \ref{fig: Adjugate_Error_vs_Size_Noise} but with FFT order as the specific dimension.
In addition, the error deviations are larger than $1$ when $\delta x < 10^{-14}$.
...
The forward transformation exhibits a larger ideal coverage area than the reverse transformations when $\delta x \rightarrow 0$, which is consistent with smaller result deviations of the reverse transformation.
...
Table \ref{tbl: ideal coverage} shows the minimal required noise to achieve ideal coverage for FFT transformations at FFT order $18$ for different signals and sine functions, which is consistent with the corresponding error distributions in Figure \ref{fig: FFT_SinCos_Clean_Histo_Indexed}, \ref{fig: FFT_SinCos_Clean_Histo_Lib}, \ref{fig: FFT_Linear_Clean_Histo_Indexed}, and \ref{fig: FFT_Linear_Clean_Histo_Lib}.
This shows that an error distribution can qualify its input uncertainty quantification.
Without known the precise result, similar histogram can be constructed from the calculated $\overline{f}$ and $\delta f$, and the result $f$ data set.
It worth investigating if such empirical histogram has similar power.
...
When uncorrelated uncertainty condition is met, statistical Taylor expansion outputs the mean and deviation of an analytic expression.
It tracks the variable dependencies in intermediate steps, rejects invalid calculations, and explicitly incorporates the sample counts and the underlying input uncertainty distributions into the statistical analysis.
...
Formula \eqref{eqn: Taylor 1d variance} converges in all cases only when the input uncertainty is bounded due to sampling, which resembles the need for re-normalization in quantum field theory.
The quantitative dependence of convergence on the choice of ideal bounding range $\hat{\kappa}$ suggests the principle of quantum physics: what it is depends on how it is measured.
