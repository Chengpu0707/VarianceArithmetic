...
As a rigorous extension of conventional Taylor expansion, statistical Taylor expansion replaces precise input variables with random input variables of known distributions and sample counts, to compute the mean, deviation, and reliability of each result.
...
As a result, it differs fundamentally from conventional approaches, which are usually path-dependent, and may serve as a unified standard for numerical computations of analytic expressions.
This study further presents an implementation of statistical Taylor expansion called variance arithmetic, and demonstrates its performance across diverse mathematical applications.
...
Formula \eqref{eqn: symetric asymptotic moment} states the large-$n$ asymptote for any symmetric $\rho(\tilde{z})$, with the prefactor $2 \rho(\kappa)$ given by the parent density at the boundary.
Formula \eqref{eqn: exponential asymptotic moment} states the corresponding asymptote for the standardized Exponential distribution under the mean-reverting bounding condition of Formula \eqref{eqn: mean-reverting bounding}; the one-sided support yields a single prefactor $\rho(\kappa)$ rather than the symmetric $2 \rho(\kappa)$.
For the Exponential, Formula \eqref{eqn: mean-reverting bounding} reduces to $(1+\varrho)\,e^{-(1+\varrho)} = (1+\kappa)\,e^{-(1+\kappa)}$, whose non-trivial root $\varrho \in (-1, 0)$ is paired with the user-chosen upper bound $\kappa > 0$.
...
For example, when the sample count $N$ is sufficiently large:
\begin{itemize}
...
\end{align}
Because the variance computation via Formula (2.6) on Formula (5.1) requires moments up to order $2N$, $N$ in Formula \eqref{eqn: polynomial Taylor} can reach only half of the maximal expansion order of Formula \eqref{eqn: Taylor 1d variance}, for example, $N = 224 = 448/2$ for $\zeta(2n,5)$ of Gaussian input uncertainty.
...
In Figure \ref{fig: Matrix_Determinant_Prec_vs_Condition}, adding small noise to a matrix does not change its conditional number, but increases result precision, so that determinant precision cannot replace matrix condition number.  
...
Suppose a data set contains $N$ samples $X_j, j = 1 \dots N$, to fit with a parameter $f[0]$ such that the residual $R_0 \equiv \sum_{j} (X_j - f[0])^2 /N$ is minimized.
The solution is $f[0] = \overline{x}$ and $R_0 = (\delta x)^2$, where $\overline{x}$ and $(\delta x)^2$ are the mean and variance of the data set.
...
\iffalse 
Formulas \eqref{eqn: set slope} and \eqref{eqn: set intercept} give $f[1]$ and $f[0]$, respectively, while Formulas \eqref{eqn: set slope variance} and \eqref{eqn: set intercept variance} yield their variances, respectively, which are both determined by $R_1/N$.
$(\delta f[1])^2$ decreases with larger $(\delta x)^2$ as expected.
Because $(\delta f[0])^2$ changes with the choice of the coordinate origin for $x$, it has little statistical merit.

\begin{align}
\label{eqn: set slope variance}
&(\delta f[1])^2 = \frac{R_1}{N} \frac{1}{(\delta x)^2}; \\
\label{eqn: set intercept variance}
&(\delta f[0])^2 = \frac{R_1}{N} \left( 1 + \frac{\overline{x}^2}{(\delta x)^2} \right);
\end{align}

\fi

Suppose a data set contains $N$ samples $(X_j, Y_j), j = 1 \dots N$, to fit with two parameters $Y_j = f[0] + f[1] X_j$ such that the residual $R_1 \equiv \sum_{j} (Y_j - f[1] X_j - f[0])^2 /N$ is minimized, as in Formula \eqref{eqn: set residual}, where $\nu(x,y) \equiv \overline{x y} - \overline{x}\;\overline{y}$ is the covariance between $x$ and $y$.
Formulas \eqref{eqn: set slope} and \eqref{eqn: set intercept} give $f[1]$ and $f[0]$, respectively, which shows that $f[0]$ is derived from $f[1]$ and thus is worth no further discussion.
Formula \eqref{eqn: set slope precision} shows that the precision of the slope $P(f[1])$ is the product of the central limit theorem factor $1/\sqrt{N}$ and the residual-to-signal ratio, while Formula \eqref{eqn: set R2} shows that the conventional goodness-of-fit $R^2$ depends only on the residual-to-signal ratio and not on $N$.
Whether $P(f[1])$ or $R^2$ is the better measure of goodness-of-fit is debatable.
\begin{align}
\label{eqn: set residual}
R_1 &\equiv \frac{1}{N} \sum_{i=1}^N (Y_i - f[0] - f[1] X_i)^2; \Rightarrow R_1 = (\delta y)^2 - \frac{\nu(x, y)^2}{(\delta x)^2} ; \\
\label{eqn: set slope}
f[1] &= \frac{\nu(x,y)}{(\delta x)^2}; \\
\label{eqn: set intercept}
f[0]  &= \overline{y} - \overline{x} f[1]; \\
\label{eqn: set slope precision}
P(f[1])^2 &= \frac{1}{N} \frac{R_1 (\delta x)^2}{\nu(x,y)^2}; \\
\label{eqn: set R2}
...
Formulas \eqref{eqn: set residual} to \eqref{eqn: set R2} are not invariant under swapping $x$ and $y$. 
A swap-consistent regression (total least squares, geometric-mean regression, or Deming) \cite{Linear_Regression} is more appropriate when both variables carry uncertainty, which indicates a future development of regression using imprecise values.
...
Formula \eqref{eqn: multi slope} provides the solution for $f[m]$, in which $\mathbf{C}$ is the covariance matrix with $(\mathbf{C})_{m,n} = \nu(x_m, x_n)$, and $\boldsymbol{\nu}$ is the covariance vector $(\boldsymbol{\nu})_m = \nu(x_m, y)$.
...
R_M &\equiv \frac{1}{N} \sum_{i=1}^N \left(Y_i - f[0] - \sum_{m=1}^{M} f[m]\, X_{m,i}\right)^2 
		\Rightarrow R_M = (\delta y)^2 - \boldsymbol{\nu}^T \mathbf{C}^{-1} \boldsymbol{\nu}; \\
\label{eqn: multi slope}
&\big(f[1], \dots, f[M]\big)^T = \mathbf{C}^{-1} \boldsymbol{\nu}; \\
\label{eqn: multi slope cov}
&\nu(f[m],\, f[n]) = \frac{1}{N} R_M (\mathbf{C}^{-1})_{m,n} \quad m, n = 1, \dots, M;
\end{align}
...
For $n = 0$, Formula \eqref{eqn: multi intercept} rearranges to $\sum_{p=0}^{P} f[p]\, \overline{x^p} = \overline{y}$, which is Formula \eqref{eqn: poly normal} at $n = 0$.
For $n = 1, \dots, P$, substituting Formula \eqref{eqn: multi intercept} for $f[0]$ in the left-hand side of Formula \eqref{eqn: poly normal} and applying Formulas \eqref{eqn: multi slope} and \eqref{eqn: poly nu} gives Formula \eqref{eqn: poly normal from multi}.
\begin{align}
\label{eqn: poly normal from multi}
\sum_{p=0}^{P} f[p]\, \overline{x^{p+n}}
&= \overline{x^n}\,\overline{y} + \sum_{p=1}^{P} f[p]\, C_{np}
...
Because of the inter-dependency between $x^m$ and $x^n$, it seems that polynomial regression should not exist in general.

Polynomial regression fits $N$ samples $(X_j, Y_j)$ to $\hat{Y}_j = \sum_{p=0}^{P} f[p]\, X_j^p$ by minimizing the residual in Formula \eqref{eqn: poly residual def} using the least-squares method directly on $f[p]$.
Differentiating $R_P$ with respect to each $f[n]$ yields Equation \eqref{eqn: poly normal}.
\begin{align}
\label{eqn: poly residual def}
R_P &\equiv \frac{1}{N} \sum_{i=1}^N (Y_i - \sum_{p=0}^{P} f[p]\, X_i^p)^2; \\
\label{eqn: poly normal}
&\sum_{p=0}^{P} f[p]\, \overline{x^{p+n}} = \overline{x^n y}, \quad n = 0, 1, \dots, P; 
\end{align}
...
Because $x^m$ and $x^n$ are not independent of each other, $|(\mathbf{C}^{-1})_{mn}|$ is generally significant.
For example, when $(\delta x)/\overline{x} \rightarrow 0$, $\nu(x^m, x^n) \rightarrow (\delta x^m) (\delta x^n)$ making $\mathbf{C}$ near-singular with large $|(\mathbf{C}^{-1})_{mn}|$ in an unstable $\mathbf{C}^{-1}$, so that polynomial regression should be carried out in a coordinate with $\overline{x} = 0$.
...
\subsection{Symmetric Linear Regression}

The total-least-squares fit minimizes the squared perpendicular distance from each sample $(X_j, Y_j)$ to the line $Y_j = f[0] + f[1] X_j$, yielding the residual in Formula \eqref{eqn: tls residual def}.
\begin{align}
\label{eqn: tls residual def}
R_1^{\perp} &\equiv \frac{1}{N(1 + f[1]^2)} \sum_j (Y_j - f[1] X_j - f[0])^2;
\end{align}
Setting $\partial R_1^{\perp}/\partial f[0] = 0$ recovers the OLS intercept Formula \eqref{eqn: set intercept}; setting $\partial R_1^{\perp}/\partial f[1] = 0$ yields the quadratic Formula \eqref{eqn: tls slope quadratic}, whose root with $\mathrm{sign}(f[1]) = \mathrm{sign}(\nu(x,y))$ is the slope Formula \eqref{eqn: tls slope}.
The minimum residual Formula \eqref{eqn: tls residual} retains the OLS form $R_1^{\perp} = (\delta y)^2 - f[1]\, \nu(x,y)$ but with $f[1]$ from Formula \eqref{eqn: tls slope} rather than Formula \eqref{eqn: set slope}.
The slope variance Formula \eqref{eqn: tls slope variance} follows from principal-axis perturbation theory, $(\delta f[1])^2 = (1 + f[1]^2)^2\, \lambda_+ \lambda_-/(N\,(\lambda_+ - \lambda_-)^2)$, with the Jacobian factor $(1 + f[1]^2)^2$ converting the principal-angle variance into slope variance.
\begin{align}
\label{eqn: tls slope quadratic}
0 &= \nu(x,y)\, f[1]^2 + ((\delta x)^2 - (\delta y)^2)\, f[1] - \nu(x,y); \\
\label{eqn: tls slope}
f[1] &= \frac{(\delta y)^2 - (\delta x)^2 + \sqrt{((\delta y)^2 - (\delta x)^2)^2 + 4\, \nu(x,y)^2}}{2\,\nu(x,y)}; \\
\label{eqn: tls residual}
R_1^{\perp} &= (\delta y)^2 - f[1]\, \nu(x,y)
= \frac{(\delta x)^2 + (\delta y)^2 - \sqrt{((\delta x)^2 - (\delta y)^2)^2 + 4\, \nu(x,y)^2}}{2}; \\
\label{eqn: tls slope variance}
(\delta f[1])^2 &= \frac{(1 + f[1]^2)^2 \bigl((\delta x)^2 (\delta y)^2 - \nu(x,y)^2\bigr)}{N\, \bigl(((\delta x)^2 - (\delta y)^2)^2 + 4\, \nu(x,y)^2\bigr)};
\end{align}
$R_1^{\perp}$ is the smaller eigenvalue $\lambda_-$ of the covariance matrix $\bigl(\begin{smallmatrix}(\delta x)^2 & \nu(x,y) \\ \nu(x,y) & (\delta y)^2\end{smallmatrix}\bigr)$, while $\lambda_+ = (\delta x)^2 + (\delta y)^2 - R_1^{\perp}$ is the variance along the fitted line; the slope direction $(1, f[1])$ is the eigenvector of $\lambda_+$.
The numerator $(\delta x)^2 (\delta y)^2 - \nu(x,y)^2 = \lambda_+ \lambda_-$ vanishes at perfect correlation, where $f[1]$ is determined exactly and $(\delta f[1])^2 \to 0$.
When $(\delta x)^2 = (\delta y)^2$, Formula \eqref{eqn: tls slope} reduces to $f[1] = \mathrm{sign}(\nu(x,y))$, the $45^{\circ}$ line through the centroid.

Under the $x \leftrightarrow y$ swap, $\nu(x,y)$ is unchanged while $(\delta x)^2$ and $(\delta y)^2$ exchange, so Formula \eqref{eqn: tls slope quadratic} becomes $\nu(x,y)\, b'^2 - ((\delta x)^2 - (\delta y)^2)\, b' - \nu(x,y) = 0$.
Substituting $b' = 1/f[1]$ and multiplying by $f[1]^2$ recovers Formula \eqref{eqn: tls slope quadratic}, so the swapped TLS slope is $1/f[1]$ --- the fitted line is the same in $(x, y)$ space regardless of which variable is treated as the predictor.
