...
0 &= (x_i - X_i) + \left( \hat{y}(x_i) - Y_i \right) \frac{d \hat{y}(x_i)}{d x_i}; \\
C &= (x_i - X_i)^2 + \left( \hat{y}(x_i) - Y_i \right)^2;
\end{align*}
\fi
...
\end{align}
\fi
This equivalence suggests that Formula \eqref{eqn: poly normal} implies that $x^m$ and $x^n$ are independent, which is not valid conceptually.
...
When $P = 1$, Formulas \eqref{eqn: tls slope} and \eqref{eqn: tls slope variance} shows that the least square solution is symmetric between $\delta x$ and $\delta y$, which is an improvement over Formulas \eqref{eqn: set slope} and \eqref{eqn: set slope precision} because the latter bias toward smaller absolute slope values \cite{Linear_Regression}.
...
This compromise of false independence introduces dependence problem into the regression.
...
These conditioning problems are well known and motivate the use of orthogonal polynomial bases (Chebyshev, Legendre, Hermite) \cite{Polynomial Regression}, which shows that \emph{a dependency problem can be resolved by orthogonal transformations under restricted conditions}.
