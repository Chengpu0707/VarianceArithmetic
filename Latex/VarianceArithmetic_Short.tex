...
Formula \eqref{eqn: multi slope cov} shows that slopes from different dimensions may be inter dependent, in which $\mathbf{C}$ is the covariance matrix with $(\mathbf{C})_{m,n} = \nu(x_m, x_n)$, and $R_M$ is the residual.
Such colinearity makes slopes sensitive to small changes in input data.
\begin{align}
\label{eqn: multi slope cov}
\nu(f[m],\, f[n]) &= \frac{1}{N} R_M (\mathbf{C}^{-1})_{m,n} \quad m, n = 1, \dots, M;
\end{align}

When assemble $\mathbf{C}$ from subset, conventional multi-dimensional regression is defective for the same reason as the one-dimensional case.
...
Conversely, the multi-dimensional regression of Formula \eqref{eqn: multi slope} also yields Formula \eqref{eqn: poly normal}, confirming that these two methods are equivalent.
\begin{align}
\label{eqn: poly C}
...
Because $x^m$ and $x^n$ are not independent of each other, a collinearity $|(\mathbf{C}^{-1})_{mn}|$ is generally significant.
For example, when $(\delta x)/\overline{x} \rightarrow 0$, $\nu(x^m, x^n) \rightarrow (\delta x^m) (\delta x^n)$ making $\mathbf{C}$ near-singular with large $|(\mathbf{C}^{-1})_{mn}|$ in an unstable $\mathbf{C}^{-1}$.
