...
Because $f$ contains unknown amount of rounding error \cite{Rounding_Error}\cite{Precise_Numerical_Methods}\cite{Algorithms_Accuracy}, a 32-bit floating-point representation with $10^{-7}$ resolution may not be good enough for inputs with $10^{-2}$ to $10^{-6}$ precision.
...
\begin{align}
\label{eqn: sin mean}
\overline{\sin(x)} =& \sum_{n=0}^{\infty} (\delta x)^n \sin(x)_x^{(n)} \frac{\zeta(n, \kappa)}{n!}; \\
...
\ifdefined\Verbose
For example, Formula \eqref{eqn: uniform moment} gives $\zeta(2n, \kappa)$ for Uniform distributions, Formula \eqref{eqn: symetric asymptotic moment} shows the asymptoticity for any symmetric distribution $\rho(\tilde{z})$.
...
Figure \ref{fig: Bounding_Factor_Leakage} sections Figure \ref{fig: Normal_Bounding_Leakage} along the y-axis direction for $\kappa_s=4,5,6$, and shows that to reach the stable variance for a $\kappa_s$, the required sample count is $N \gtrsim 10^3$, whereas the stable resulting linear leakage $\omega$ is smaller for larger measuring bounding range $\kappa_s$.
It also shows that linear leakage converges toward $0$ with increasing sample count $N$ faster when $\kappa_s$ is larger.
...
\end{align}
Define \emph{ideal leakage} as $\varepsilon \equiv 1 - \delta^2 f/\widehat{\delta^2} f \simeq \omega$, which quantifies the reliability of $\hat{\delta^2} f$ from the underlying distributions and the corresponding sample counts of all inputs, with Formulas \eqref{eqn: sum leakage} and \ref{eqn: product leakage} as two examples.
...
The unbiased bounding range $[\varrho, \kappa]$ of Uniform distribution is estimated as $1 + \frac{1}{N}$ times of the minimal and maximal values in sampled data set \cite{Uniform_Distrobution Estimator}.
...
The difference between Figures \ref{fig: Normal_Function} and \ref{fig: Uniform_Function} shows the effect of input uncertainty distribution to the output deviation $\delta f$.
...
It has almost the same trend of upper bounds $P(x)$ increasing with $c$ as shown in Figure \ref{fig: Pow_Conv_Edge_Uniform} but just with different y-scaling: $1/\sqrt{3}$ instead of  $1/5$.
\else
Similar trends holds around $\hat{\kappa} = \sqrt{3}$ when the input uncertainty follows Uniform distribution.
\fi 
...
In addition to convergence, the variance expansion may yield negative results when the expansion is terminated arbitrarily, as in Formula \eqref{eqn: sin precision} for $sin(x \pm \delta x)$, such that an upper bound for $\delta x$ is need to guarantee non-negative variance.
...
Because $\sin(x) \in [-1, +1]$, $\overline{\sin(x)} \pm \delta \sin(x)$ should also follow this range, as indicated in \ref{fig: Sin_Conv_Edge}.
\ifdefined\Verbose
...
Let $[j_1, j_{2} \dots j_m]_n$ and $(-1)^{[j_1, j_{2} \dots j_m]_n}$ denote a permutation of $<[j_1, j_{2} \dots j_m]>_n$ and the corresponding sign \cite{Linear_Algebra}, in which $<[j_1, j_{2} \dots j_m]>_n$ denote the corresponding ordered permutation.
...
It could relate to floating-point representation.
...
For each calculation window, Formulas \eqref{eqn: time-series linear regression 0} and \eqref{eqn: time-series linear regression 1} present the least-square line-fit of $Y = \alpha + \beta j$ \cite{Prev_Precision_Arithmetic}.
...
However, DFT if not a faithful digital implementation of continuous Fourier transform (FT) \cite{Numerical_Recipes}.
...
\item The reverse transformation spreads the precise frequency-domain spectrum (where most values are zero) back into a time-domain sine or cosine signal, causing its uncertainties to grow more slowly. 
\end{itemize}
...
\end{align}
Formulas \eqref{eqn: phase sin} and \eqref{eqn: phase cos} calculate $\sin(j \pi/2^L), \cos(j \pi/2^L), j = 0 \dots 2^{L - 2}$ repressively for regression order $L = 0 \dots 17$ starting from Formula \eqref{eqn: phase boundary}.
