...
This approach tracks the propagation of input uncertainties through intermediate steps, making final result path independent.
Therefore, it differs fundamentally from conventional approaches which are usually path dependent, so that it may standardize numerical computations for analytic expressions.
...
The study also reveals potential and substantial impact of numerical errors in library functions. 
...
Statistical Taylor expansion determines the result $f \pm \delta f$ and reliability of a general analytic expression $f(x, \dots)$ based on the inputs $x \pm \delta x, \dots$ and their corresponding distributions and sample counts. 
\begin{itemize}
\item 
...
\item Calculating the reliability $\alpha \in [0, 1]$ of $f \pm \delta f$ seems completely new.
\end{itemize}
...
However, statistical Taylor expansion can account rounding errors as part of the result deviation $\delta f$.
...
\begin{enumerate}
\item Section \ref{sec: introduction} briefly compares statistical Taylor expansion with all other numerical arithmetic methods.
...
\item Section \ref{sec: polynomial} illustrates variance arithmetic in polynomial computation, demonstrating its capability to trace floating-point rounding errors and its continuity in parameter space.

\item Section \ref{sec: matrix} applies variance arithmetic to matrix inversion, distinguishing between distribution tests and value tests.

\item Section \ref{sec: Math Library} evaluates variance arithmetic on common mathematical library functions, showing the effect of distribution pole.
...
When there is no systematic error \cite{Statistical_Methods}\cite{Precisions_Physical_Measurements} among the inputs, their uncertainties are uncorrelated, even though their values can have significant correlations among them.
...
Viewed in the $f^{-1}(\tilde{y})$ coordinate, $\rho(\tilde{y}, \mu_y, \sigma_y)$ is given by $\rho(\tilde{x}, \mu, \sigma)$ multiplied by $1/f^{(1)}_x$, in which $f^{(1)}_x$ is the first derivative of $f(x)$ with respect to $x$.
\begin{itemize}
\item 
...
For any symmetric $\rho(-\tilde{z}) = \rho(\tilde{z})$, $\varrho = -\kappa$ and $\zeta(2n+1) = 0$.
...
Under the uncorrelated uncertainty condition, Formulas \eqref{eqn: Taylor 2d mean} and \eqref{eqn: Taylor 2d variance} compute the mean and variance of the Taylor expansion given in Formula \eqref{eqn: Taylor 2d}, where $\zeta_x(m)$ and $\zeta_y(n)$ denote the variance moments for $x$ and $y$, respectively.
...
Statistical Taylor expansion rejects the distributional zero of $\log(x)$ or $(x \pm \delta x)^c$ in the range of $P(x) \gtrapprox 1/\hat{\kappa}$ statistically due to the divergence of Formulas \eqref{eqn: log convergence} and \eqref{eqn: pow convergence} mathematically, with $\zeta(2n, \hat{\kappa})$ providing the connection between these two perspectives.
...
\captionof{figure}{
Measured result range $\kappa$ (left y-axis) and corresponding measured result leakage $\varepsilon(\kappa)$ (right y-axis) for varying sample count $N$ (x-axis) when the underlying distribution is normal, with different measuring bounding range $\kappa_s$ (legend).
}
...
Figures \eqref{eqn: offset approximation} and \eqref{eqn: variance approximation} show the first-order approximation of Formulas \eqref{eqn: Taylor 1d mean} and \eqref{eqn: Taylor 1d variance}, respectively.
...
When the underlying distribution is Normal, Formula \eqref{eqn: Gaussian result bounding leakage} presents the difference between the result leakage $\varepsilon(\kappa)$ and the bounding leakage $\epsilon(\kappa)$, while Formulas \eqref{eqn: Gaussian theoretical bounding leakage} and \eqref{eqn: Gaussian experimental bounding leakage} give the distributional bounding leakage $\epsilon(\kappa)$ and the sample bounding leakage $\epsilon(\kappa_s, N)$ respectively, where $\xi()$ is the Normal error function \cite{Probability_Statistics}.
Figure \ref{fig: Normal_Bounding_Leakage} shows that $\epsilon(\kappa_s) < \epsilon(\kappa_s, N)$, and $\lim_{N \rightarrow \infty} \epsilon(\kappa_s, N) = \epsilon(\kappa_s)$ along the y-axis direction, and that $\kappa(\kappa_s, N) < \kappa_s$ and $\lim_{N \rightarrow \infty} \kappa(\kappa_s, N) = \kappa_s$ along the x-axis direction. 
...
Dependency tracing requires an analytic form of the function to apply statistical Taylor expansion for the result mean and variance, as in Formulas \eqref{eqn: Taylor 1d mean}, \eqref{eqn: Taylor 1d variance}, \eqref{eqn: Taylor 2d mean}, and \eqref{eqn: Taylor 2d variance}.
...
For example, after the respective upper bounds $P(x)$ for $(x \pm \delta x)^c$ are normalized by the corresponding $\hat{\kappa}$, they show almost the same trends of increasing with $c$.
\ifdefined\Verbose
...
Empirically, error distributions are close to Normal when error deviations are close to 1, regardless of input uncertainty distribution, and such convergence occurs rapidly \cite{Prev_Precision_Arithmetic}.
...
Formulas \eqref{eqn: determinant mean} and \eqref{eqn: determinant variance} gives the statistical Taylor expansion of the determinant of a square matrix $\mathbf{M}$ when the uncertainties of matrix elements are all independent of each other.
...
By assigning a random integer between $[-2^8, +2^8]$ to each element, $\mathbf{M}$ is created.
By adding Gaussian noise of deviation $\delta x$ to each element of $\mathbf{M}$, $\widetilde{\mathbf{M}}$ is created.
The value error of $\mathbf{M}^A$ is the difference between $\widetilde{\mathbf{M}^A}$ and $\mathbf{M}^A$, whereas the result deviation is calculated using Formula \eqref{eqn: determinant variance}, so that $\widetilde{\mathbf{M}^A} - \mathbf{M}^A$ is a distribution test.
...
Figure \ref{fig: Matrix_Determinant_Prec_vs_Condition} shows that there is a strong linear correlation between conditional numbers \cite{Linear_Algebra} and the corresponding determinant precision of the matrices.
...
\captionof{table}{
The result error deviation of selected basic functions with Gaussian input noise $x \pm \delta x$ when $\delta x > 10^{-15}$, obtained by using variance arithmetic.
...
When $x$ is 2's fractional such as $1/2$ or $1$, the resulting uncertainties are significantly smaller due to floating-point representation.
}
...
\captionof{figure}{
Histograms of normalized errors of Sin/Cos signals for forward, reverse and round-trip transformations (legend) using the Quart sine function.
The FFT order is $18$.  
}
...
When the FFT order $L$ is less than 8, the error deviations oscillate around $1$ due to an insufficient sample count of $2^L$.
...
When $L=18$ and $\delta x = 0$, Figure \ref{fig: FFT_SinCos_Clean_Histo_Indexed} shows that the error distributions of Sin/Cos signal resemble Normal distributions, with an additional Delta-like distribution at $\tilde{z} = 0$.
...
\captionof{figure}{
Histograms of normalized errors of Sin/Cos signal for forward, reverse and round-trip transformations (legend) computed using the Library sine function.
The FFT order is $18$.
}
...
With FFT order as the specific dimension, the error deviations obtained using the Library sine for the forward and the reverse transformations resemble those shown in Figure \ref{fig: Adjugate_Error_vs_Size_Noise}, while those for the round-trip transformation are almost identical to those in Figure \ref{fig: Forward_Error_vs_Size_Noise}.
...
As shown in Figure \ref{fig: Sin_Diff}, the Library sine function contains more numerical errors, as a result, the error distribution for the reverse transformation obtained using the Library sine function in Figure \ref{fig: FFT_SinCos_Clean_Histo_Lib} is more structured and broader than that in Figure \ref{fig: FFT_SinCos_Clean_Histo_Indexed} obtained using the Quart sine function, whereas the error distributions for the forward transformations are more similar.
...
When $\delta x = 0$, using the Library sine function, for a sine wave with a frequency of $3$, Figures \ref{fig: FFT_Sin_Clean_6_3_Spec_Lib}, \ref{fig: FFT_Sin_Clean_6_3_Wave_Lib}, and \ref{fig: FFT_Sin_Clean_6_3_Roundtrip_Lib} present the value errors for the forward, the reverse, and the round-trip transformations, respectively. 
...
\captionof{figure}{
Histograms of normalized errors of Linear signals for forward, reverse and round-trip transformations (legend) computed using the Quart sine function.
The FFT order is $18$.
}
...
\captionof{figure}{
Histograms of normalized errors of Linear signals for forward, reverse and round-trip transformations (legend) computed using the Library sine function.
The FFT order is $18$.
}
...
Figures \ref{fig: FFT_Linear_Lib_Forward_ErrorDev_vs_Noise_Order} and \ref{fig: FFT_Linear_Lib_Reverse_ErrorDev_vs_Noise_Order} show a much smaller ideal coverage areas than those in Figures \ref{fig: FFT_Linear_Indexed_Forward_ErrorDev_vs_Noise_Order} and \ref{fig: FFT_Linear_Indexed_Reverse_ErrorDev_vs_Noise_Order}.
Because result deviations grow more slowly in the reverse transformation than in the forward transformation, the former exhibits a smaller ideal coverage region.
...
Although FFT is widely regarded as one of the most robust numerical algorithms \cite{Numerical_Recipes}\cite{Precise_Numerical_Methods}, and is generally insensitive to input errors, it can still fail due to numerical errors in the Library sine function.
Such deterioration in calculation precision is not easily detectable when using conventional floating-point arithmetic.

Figure \ref{fig: FFT_Linear_Lib_Roudtrip_ErrorDev_vs_Noise_Order} shows that, even when variance arithmetic can no longer effectively track the value errors for either the forward or the reverse transformations, it can still effectively track the value errors for the round-trip transformation, as shown by the plateau region at high $L$ and low $\delta x$ in Figure \ref{fig: FFT_Linear_Lib_Roudtrip_ErrorDev_vs_Noise_Order}.
...
Table \ref{tbl: ideal coverage} shows the minimal required noise to achieve ideal coverage for FFT transformations at FFT order $L=18$ for different signals and sine functions, which is consistent with the corresponding error distributions in Figures \ref{fig: FFT_SinCos_Clean_Histo_Indexed}, \ref{fig: FFT_SinCos_Clean_Histo_Lib}, \ref{fig: FFT_Linear_Clean_Histo_Indexed}, and \ref{fig: FFT_Linear_Clean_Histo_Lib}.
An error distribution can qualify its input uncertainty quantification.
Without knowing precise result, similar histogram can be constructed from the result data set for $f$ using the calculated mean $\overline{f}$ and deviation $\delta f$.
...
\captionof{figure}{
The precision (left y-axis) and uncertainties (right y-axis) of $\sin(\pi j/2^{18})$ versus $j$ (x-axis) and $L$ using either Quart or regressive sine function (legend).
}
...
Formula \eqref{eqn: regr error} shows that such regression guarantees both $\sin(x)^2 + \cos(x)^2 = 1$ and $\sin(2x) = 2\sin(x) \cos(x)$, so that value errors will not accumulate when the regression order $L$ increases.
...
Unlike the sneaky catastrophic cancellation in floating-point arithmetic, variance arithmetic uses coarser precision to demonstrate where the regression algorithm starts to be unfit to compute $\sin(x)$.
...
When uncorrelated uncertainty condition is satisfied, statistical Taylor expansion produces the mean, deviation, and reliability of an analytic expression.
...
Library mathematical functions should be recalculated using variance arithmetic, to ensure each output value is accompanied by its corresponding uncertainty.
...
Bound momentum $\zeta(n, \kappa)$ needs to be extended to all probability distributions.
The choice of ideal bounding range $\hat{\kappa}$ needs to be extended to other distributions, perhaps by using the same value of $1 - \zeta(2, \kappa)$.
The procedure to find the bound moments for each sample count needs to be developed for discrete distributions.
The statistical meaning for ideal ratio $\alpha \in [0, 1]$ in the context of its choice of ideal bounding range $\hat{\kappa}$ needs further investigation: Is $\alpha$ the possibility for the result to be bound by $\hat{\kappa}$?
...
In variance arithmetic, deviations are comparable to values, but variances are used in calculation.
This effectively limits the range of deviations to the square root of that of the values.
...
Modern symbolic computation tools such as \textit{SymPy} and \text{Mathematica} can significantly facilitate such calculations.
...
This capability enables engineers to pinpoint the primary sources of measurement inaccuracy and guiding them to targeted improvements in data acquisition and processing strategies.
For example, Formula \eqref{eqn: sum leakage} can provide guidance on improving the ideal ratio of $x \pm y$.

Because conventional numerical approaches are based on floating-point arithmetic, they must be reexamined or even reinvented within the framework of variance arithmetic.
...
For input distribution without bounding, Formula \eqref{eqn: Taylor 1d variance} may not converge at all, for example, when the input uncertainty is Laplace \cite{Probability_Statistics}, $\zeta(2n) = (2n)!$.
