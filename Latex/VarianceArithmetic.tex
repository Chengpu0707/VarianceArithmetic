...
\hat{v} (f(x)g(x)) &= \sum_{n=1}^{\infty} (\delta x)^{2n} \zeta(2n) \sum_{j=1}^{2n-1}
    \left( \sum_{k=0}^{j} \frac{f^{(k)}_x}{k!} \frac{g^{(j-k)}_x}{(j-k)!} \right)
    \left( \sum_{l=0}^{2n-j} \frac{f^{(l)}_x}{l!} \frac{g^{(2n-j-l)}_x}{(2n-j-l)!} \right) \nonumber \\
&\eqspace - \left( \overline{f} g + f \overline{g} + 2 \hat{c} (f g) \right)^2;
\end{align}
...
For example, if Formula \eqref{eqn: multiplication precision} were applied instead of Formula \eqref{eqn: square precision}, the result variance would be only half.
...
Thus, the minimal non-zero bounding leakage is approximately $\frac{15.259}{16} 10^{-6}$ which is equivalent to $\sigma \leq 4.467$.
