...
\end{itemize}
The variance represents the reliable information content of a measurement statistically, with finer variance means higher reliability and better reproducibility of the measurement \cite{Statistical_Methods}\cite{Precisions_Physical_Measurements}.  
...
The goal of variance arithmetic is to calculate the uncertainty variance around each output value.
...
The correlated parts common to different measurements are regarded as signals, which can either be designed or unwanted.
...
The uncorrelated uncertainty assumption is consistent with the common methods in processing experimental data \cite{Statistical_Methods}\cite{Precisions_Physical_Measurements}, such as the common knowledge that precision improves with the count $n$ as $1 / \sqrt{n}$ during averaging.
...
To avoid the rounding error accumulation in the conventional floating-point representation, the variance arithmetic designs calculation starting from the bit level.
...
\subsection{Variance Representation}
...
However, such round-up of $V$ conflicts with the accuracy of $R$ which requires $V$ be reduced to only 1/2-fold during round up according to Formula \eqref{eqn: rounding error range vs deviation}.
...
The round-up rule of $V$ allows the uncertainty to be outside the bounding range with a small probability which is called \emph{bounding leakage}. 
When $P_{8/2}$ is replaced by $P_2$, the measured bounding leakage is 0.06\%.
Because $R$ is no longer ideal, the bounding leakage outside the range of $(-R, +R)$ is $\xi(R/\sqrt{V}/\sqrt{2})$, in which $\xi()$ is the Gaussian error function \cite{Probability_Statistics}, and $R/\sqrt{V}$ is defined as as the \emph{range ratio}.
...
Thus, $S\;V@E$ represents $x \pm \delta x$ as:
\begin{align}
...
To be compatible with the conventional 64-bit floating-point standard \cite{Floating_Point_Standard}, the bit counts for $S$ and $V$ are 53, while  the bit count for $E$ is 11.
In addition, the representation needs 1 bit for sign, and 1 bit for the carry error sign, making the total bit count to be 94.
...
\end{enumerate}
\begin{itemize}
\item When $p \le 26$, which is approximately $P \ge 1.5\;10^{-8}$, the significance bit count is $p + 26$, which means maximally 26-bit calculated inside uncertainty.

\item When $38 < p \leq 52$, the maximal significance bit count is kept at 53, and $V$ decreases toward 0 for larger $p$.
When $p=51$, which is $P = 0.5\;10^{-15}$ the bounding leakage is $10^{-6}$.  When $p=52$, the bounding leakage is $1.4\%$.

\item When $53 \leq p$, $V$ becomes 0, so that the variance representation represents a precise value which is identical to the conventional 64-bit floating-point value $S@E$.
\end{itemize}

Variance arithmetic assumes that the LSB of a conventional floating-point value is unreliable, so that $V = 1$ by default.
...
According to the central limit theorem, addition and subtraction make $x \pm \delta x$ Gaussian distributed, with Formula \eqref{eqn: central limit theorem} as the probability density function, in which $N()$ is the density function of the  normal distribution \cite{Probability_Statistics}. 
\begin{equation}
...
For example, the difference between $1.001 \pm 0.001$ and $1.000 \pm 0.002$ is $0.001 \pm 0.00224$, so that the probability for them to be equal is $\xi(-\frac{0.001}{0.00224}, +\frac{0.001}{0.00224}) = 94.6\%$, in which $\xi()$ is the Gaussian error function.
...
P(e^x)^2 &\simeq \delta^2 x + \frac{7-1}{4} (\delta^2 x)^2 + \frac{31-3}{24} (\delta^2 x)^3 + \frac{127-7}{192} (\delta^2 x)^4; 
\end{align*}
...
\delta^2 \ln(x) &\simeq P(x)^2 + \frac{11-1}{4} P(x)^4 + \frac{137-9}{12} P(x)^6 + \frac{1089-49}{16} P(x)^8;
\end{align*}

\begin{align*}
& (x + \tilde{x})^c - x^c = x^c ((1 + \frac{\tilde{x}}{x})^c - 1) = x^c \sum_{j=1}^{\infty} (\frac{\tilde{x}}{x})^j \prod_{k=1}^{j} \frac{c + 1 - k}{k}; \\
\widehat{x^c} &\simeq \frac{c(c-1)}{2} P(x)^2 + \frac{c(c-1)(c-2)(c-3)}{8} P(x)^4 + \frac{c(c-1)(c-2)(c-3)(c-4)(c-5)}{48} P(x)^6; \\
P(x^c)^2 &\simeq c^2 P(x)^2 + c^2(c-1)\frac{(7c-11) - (c-1)}{4} P(x)^4 + c^2 (c-1) (c-2) \frac{(31c^2-132c + 137) - (c^2-4c+3)}{24} P(x)^6;
\end{align*}

\fi

If $f(x)$ can be Taylor expanded as Formula \eqref{eqn: Taylor 1d}, the mean $\overline{f(x)}$ and variance $\delta^2 f(x)$ of the result distribution are calculated by Formula \eqref{eqn: function mean} and \eqref{eqn: function variance}, respectively.  
The mean $\overline{f(x)}$ can be decomposed as:
\begin{itemize}
...
& M(x, n) \equiv \int (\tilde{x} - x)^{n} \rho(\tilde{x}, x, \delta x) d\tilde{x}; \\
\label{eqn: function mean}
\overline{f(x)} &= f(x) + \widehat{f(x)}; \eqspace 
\widehat{f(x)} \equiv \sum_{n=1}^{\infty} \frac{f^{(n)}_x}{n!} M(x, n); \\
\label{eqn: function variance}
\delta^2 f(x) &= \sum_{n=1}^{\infty} \sum_{j=1}^{n-1} \frac{f^{(j)}_x}{j!} \frac{f^{(n-j)}_x}{(n-j)!} M(x, n) - \widehat{f(x)}^2; 
\end{align}

Formula \eqref{eqn: function mean} and \eqref{eqn: function variance} both contains \emph{variance momentum} which is defined in Formula \eqref{eqn: nth variance}.
The integration range of Formula \eqref{eqn: nth variance} is limited by $\tilde{x}$, such as in $\sqrt{x}$ or $1/x$, which affect the convergence of Formula \eqref{eqn: function mean} and \eqref{eqn: function variance}.
If the input uncertainty is given by Formula \eqref{eqn: central limit theorem}, and if $\tilde{x} = 0$ is the lower integration range bound, Formula \eqref{eqn: nth variance} can be simplified as Formula \eqref{eqn: Gaussian nth variance}.
\begin{align}
\label{eqn: Gaussian nth variance}
& \varsigma(x, n) = \sum_{j=1}^{n/2} \frac{x^{2j-1}}{(2j-1)!!} N(x); \nonumber \\
M(x, n) &= (\delta x)^n \int_{0}^{\infty} \tilde{z}^n N(\tilde{z}) d \tilde{z} = (\delta x)^n 
\begin{cases}
(\frac{1 + \xi(\frac{x}{\sqrt{2}})}{2} - \varsigma(x, n)) (n-1)!!, &\text{if $n$ is even} \\
\varsigma(x, n), &\text{if $n$ is odd}
\end{cases}; 
\end{align}

In Formula \eqref{eqn: function variance}, $\widehat{f(x)}^2$ has a very small contribution to $\delta^2 f(x)$.
In the \emph{accurate output approximation} of variance arithmetic, $\widehat{f(x)}^2$ is ignored, so that the variance of the function becomes Formula \eqref{eqn: Taylor 1d variance}, in which the variance is no longer calculated against the mean $\overline{f(x)}$, but against the accurate result $f(x)$.
...
\delta^2 f(x) &= \int (f(x + \tilde{x}) - f(x))^2 \rho(\tilde{x}, x, \delta x) d \tilde{x}; \\
...
\delta^2 f(x, y) &= \int \int (f(x + \tilde{x}, y + \tilde{y}) - f(x, y))^2 \rho(\tilde{x}, x, \delta x) \rho(\tilde{y}, y, \delta y)\; d \tilde{x} d \tilde{y} \nonumber \\
&= \sum_{m=0}^{\infty} \sum_{n=0}^{\infty} M(x, m) M(y, n) 
  \sum_{i=1}^{m-1} \sum_{j=1}^{n-1} \frac{f^{(i,j)}_{(x,y)}\;f^{(m-i,n-j)}_{(x,y)}}{i!(m-i)!\;j!(n-j)!} - f(x,y)^2;
\end{align}

When $\rho(\tilde{x}, x, \delta x)$ is Formula \eqref{eqn: central limit theorem}, Formula \eqref{eqn: exp bias}, \eqref{eqn: exp precision}, \eqref{eqn: log bias}, \eqref{eqn: log precision}, \eqref{eqn: power bias} and \eqref{eqn: power precision} give the result variance bias and precision for $e^x$, $\ln(x)$, and $x^c$, respectively.
\begin{align}
\label{eqn: exp bias}
\frac{\widehat{e^x}}{e^x} &\simeq \frac{1}{2} (\delta^2 x) + \frac{1}{8} (\delta^2 x)^2 + \frac{1}{48} (\delta^2 x)^3; \\
\label{eqn: exp precision}
P(e^x)^2 &\simeq (\delta^2 x) + \frac{7}{4} (\delta^2 x)^2 + \frac{31}{24} (\delta^2 x)^3; \\
\label{eqn: log bias}
\widehat{\ln(x)} &\simeq - \frac{1}{2} P(x)^2 - \frac{3}{4} P(x)^4 - \frac{5}{2} P(x)^6; \\
\label{eqn: log precision}
\delta^2 \ln(x) &\simeq P(x)^2 + \frac{11}{4} P(x)^4 + \frac{137}{12} P(x)^6; \\
\label{eqn: power bias}
\frac{\widehat{x^c}}{x^c} &\simeq \frac{c(c-1)}{2} P(x)^2 + \frac{c(c-1)(c-2)(c-3)}{8} P(x)^4 \nonumber \\
&\;\; + \frac{c(c-1)(c-2)(c-3)(c-4)(c-5)}{48} P(x)^6; \\
\label{eqn: power precision}
P(x^c)^2 &\simeq c^2 P(x)^2 + c^2(c-1)\frac{7c-11}{4} P(x)^4 \nonumber \\
&\;\; + c^2 (c-1) (c-2) \frac{31c^2-132c+137}{24} P(x)^6;
\end{align}
...
P(\sqrt{x})^2 &\simeq \frac{1}{4} P(x)^2 + \frac{15}{64} P(x)^4 + \frac{315}{512} P(x)^6; \\
\label{eqn: inversion precision}
P(1/x)^2 &\simeq P(x)^2 + 9 P(x)^4 + 75 P(x)^6;
\end{align}
...
Formula \eqref{eqn: Taylor 1d variance}, \eqref{eqn: Taylor 2d variance}, and their extensions can still be used to calculate the arithmetic calculations of the two functions, such as sum and product in Formula \eqref{eqn: sum dependency}, and \eqref{eqn: product dependency}, respectively.
\begin{align}
\label{eqn: sum dependency}
&\delta^2 (f(x)+g(x)) = \delta^2 f(x) + \delta^2 g(x) + 2 \sum_{n=0}^{\infty} M(x,n) \sum_{j=1}^{n-1} \frac{f^{(n-j)}_x }{(n-j)!} \frac{g^{(j)}_x}{j!} \\
&\eqspace \simeq \delta^2 f(x) + \delta^2 g(x) + 2f^{(1)}_x g^{(1)}_x (\delta^2 x) + 
...
The sum of all the $n > 1$ terms in Formula \eqref{eqn: Taylor 1d variance} is defined as the \emph{calculation cost} in variance arithmetic.
...
It is equivalent to Formula \eqref{eqn: stat +}, \eqref{eqn: stat -}, \eqref{eqn: stat *}, \eqref{eqn: stat /} by assuming $\gamma = 0$.
