...
Instead, only Formula \eqref{eqn: Fourier} will be used.
...
\item The major source of the calculation error for Sin/Cos signals is the calculation errors of $\sin(\pi j /2^L), j = 0, 1, 2 ... 2^{L-1}$.
The major source of the calculation error for \emph{Linear} signals is the calculation errors of $\tan(\pi (j - 1) /2^L), j = 1, 2 ... 2^{L-1}$.

\end{itemize}


\subsection{sin(x) and cos(x) to Use}

\begin{figure}[p]
\includegraphics[width=4.5in,height=2.75in]{Sin_Diff.png} 
\captionof{figure}{
The difference Between Library $\sin(x)$ and indexed $\sin(x)$, for $x \in [0, 8 \pi]$.
The indexed $\sin(x)$ extends $\sin(x), cos(x), x \in [0, \pi/4]$ to other $x$ using index frequency for a given FFT order.
In the figure, the FFT order is $7$.
}
\label{fig: Sin_Diff}
\end{figure}

\begin{figure}[p]
\includegraphics[width=4.5in,height=2.75in]{FFT_Sin_Clean_1_4_Spec_Indexed.png} 
\captionof{figure}{
The FFT spectrum of $\sin(j 1/2^4 \pi)$ using the indexed sine functions after the forward transformation calculated by the variance arithmetic, with the uncertainty and the value errors shown in the legend.
The x-axis shows the index frequency.
The y-axis shows the absolute value errors.
}
\label{fig: FFT_Sin_Clean_1_4_Spec_Indexed}
\end{figure}

\begin{figure}[p]
\includegraphics[width=4.5in,height=2.75in]{FFT_Sin_Clean_1_4_Spec_Lib.png} 
\captionof{figure}{
The FFT spectrum of $\sin(j 1/2^4 \pi)$ using the library sine functions after the forward transformation calculated by the variance arithmetic, with the uncertainty and the value errors shown in the legend.
The x-axis shows the index frequency.
The y-axis shows the absolute value errors.
}
\label{fig: FFT_Sin_Clean_1_4_Spec_Lib}
\end{figure}

\begin{figure}[p]
\includegraphics[width=4.5in,height=2.75in]{FFT_Sin_Clean_1_4_Wave_Indexed.png} 
\captionof{figure}{
The FFT waveform of $\sin(j 1/2^4 \pi)$ using the indexed sine functions after the reverse transformation calculated by the variance arithmetic, with the uncertainty and the value errors shown in the legend.
The x-axis shows the index frequency.
The y-axis shows the absolute value errors.
}
\label{fig: FFT_Sin_Clean_1_4_Wave_Indexed}
\end{figure}

\begin{figure}[p]
\includegraphics[width=4.5in,height=2.75in]{FFT_Sin_Clean_4_4_Wave_Lib.png} 
\captionof{figure}{
The FFT waveform of $\sin(j 4/2^4 \pi)$ using the indexed sine functions after the reverse transformation calculated by the variance arithmetic, with the uncertainty and the value errors shown in the legend.
The x-axis shows the index frequency.
The y-axis shows the absolute value errors.
}
\label{fig: FFT_Sin_Clean_4_4_Wave_Lib}
\end{figure}



Formula \eqref{eqn: Fourier} limits the use of $\sin(x)$ and $\cos(x)$ to $x = j \frac{2 \pi}{N}$.
...
\begin{enumerate}
\item Instead of a floating-point value $x$ for $\sin(x)$ and $\cos(x)$ for the \emph{library sine functions}, the integer $j$ is used to specify the input to $\sin$ and $\cos$, as $\sin(j \frac{2\pi}{N})$ and $\cos(j \frac{2\pi}{N})$, to avoid the floating-point rounding error of $x$.

\item The values of the indexed sine functions is extended from $j = 0,1,...\frac{N}{8}$ to the whole integer region using the periodicity of $\sin$ and $\cos$.
\end{enumerate}
Figure \ref{fig: Sin_Diff} shows that the value difference between the library sine and the indexed sine increases with increasing $x$.

Such difference will have a large impact on the result of numerical Fourier transformation.
For the waveform of $\sin(j f/2^4 \pi)$ in which $f$ is the index frequency, and $j$ is the index time:
\begin{itemize}
\item For the forward transformation, Figure \ref{fig: FFT_Sin_Clean_1_4_Spec_Indexed} shows that using the indexed sine functions, the result value errors are comparable to the result uncertainties, while Figure \ref{fig: FFT_Sin_Clean_1_4_Spec_Lib} shows that using the library sine functions, the result value errors are much larger to the result uncertainties.
Particularly, using the index sin functions, when the result uncertainties are $0$, the corresponding value errors are also $0$, and the error deviation is close to $1$.
On the other hand, using the library sine functions, such relation between the result uncertainties and the corresponding value errors no longer exists, so that  the error deviations can be as large as $10^{16}$.

\item For the reverse transformation, Figure \ref{fig: FFT_Sin_Clean_1_4_Wave_Indexed} shows that using the indexed sine functions, the result value errors are comparable to the result uncertainties, while Figure \ref{fig: FFT_Sin_Clean_4_4_Wave_Lib} shows that using the library sine functions, the result value errors are much larger to the result uncertainties.
The increase of the value errors in Figure \ref{fig: FFT_Sin_Clean_4_4_Wave_Lib} is consistent with the increase of the value errors in Figure \ref{fig: Sin_Diff}.
\end{itemize}

For FFT, it is clear that the indexed sine functions are much better than the library sine functions.
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_SinCos_Clean_Histo_Lib.png} 
\captionof{figure}{
The histograms of the normalized errors of Sin/Cos signals without input noises using the library sine functions for forward, reverse and roundtrip FFT transformations, as shown in the legend.
}
...
\captionof{figure}{
The result error deviations and uncertainty means for Sin/Cos signals of order 16 using indexed sin/cos functions vs. input uncertainties for forward, reverse and roundtrip FFT transformations.
...
\captionof{figure}{
The result error deviations and uncertainty means for Sin/Cos signals of order 16 using library sin/cos functions vs. input uncertainties for forward, reverse and roundtrip FFT transformations.
The error deviations are scaled to the linear y-axis on the right, while the uncertainty means are scaled to the logarithmic log y-axis on the right.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_SinCos_1e-13_vs_Order.png} 
\captionof{figure}{
...
\label{fig: FFT_SinCos_1e-13_vs_Order}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_SinCos_1e-13_Histo.png} 
\captionof{figure}{
...
\label{fig: FFT_SinCos_1e-13_Histo}
\end{figure}



Figure \ref{fig:  FFT_SinCos_vs_Noise_Indexed} shows the result error deviations and uncertainty means vs. different input uncertainties for Sin/Cos signals of order 16 using the indexed sin/cos functions.  
When the deviation of the input noise is more than $10^{-13}$, the input uncertainties become ideal coverage:
\begin{itemize}
...
\hline 
Signal & Sin/Cos   & Figure & Lower limit            & Reverse error deviation \\ 
         & functions &           & for ideal coverage  & without noise \\ 
\hline 
Sin/Cos & Indexed & \ref{fig:  FFT_SinCos_vs_Noise_Indexed} & $10^{-13}$ & $0.85$ \\ 
\hline 
Sin/Cos & Library & \ref{fig:  FFT_SinCos_vs_Noise_Lib} & $10^{-12}$ & $13$ \\ 
\hline 
Sin/Cos & Indexed & \ref{fig:  FFT_Linear_vs_Noise_Indexed} & $10^{-8}$ & $1.25$ \\ 
\hline 
Sin/Cos & Library & \ref{fig:  FFT_Linear_vs_Noise_Lib} & $10^{-6}$ & $2.7 \; 10^{3}$ \\ 
\hline 
\end{tabular}
\captionof{table}{
FFT Ideal Coverage Lower Limit.
}
\label{tab: FFT Ideal Coverage Lower Limit}
\end{table}

Table \ref{tab: FFT Ideal Coverage Lower Limit} shows that the lower limits for the ideal coverage increase with either using library sine and cosine instead of indexed sine and cosine, and with Linear signals instead of sin/cos signals.

Figure \ref{fig:  FFT_SinCos_1e-13_vs_Order}  shows the result error deviations and uncertainty means vs. FFT order using Sin/Cos signals with $10^{-13}$ noise, when the input uncertainties start to have idea coverage over the input errors.
\begin{itemize}
...
Figure \ref{fig:  FFT_SinCos_1e-13_Histo} shows that with $10^{-13}$ input noise, the result normalized errors for the forward and reverse transformations are normal distributed, while the result normalized errors for the roundtrip transformation is delta distributed, regardless of the types of input noises.
When the input noises are larger than the corresponding lower limits for ideal coverage in Table \ref{tab: FFT Ideal Coverage Lower Limit}, the  the result normalized errors for the forward and reverse transformations are ideally normal distributed.

For both signals using either sin/cos functions, FFT transformations are validated for ideal coverage.
