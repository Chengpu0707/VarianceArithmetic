...
For reversible transformations, a unique requirement is to recover every original input for both value and uncertainty after a \emph{round-trip} transformation that is performing a \emph{forward} transformation followed by its \emph{reverse} transformation.  
...
\captionof{figure}{
FFT spectrum of $\sin(3 \frac{2 \pi}{2^6} j)$ computed using the Quart sine function after the forward transformation.
...
\captionof{figure}{
FFT waveform of $\sin(3 \frac{2 \pi}{2^6} j)$ computed using the the Quart sine function after the reverse transformation. The legend distinguishes between the uncertainty and the value error.
...
Using the Quart sine function, for a sine wave with a frequency of $3$, Figure \ref{fig: FFT_Sin_Clean_6_3_Spec_Indexed} displays the spectrum for the forward transformation, while Figure \ref{fig: FFT_Sin_Clean_6_3_Wave_Indexed} presents the waveform for the reverse transformation.
\begin{itemize}
\item In the forward transformation, the value errors are slightly smaller than the corresponding result uncertainties, with an error deviation of $0.31$.
In the reverse transformation, the error deviation is $0.43$.
...
\begin{enumerate}
\item As the FFT order increases, the uncertainty mean of the forward transformation grows exponentially faster than that of the reverse  transformation.
At FFT order $18$, the uncertainty mean of the forward transformation is $10^3$ times greater than that of the reverse transformation.

\item At FFT order $18$, Figure \ref{fig: FFT_SinCos_Clean_Histo_Indexed} shows that the normalized error distribution for the reverse transformation is wider than that of the forward transformation, although the two distributions do not differ by an order-of-magnitude.
Therefore, the value error increases at least $10^2$ times faster in the forward transformation than in the reverse transformation.

\item In forward transformation, error deviations reach their stable values rapidly once the FFT order $L \geq 4$. 
In contrast, in the reverse transformation, error deviations stabilize more slowly with increasing FFT order, approaching stability only when $L \geq 15$.
...
\captionof{figure}{
FFT spectrum of $\sin(3 \frac{2 \pi}{2^6} j)$ computed using either the Library sine function or \textit{SciPy} after the forward transformation.
The legend distinguishes between the uncertainty and the value error.
The x-axis represents the frequency index, while the y-axis represents both the uncertainty and the value error.
}
...
\captionof{figure}{
FFT waveform of $\sin(3 \frac{2 \pi}{2^6} j)$ computed using either the Library sine function or \textit{SciPy} after the reverse transformation.
The legend distinguishes between the uncertainty and the value error.
The x-axis represents the time index, while the y-axis represents both the uncertainty and the value error.
}
...
Using the Library sine function, for a sine wave with a frequency of $3$, Figure \ref{fig: FFT_Sin_Clean_6_3_Spec_Lib} displays the spectrum for the forward transformation, while Figure \ref{fig: FFT_Sin_Clean_6_3_Wave_Lib} presents the waveform for the reverse transformation:
\begin{itemize}
...
\item
Due to numerical errors in the Library sine function, error deviations reach their stable values much quicker with FFT order in Figure \ref{fig: FFT_SinCos_Clean_vs_Order_Lib} that that in Figure \ref{fig: FFT_SinCos_Clean_vs_Order_Indexed}.
...
In the forward transformation, although there is no visible resonant pattern in FFT spectrum, the intensity of value errors increase with FFT order.
...
In contrast, this increase is completely absent when using the Quart sine function, as seen in Figure \ref{fig: FFT_Sin_Clean_6_3_Spec_Indexed} and \ref{fig: FFT_Sin_Clean_6_3_Wave_Indexed}.
Figure \ref{fig: Sin_Diff} indicates that the numerical errors using the Library sine function increase with a periodicity of $\pi$, which may resonate with a signal whose periodicity of an integer multiply of $\pi$, producing the resonant pattern in Figure \ref{fig: FFT_Sin_Clean_6_3_Spec_Lib} and \ref{fig: FFT_Sin_Clean_6_3_Wave_Lib}.
...
Because the input to the reverse transformation no longer consists predominantly of precise zeros, the output uncertainty increases much more rapidly with FFT order than its counterpart in Figure \ref{fig: FFT_SinCos_Clean_vs_Order_Indexed}.
This increase enables the proper coverage in Figure \ref{fig: FFT_Linear_vs_Order_Indexed}, resulting in error deviations similar to those Figure \ref{fig: FFT_SinCos_Clean_vs_Order_Indexed}.
The normalized error histogram of the reverse transformation in Figure \ref{fig: FFT_Linear_Clean_Histo_Lib} is narrower than that of Figure \ref{fig: FFT_SinCos_Clean_Histo_Lib}, confirming Figure \ref{fig: FFT_Linear_vs_Order_Indexed}.
...
The key question is whether variance arithmetic can effectively track these additional errors.
...
Adding sufficient noise to the input can suppress unspecified input errors and thereby achieve ideal coverage.
For example, applying an input noise of $10^{-3}$ to a Linear signal when using the Library sine function result in ideal coverage.

Figure \ref{fig:  FFT_Linear_1e-3_vs_Order_Lib} illustrates the error deviations and uncertainty means:
\begin{itemize}
\item As expected, the result uncertainty means for the forward transformations increase with FFT order $L$ as $\sqrt{2}^L$.

\item As expected, the result uncertainty means for the reverse transformations decrease with FFT order $L$ as $\sqrt{1/2}^L$.
...
\item As expected, the result error deviations for the forward and reverse transformations remain constant at $1$, whereas those for the roundtrip transformation decay exponentially toward $0$ as the FFT order increases.
...
Outside this region, proper coverage cannot be achieved.
...
The forward transformations exhibits a broader ideal coverage region than the reverse transformations.
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Linear_Clean_vs_Order_Prec.pdf} 
\captionof{figure}{
...
\label{fig: FFT_Linear_Clean_vs_Order_Prec}
\end{figure}
...
\end{tabular}
\captionof{table}{
The minimal required noise to achieve ideal coverage for FFT transformations at FFT order $18$ for different sine functions.
}
...
This raises the question of whether a sine function with a more accurate uncertainty can be developed.
...
Figure \ref{fig: FFT_Linear_Clean_vs_Order_Prec} and \ref{fig: FFT_Linear_Clean_Histo_Prec} demonstrate that the the Prec sine function slightly underestimates uncertainties, maintaining a stable error deviation around $1.8$ for both the forward and the reverse transformations without added noise.
Compared with Figure \ref{fig: FFT_Linear_Indexed_Reverse_ErrorDev_vs_Noise_Order}, Figure \ref{fig: FFT_Linear_Prec_Reverse_ErrorDev_vs_Noise_Order} shows that the ideal coverage achieved using the the Prec sine function is almost identical to that obtained with the Quart sine function. 
The results show that Prec sine function slightly underestimates value errors.
Table \ref{tbl: ideal coverage} compares using different sine functions for Sin/Cos or Linear signals.

The question is: Why the Prec sine function has not produced significant improvement over the Quart sine function?
Figure \ref{fig: Sin_Error_Quart_Prec} compares the value errors and the uncertainties of  $\sin(x)^2 + \cos(x)^2 - 1$ between the Prec sine function and the Quart sine function.
It shows that the Prec sine function produces slightly larger value errors, while having half uncertainties.
The value errors of the 128-bit sine function is $10^{-3}$ times less than that of the 64-bit sine function.
Therefore, the value errors for the precise sine function are due to the rounding errors of converting 128-bit floating-point numbers to 64-bit.
Unlike Figure \ref{fig: FFT_Linear_Clean_vs_Order_Prec} and \ref{fig: FFT_Linear_Prec_Reverse_ErrorDev_vs_Noise_Order} when the FFT order $L$ is larger than $3$, Figure \ref{fig: Sin_Error_Quart_Prec} does not show underestimation of value errors.
Except multiplication between two precise values, variance arithmetic does not track rounding errors during computation, which may have caused this difference.
...
\captionof{figure}{
The FFT execution time (y-axis) versus the FFT order (x-axis) for the implementation using either C++, Java, or Python (legend).
The C++ executable is complied with \textit{-O3} optimization.  
}
\label{fig: FFT_Performance}
\end{figure}


Except the performance, variance arithmetic shows no result difference when implemented using either C++, Java, or Python.
As expected, the execution time increases exponentially with the FFT order $L$ as $2^L$ when the FFT order is large enough, validating the calculation response.

It is surprising that the Java implementation is 3 times faster than the C++ implementation, even though the C++ implementation has \textit{-O3} optimization (the optimization already makes the C++ implementation run 3 times faster).
It shows that under repeated and heavy computation, the dynamic just-in-time optimization of Java is superior to the static optimization of C++. 
All three implementation are single-threaded without GPU-assistance.
One possibility is that because FFT contains independent and logically identical branches, the dynamic just-in-time compiler parallelize the execution to the 10 cores on the laptop computer.
If this is true, perhaps it is necessary to rethink multi-threading as a way to manually increase the performance of similar computations.
...
Computational result obtained with floating-point arithmetic are highly sensitive to the input data, the scale of computation, and the specific implementation of the mathematical library.
This implies that a small-scale test cannot reliably qualify or predict the behavior of large-scale calculations.
