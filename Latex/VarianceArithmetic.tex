...
Formula \eqref{eqn: exp precision}, \eqref{eqn: log precision}, \eqref{eqn: sin precision}, and \eqref{eqn: power precision} are tested by the corresponding math library functions \textit{exp}, \textit{log}, \textit{sin}, and \textit{pow}, respectively, for ideal coverage.
...
\item Each imprecise value carries its own $\sigma$ or $\epsilon$ to indicate the statistical significance of its value for its uncertainty.
...
For example, the acceptable leakage level of $\sigma=5$ dictates the minimal $x$ for a given $\delta x$ in $\log(x + \delta x)$ be $5 \delta x$, as the minimal non-divergent $x$ for each $\delta x$ in Figure \ref{fig: Log_Dev}.

\item The acceptable leakage level should be application specific.
\end{enumerate}
...
To avoid the unfaithful representation, the validation of the variance arithmetic using discrete Fourier transformation will not rely on the ideal result from continuous Fourier transformation.
...
Figure \ref{fig: FFT_SinCos_Clean_vs_Order} shows the result uncertainty means and error deviations for forward, reverse and roundtrip FFT transformations on Sin/Cos signals vs. FFT order $L$.
...
\begin{enumerate}
\item The variance arithmetic can not account for numerical errors in the library function, which will probably increase with the FFT order $L$.
It may be the reason why the error deviations of the reverse transformation increase slowly with $L$ as $0.0322 L$.
...
Figure \ref{fig:  FFT_Linear_1e-5_vs_Order}  shows the result error deviations and uncertainty means vs. FFT order using Lienar signals with $10^{-5}$ noises, when the input uncertainties are below the ideal coverage of $10^{-3}$.
...
If both the $\sin$ and the $\tan$ library functions were calculated with the variance arithmetic so that the result have ideal coverage, respectively, ideal coverage for the FFT transformations are expected, so that both figures can be greatly simplified as constant $1$ except when the FFT orders are too small to give a large enough sample size for the error deviations.
