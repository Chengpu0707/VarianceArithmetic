...
P(e^x)^2 & = \frac{\delta^2 e^x}{\overline{e^x}^2} \simeq \frac{\delta^2 e^x}{(e^x)^2}
     = \sum_{n=1}^{\infty} (\delta x)^{2n}  \left( \sum_{j=1}^{2n-1} \frac{\zeta(2n)}{j!\;(2n - j)!} 
 -\sum_{j=1}^{n-1} \frac{\zeta(2j)}{(2j)!} \frac{\zeta(2n - 2j)}{(2n - 2j)!} \right) \\
...
P(x^c)^2 &= \frac{\delta^2 x^c}{\overline{x^c}^2} \simeq \frac{\delta^2 x^c}{(x^c)^2} = \sum_{n=1}^{\infty} P(x)^{2n} \nonumber \\
...
\end{itemize}
Whether the input uncertainties accurately describe the corresponding errors or not will result in different result error characteristics.
...
They should usually be normal distributed or Gaussian distributed, showing the \emph{convergence power} according to the central limit theorem \cite{Probability_Statistics}.
...
For reversible transformations, a unique requirement for uncertainty-bearing arithmetic is to introduce the least amount of additional uncertainty after a \emph{round-trip} transformation which is a forward and reverse transformation, which provides an objective testing standard for a uncertainty-bearing arithmetic.  
...
\captionof{figure}{
The measured vs calculated uncertainty deviation for $\sin(x \pm \delta x)$, for different $\delta x$ as shown in the legend, in which "Gaussian" means using Gaussian noise, and "calculated" means the result from variance arithmetic. 
The $x$-axis is in radians divided by $\pi$, from $-1$ to $+1$.
The calculated values are obtained by Formula \eqref{eqn: sin precision}, while each measured deviation is obtained by 10000 samples using library \textit{sin} function.
%Each measured values fits well with the corresponding calculated value.
}
\label{fig: Sin_Dev}
\end{figure}
...
\captionof{figure}{
The measured vs calculated uncertainty deviation for $(1 \pm \delta x)^c$, for different $\delta x$ as shown in the legend, in which "white" means using white noise, "Gaussian" means using Gaussian noise, and "calculated" means the result from variance arithmetic. 
The $x$-axis is the exponent $c$ from $-2$ to $+3$.
The calculated values are obtained by Formula \eqref{eqn: power precision}, while each measured deviation is obtained by 10000 samples using library \textit{pow} function.
%Each measured values fits well with the corresponding calculated value except when $c < -1$ and $\delta x = 0.2$.
}
\label{fig: Pow_Dev}
\end{figure}
...
\captionof{figure}{
The measured probability density of $(1 \pm \delta x)^{c}$ for different \textit{exp=}$c$ and \textit{dev=}$\delta x$ as shown in the legend. 
The $x$-axis is the normalized deviation from the mean of $(1 \pm \delta x)^{c}$.
The $y$-axis is the measured histogram at each $x$.
}
...
\captionof{figure}{
The individual variance term of $(1 \pm \delta x)^{c}$ for the different Taylor expansion order $2n$, different \textit{exp=}$c$ and \textit{dev=}$\delta x$ as shown in the legend. 
...
It shows that $(1 \pm 0.2)^{-2}$ clearly diverges, $(1 \pm 0.2)^{-1}$ seems diverges, and other cases clearly converge.
}
...
\end{itemize}
Both Sin and Cos signals are faithful in Fourier transformation.
...
\begin{itemize}
\item The amount of calculation is $L$.
...
\item The forward and reverse transformation is identical except a sign.  

\item The forward transformation cancels each datum of a \emph{Sin} or \emph{Cos} signal into a spectrum of mostly $0$ values, so it is more susceptible to calculation errors, but the distributions of the calculation errors converge to normal distribution quicker.

\item The reverse transformation spread the spectrum of mostly $0$ values to each datum of a \emph{Sin} or \emph{Cos} signal, so it is less susceptible to calculation errors, but the distributions of the calculation errors converge to normal distribution slower.

\end{itemize}


\subsection{Evaluating Floating-Point Calculation Errors}

\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Sin_Uncertainty.png} 
\captionof{figure}{
The result uncertainty means and uncertainty deviations using Sin and Cos signals vs. FFT order for forward, reverse and roundtrip FFT transformations, as shown in the legend.
}
\label{fig: FFT_Sin_Uncertainty}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Sin_Error.png} 
\captionof{figure}{
The result error maximums and error deviations using Sin and Cos signals vs. FFT order for forward, reverse and roundtrip FFT transformations, as shown in the legend.
}
\label{fig: FFT_Sin_Error}
\end{figure}
...
\captionof{figure}{
The real and imaginary absolute value errors and uncertainties in the FFT spectrum of a clean Sin signal of order 7 and index frequency 15, as shown in the legend.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Sin_Order7_Freq15_Spec_Noisy.png} 
\captionof{figure}{
The real and imaginary absolute value errors and uncertainties in the FFT spectrum of a Sin signal of order 7 and index frequency 15 with $10^{-15}$ white noises, as shown in the legend.
}
\label{fig: FFT_Sin_Order7_Freq15_Spec_Noisy}
\end{figure}
...
\captionof{figure}{
The histograms of the normalized errors of Sin and Cos signals at FFT with $10^{-15}$ white noises for forward, reverse and roundtrip FFT transformations, as shown in the legend.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Linear_Error.png} 
\captionof{figure}{
The mean and deviation of the result uncertainties of Linear signals vs. FFT order for forward, reverse and roundtrip FFT transformations, as shown in the legend.
}
\label{fig: FFT_Linear_Error}
\end{figure}
...
\item For the reverse transformations, the uncertainty means remain a constant about $1.67 \times 10^{-16}$ with the FFT order $L$.

\item For the roundtrip transformations, the uncertainty means remain a constant about $1.38 \times 10^{-16}$ with the FFT order $L$.
\end{itemize}
The input uncertainties are the least significant values of $\sin(\pi j /2^L), j = 0, 1, 2 ... 2^{L-1}$.
This input uncertainties transfer to the result uncertainty deviations differently for different transformations in Figure \ref{fig: FFT_Sin_Uncertainty}.
\begin{itemize}
\item For the forward transformation, the result uncertainty deviations are comparable to the corresponding uncertainty mean.

\item For the reverse and roundtrip transformations, the result uncertainty deviations are much to the corresponding uncertainty mean.
\end{itemize}

Even the result uncertainties grow quite differently for the different transformations in Figure \ref{fig: FFT_Sin_Uncertainty}, the variance arithmetic is capable of both tracking and bounding the result errors, as demonstrated by Figure \ref{fig: FFT_Sin_Error}:
\begin{itemize}
...
\item Figure \ref{fig: FFT_Sin_Order7_Freq15_Spec_Clean} shows how the variance arithmetic uses uncertainties to track the value errors in a FFT spectrum on average.
The two peaks of the value errors at the index frequencies $\pm 15$ are expected, but Figure \ref{fig: FFT_Sin_Order7_Freq15_Spec_Clean} also shows a few other peaks such as at $\pm 17$, $\pm 45$, and $\pm 47$, suggesting systematic errors from the library function.

\item To overcome such systematic errors, $10^{-15}$ white noises are added to the Sin and Cos signals.
...
Figure \ref{fig: FFT_Sin_Error_Histogram} confirms that the normalized errors for the forward transformation are normal distributed with the noises.

\item In Figure \ref{fig: FFT_Sin_Error_Histogram}, the normalized errors for the roundtrip transformation are still not normal distributed, and the corresponding error deviations still increase with the FFT order $L$ as $0.0262 L$.
When $10^{-12}$ white noises are added to the Sin and Cos signals, for the roundtrip transformation, the normalized errors become a constant of $1$, and the normalized errors become normal distributed.

\item In the above steps, the noises are only added to the Sin and Cos signals.
If $10^{-15}$ white noises are added to the library function, the result error deviations remain near $1$ for the roundtrip transformation.
Thus, the library function $\sin(x), x \in [0, \pi/2]$ is estimated to contains $10^{-15}$ numerical errors.
\end{enumerate}
...
Linear signals contain the library function $\tan(\pi j /2^L), j = 0, 1, ... 2^{L-1} - 1$ for the FFT order $L$ in Formula \eqref{eqn: Fourier spec for linear}.
...
For the order $18$ roundtrip transformations, $10^{-3}$ input noises are necessary for the result error deviation to be 1 and the result value errors to be normal distributed, which is proportional to the $10^{-12}$ input noise necessary for the Sin and Cos signals.

For pure floating-point FFT calculations, both the tracking power and the bounding power of the variance arithmetic are validated.



\subsection{Evaluating Error Response}

\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Sin_Uncertainty_vs_Uncertainty.png} 
\captionof{figure}{
The result uncertainty means using Sin and Cos signals of order 18 vs. input uncertainties for forward, reverse and roundtrip FFT transformations using either Gaussian noise or white noise, as shown in the legend.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Linear_Uncertainty_vs_Uncertainty.png} 
\captionof{figure}{
The result uncertainty means using Linear signals of order 18 vs. input uncertainties for forward, reverse and roundtrip FFT transformations using either Gaussian noise or white noise, as shown in the legend.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Sin_ErrorDev_vs_Uncertainty.png} 
\captionof{figure}{
The result error deviations using Sin and Cos signals of order 18 vs. input uncertainties for forward, reverse and roundtrip FFT transformations using either Gaussian noise or white noise, as shown in the legend.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Linear_ErrorDev_vs_Uncertainty.png} 
\captionof{figure}{
The result error deviations using Linear signals of order 18 vs. input uncertainties for forward, reverse and roundtrip FFT transformations using either Gaussian noise or white noise, as shown in the legend.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Sin_ErrorMax_vs_Uncertainty.png} 
\captionof{figure}{
The result error deviations using Sin and Cos signals of order 18 vs. input uncertainties for forward, reverse and roundtrip FFT transformations using either Gaussian noise or white noise, as shown in the legend.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Linear_ErrorMax_vs_Uncertainty.png} 
\captionof{figure}{
The result error deviations using Linear signals of order 18 vs. input uncertainties for forward, reverse and roundtrip FFT transformations using either Gaussian noise or white noise, as shown in the legend.
}
...
\item As expected, the result uncertainty means for the forward transformation is order-of-magnitude larger than the corresponding uncertainty means for the reverse transformation.

\item The result uncertainty means for the roundtrip transformation is order-of-magnitude smaller than the corresponding uncertainty means for both the forward transformation and the reverse transformations, showing that the variance arithmetic can largely recover the original uncertainties.
However, the result uncertainty means are not exactly 0, probably due to the rounding errors of the conventional floating-point calculations. 

\item All the result uncertainty means increases linearly with the corresponding input uncertainties, except for the roundtrip transformations when the input uncertainties are below $10^{-13}$.
This is expected because all FFT transformations are linear.

\item When an added noise is less than the rounding error and the numerical error, it becomes ineffective so that the result uncertainty means will no longer decrease with the lower input noise uncertainties.
For Sin and Cos signals, the maximal least significant values are around $10^{-16}$, which is the lower linear limits for both the forward and the reverse transformations.
However, for the roundtrip transformation, this lower limit is around $10^{-13}$.
This is understandable because the roundtrip transformation is the forward transformation followed by the reverse transformation, so that the effect of the least significant values should be amplified.
...
Because $\tan(\pi j /2^L), j = 0, 1, ... 2^{L-1} - 1$ have much larger least significant values than $\sin(\pi j /2^L), j = 0, 1, ... 2^{L-1}$, the lower linear limits for the Linear signals are much larger than the corresponding lower linear limits for the Sin and Cos signals: $10^{-10}$ for the forward and reverse transformations, and $10^{-8}$ for the roundtrip transformation.

Figure \ref{fig: FFT_Sin_ErrorDev_vs_Uncertainty} and \ref{fig: FFT_Linear_ErrorDev_vs_Uncertainty} show the result error deviations vs. different input uncertainties using either Sin and Cos or Linear signals of order 18.
...
\begin{itemize}
\item For Sin and Cos signals, the lower limit is $10^{-15}$ for the forward and reverse transformations, and $10^{-12}$ for the roundtrip transformation.
The $10^{-15}$ agree with the previously estimated numerical errors for $\sin(\pi j /2^L), j = 0, 1, ... 2^{L-1}$.

\item For Linear signals, the lower limit is $10^{-10}$ for the forward and reverse transformations, and $10^{-3}$ for the roundtrip transformation.
The $10^{-10}$ agree with the previously estimated numerical errors for $\tan(\pi j /2^L), j = 0, 1, ... 2^{L-1} - 1$.

\item The roundtrip transformation contains the combined amount calculation of the forward and the reverse transformations, so that it has higher lower limits, $10^{-12}$ for Sin and Cos signals, and $10^{-2}$ for Linear signals.

\item The transition of error deviations to 1 in Figure \ref{fig: FFT_Sin_ErrorDev_vs_Uncertainty} and \ref{fig: FFT_Linear_ErrorDev_vs_Uncertainty} is much sharper than the transition of uncertainty means to the linear regions in Figure \ref{fig: FFT_Sin_Uncertainty_vs_Uncertainty} and \ref{fig: FFT_Linear_Uncertainty_vs_Uncertainty}.
\end{itemize}
Adding noises then checking error deviations to be 1 seems a valid way to evaluate numerical errors in library functions.

Figure \ref{fig: FFT_Sin_ErrorMax_vs_Uncertainty} and \ref{fig: FFT_Linear_ErrorMax_vs_Uncertainty} show the result normalized error maximums vs. different input uncertainties using either Sin and Cos or Linear signals of order 18.
Except when the input uncertainties are accurate for the corresponding input errors for the reverse transformations, the error maximums are  $3.1$.

Figure \ref{fig: FFT_Sin_Error_Histogram} shows that when the input noises is white, the result normalized errors are largely uniformly distributed.
When the input uncertainties are accurate for the corresponding input errors, the error maximums in Figure \ref{fig: FFT_Sin_ErrorMax_vs_Uncertainty} and \ref{fig: FFT_Linear_ErrorMax_vs_Uncertainty} are $\sqrt{3}$, also suggesting that the result normalized errors are largely uniformly distributed.
The loss of the convergence power of the reverse transformations needs further investigations.

For FFT calculations with different uncertainties, both the tracking power and the bounding power of the variance arithmetic are validated, and the error response is found to be linear when the input uncertainties are accurate for the corresponding input errors.
