...
The input uncertainties run from $10^{-16}$ to $10^{-1}$, while the FFT Order runs from $4$ to $18$.
A Slope signal is constructed by replacing $\tan(n \pi /N)$ in Formula \eqref{eqn: Fourier spec for linear} with $\frac{\sin(n \frac{2\pi}{2N})}{\sin(n \frac{2\pi}{2N} + \frac{\pi}{2})}$ for the corresponding Linear signal.
}
...
One way to reduce the numerical errors in Linear signals is to replace $\tan(n \pi /N)$ in Formula \eqref{eqn: Fourier spec for linear} with $\frac{\sin(n \frac{2\pi}{2N})}{\sin(n \frac{2\pi}{2N} + \frac{\pi}{2})}$, so that only $\sin$ library functions are used.
...
The effect of such seemly idiotic mathematical replacement could be quite large and delicate, so that it seems hopeless to try to improve numerical results using this kind of maneuvers.
...
\clearpage
\section{Sine Library Function with Uncertainty}
\label{sec: recursion}
