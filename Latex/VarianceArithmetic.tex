...
\hline 
Sin/Cos   & Quart           & $10^{-15}$  & $10^{-12}$ & $10^{-14}$ \\ 
\hline 
Sin/Cos   & Library         & $10^{-15}$  & $10^{-11}$ & $10^{-12}$ \\ 
\hline 
...
\subsection{Input Uncertainty Recovery by Roundtrip Transformations}
...
Without added noises, the value error waveform for the roundtrip transformation is qualitatively similar to that of the the reverse transformation, with error deviations close to $1$, such as in Figure \ref{fig: FFT_SinCos_Clean_vs_Order_Indexed}, \ref{fig: FFT_SinCos_Clean_vs_Order_Cpp}, \ref{fig: FFT_SinCos_Clean_vs_Order_Lib}, and \ref{fig: FFT_Linear_vs_Order_Indexed}, even when proper coverage can no longer achieve for both forward and reverse transformations in Figure \ref{fig: FFT_Linear_vs_Order_Lib}.
Without added noises, roundtrip transformation can always recover the input uncertainty statistically.

With added noises, using Lib sine function:
\begin{itemize}
\item Figure \ref{fig:  FFT_Linear_1e-3_vs_Order_Lib} shows that the error deviations for the roundtrip transformation is much less than $1$, suggesting that each input uncertainty is recovered numerically.
...
\end{itemize} 
The larger ratio of specific input uncertainty to unspecified input uncertainty, the better input uncertainty recovering numerically is provided by variance arithmetic.
...
In another word, in this case, higher bit floating-point arithmetic is not necessarily superior.
...
Without such optimization, the C++ implementation is about a order of magnitude slower than the Java implementation.
}
