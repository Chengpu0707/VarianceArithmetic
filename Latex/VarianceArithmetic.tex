...
Using Formula \eqref{eqn: Gaussian function distribution}, Formula \eqref{eqn: Taylor 1d bias} and \eqref{eqn: Taylor 1d variance} gives the mean $\overline{f(x)}$ and the variance $\delta^2 f(x)$, respectively, in which $M(x, n)$ is the \emph{variance momentum} defined by Formula \eqref{eqn: variance momentum}, and $\sigma$ is defined as the \emph{bounding factor}.
\begin{align}
\label{eqn: Taylor 1d} 
...
\end{align}
In Formula \eqref{eqn: Taylor 1d bias}, the mean $\overline{f(x)}$ is decomposed as the sum of:
\begin{itemize}
...
If $x \pm \delta x$ already contains variance bias, Formula \eqref{eqn: Taylor 1d bias} becomes  Formula \eqref{eqn: accum calc bias}.
...
\zeta(2n) &\equiv \int_{-\sigma}^{+\sigma} \tilde{z}^n N(\tilde{z}) d \tilde{z}
...
\begin{align*}
&\widehat{(f(x)+g(x))} = \sum_{n=1}^{\infty} M(x, 2n) \frac{f^{(2n)}_x + g^{(2n)}_x}{n!} = \widehat{f(x)} + \widehat{g(x)}; \\
&\delta^2 (f(x)+g(x)) = \sum_{n=1}^{\infty} M(x, 2n) \sum_{j=1}^{2n-1} \frac{f^{(j)}_x + 
  g^{(j)}_x}{j!} \frac{f^{(2n-j)}_x + g^{(2n-j)}_x}{(2n-j)!} - \widehat{f(x) + g(x)}^2 \\
&= \delta^2 f(x) + \delta^2 g(x) + 2f^{(1)}_x g^{(1)}_x (\delta^2 x) + 
  2 \sum_{n=2}^{\infty} M(x,2n) \sum_{j=1}^{2n-1} \frac{f^{(2n-j)}_x }{(2n-j)!} \frac{g^{(j)}_x}{j!};
\end{align*}
...
When the inputs obeys the uncorrelated uncertainty assumption, in the intermediate steps, variance arithmetic uses statistics to account for correlation between uncertainties, as far as the same imprecise value always corresponds to the same variable when applying Formula \eqref{eqn: Taylor 1d bias}, \eqref{eqn: Taylor 1d variance}, \eqref{eqn: Taylor 2d bias}, and \eqref{eqn: Taylor 2d variance}.
