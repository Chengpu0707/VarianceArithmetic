...
For floating-point rounding errors, it predicts the result uncertainty variance numerically within $10$-fold of the actual result uncertainty variance.
...
It rejects certain calculations such as inverting an input near zero on the mathematical ground of the divergence of the result uncertainty variance, which is also the statistical ground that either a distributional pole or a distributional zero is in the input bounding ranges statistically.
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Independent_Uncertainty_Assumption.pdf} 
\captionof{figure}{
...
Theoretically, the pure rounding error due to round-off is generally shown to be uniformly distributed between the half bit of the least significant bit of the siginficand \footnote{Table \ref{tbl: rounding pattern} describes in detail on rounding error distribution.} \cite{Prev_Precision_Arithmetic}, whose probability density function is shown in Formula \eqref{eqn: rounding error 1/2 distribution}:
\begin{equation}
...
Formula \eqref{eqn: central limit theorem} gives the probability density function for $x \pm \delta x$ in general, in which $N(x)$ is the density function of the  Normal distribution \cite{Probability_Statistics}.
...
In statistics, such two imprecise values have $50\%$ possibility to be less than or greater to each other but no chance to be equal to each other \cite{Probability_Statistics}.
...
Each probability density function $\rho(\tilde{x})$ in solid line is compared with a Normal distribution $N(\tilde{x})$ of the same mode in the dash line of the same color.
The legend for the Normal distribution shows the value of Normal difference in percentage.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Cubic_Root_Distribution.pdf} 
\captionof{figure}{
...
Some probability density function $\rho(\tilde{x})$ in solid line is compared with a Normal distribution $N(\tilde{x})$ of the same mode in the dash line of the same color.
The legend for the Normal distribution shows the value of Normal difference in percentage.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Inverse_Square_Distribution.pdf} 
\captionof{figure}{
...
The probability density function $\rho(\tilde{x})$ in solid line is compared with a Normal distribution $N(\tilde{x})$ of the same mode in the dash line of the same color.
The legend for the Normal distribution shows the value of Normal difference in percentage.
}
...
Figure \ref{fig: Square_Distribution} shows the probability density function for $(x \pm 1)^2$ according to Formula \eqref{eqn: power distribution}, which has a distributional zero at $x=0$ that separates two defined region $x \ge 0$ and $x \leq 0$.
...
\delta^2 f(x)|_{x =g(x),\; (\delta x)^2 = \delta^2 g(x)} &\simeq (\delta x)^2 (f^{(1)}_x g^{(1)}_x)^2 \\
  &\eqspace + (\delta x)^4 \left( (f^{(1)}_x)^2 (g^{(1)}_x g^{(3)}_x + \frac{1}{2}(g^{(2)}_x)^2) + \frac{1}{2} (f^{(2)}_x)^2 (g^{(1)}_x)^4 \right) \nonumber \\
  &\eqspace + (\delta x)^4 \left( f^{(1)}_x f^{(3)}_x (g^{(1)}_x)^4 \right) + o((\delta x)^6);  \nonumber \\
\label{eqn: square of square root}
& \delta^2 y^2|_{y = \sqrt{x},\; (\delta y)^2 = \delta^2 \sqrt{x}} \simeq (\delta x)^2 + \frac{9}{2} (\delta x)^4; \\
\label{eqn: square root of square}
& \delta^2 \sqrt{y}|_{y = x^2,\; (\delta y)^2 = \delta^2 x^2} \simeq (\delta x)^2 + \frac{9}{8} (\delta x)^4;
\end{align}
...
But it upsets dependency tracing through variable usage.
...
\item 
Traditionally, an analytic expression can be broken down into simpler and independent arithmetic operations of negation, addition, multiplication, and division \footnote{Sometimes square root is also regarded as an arithmetic operation.}.
But this is no longer true for uncertainty-bearing arithmetic.
...
In variance arithmetic, Taylor expansion unifies different analytic expressions of the same function, to give the same result \footnote{Until now, Taylor expansion cannot be used as a general method in interval arithmetic.}.
...
$\delta^2 f(g(x))$ is calculated as Formula \eqref{eqn: composite variance 1d}, which is different from Formula \eqref{eqn: progressive variance} for $\delta^2 f(x)$ with $x = g(x)$ and $\delta^2 x = \delta^2 g(x)$.
...
\item The central limit theorem converges the result error distribution toward Normal distribution, unless at a distribution zero, or the expected result is zero.
When the expected result is zero, the result error distribution is Delta distribution.
...
When an input contains unspecified errors, such as floating-point rounding errors, or numerical errors of library functions, Gaussian or white noises of increasing deviations can be added to the input, until ideal coverage is reached.
...
Empirically, variance arithmetic can provide proper coverage to rounding errors, with the error deviations in the range of $[0.5, 2]$.

Section \ref{sec: polynomial} shows that variance arithmetic no longer needs theoretical reminder estimator.
Instead, it uses stability truncation, to guarantee that the truncation error is statistically small enough for the result uncertainty.
...
Through Taylor expansion, variance arithmetic is directly applicable to application problems.
...
Discrete Fourier Transformation is a typical reversible transforming algorithm, which contains the same amount of input and output data, and its output data can be transformed back to the input data using essentially the same algorithm.  
...
In practice, without reminder estimator, the expansion is stopped if the change of the expansion is less than an ad hoc threshold \cite{Numerical_Recipes}.
Such truncation by stability is generally not reliable because traditional calculation has no idea on what the result uncertainty should be.
...
For $x = -0.7$, $r(N)$ again does not reach $LSV$, leaving the least two significant bits incorrect, which shows that traditional numerical Taylor expansion can be less accurate than its theoretical claim.
...
Variance arithmetic tracks the uncertainty of individual value statistically, and it cannot precisely track coherent accumulation of rounding errors as what Table \ref{tbl: rounding pattern} has revealed. 
...
\begin{figure}[p]
\centering
\includegraphics[height=2.4in]{Taylor_Expansion_0.04.pdf} 
\captionof{figure}{
The reminder values and the expansion uncertainties for Taylor expansion of $\frac{1}{1 -x  \pm \delta x}$, for different Taylor expansion order $N$ as shown in the x-axis, and different input $x  \pm \delta x$ as shown in the legend.
In the legend,  
\textit{Reminder Value} is the value of Formula \eqref{eqn: Taylor expansion reminder}, 
...
\begin{figure}[p]
\centering
\includegraphics[height=2.4in]{Taylor_Expansion_Order.pdf} 
\captionof{figure}{
...
For example, the expansion of $1/(1 + 0.7 \pm 0.04)$ now terminates at Taylor expansion order $105$ instead of $70$ in Figure \ref{fig: Taylor_Expansion_0.01}.
...
Figure \ref{fig: Taylor_Expansion_Error} shows that truncation errors are linearly proportional to input uncertainties, which serves as another validation.
...
The application of Taylor expansion for uncertainty and uncertainty bias is more than polynomial expansion as described in Formula \eqref{eqn: polynomial Taylor}.
Besides stability truncation and practical stability check, it also has convergence check and reliability check, as described in Section \ref{sec: Convergence}.
In Figure \ref{fig: Taylor_Expansion_Order} and Figure \ref{fig: Taylor_Expansion_Error}, $1/(1 - 0.7 \pm \delta x)$ terminates at $\delta x \geq 0.06$ not due to practically unstable, but due to the divergence of $1/(1 - 0.7 \pm \delta x)$ when $\delta x \geq 0.06$.
Even after $1/(1 - 0.7 \pm \delta x)$ diverges, Formula \eqref{eqn: polynomial Taylor} still give numerical results because it does not have convergence check, such as $1/(1 - 0.7 \pm 0.09) = 95.9 \pm 94696$, which would have given runaway errors in Figure \ref{fig: Taylor_Expansion_Error}.
This example shows that the context of an calculation is as import as the result of the calculation.
The uncertainty is the most important context of any value, and that is the very reason why variance arithmetic is superior than traditional floating-point calculations for values only.
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Adjugate_Histogram_Size_7.pdf} 
\captionof{figure}{
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Adjugate_Histogram_Size_8.pdf} 
\captionof{figure}{
...
Figure \ref{fig: Matrix_Determinant_Prec_vs_Condition} shows that there is a strong linear correlation between the conditional number and the determinant precision of matrices, so that the determinant precision can replace the condition number, which is more difficult to calculate \footnote{There are currently different definitions of matrix condition number, each based on a different definition of matrix norm.}.
...
\begin{figure}
\centering
\includegraphics[height=2in]{Moving_Linear_Fit_Coeff.pdf} 
\captionof{figure}{ 
...
The y-axis on the left is for the value of $Y$, $\alpha$, and $\beta$, while the y-axis on the right is for the uncertainty of $\alpha$ and $\beta$.
...
\captionof{figure}{
The largest measured $\delta x$ for  $\ln{x \pm \delta x}$ for different $x$ as shown by the x-axis, using the y-axis on the left.
Also shown are the corresponding uncertainty bias and uncertainty for different $x$ as shown by the x-axis, using the y-axis on the right.
}
\label{fig: Log_Conv_Edge}
\end{figure}

Figure \ref{fig: Log_Conv_Edge} shows that Formula \eqref{eqn: log precision} for $\ln(x)$ converges when $P(x) < 1/\sigma$, which is also the theoretical upper bound for $P(x)$  because $2n$ can be regarded as a constant when compared with $\zeta(2n)$.
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Power_Conv_Edge.pdf} 
\captionof{figure}{
The largest measured $\delta x$ for  $\ln(x \pm \delta x)$ for different $x$ as shown by the x-axis, using the y-axis on the left.
Also shown are the corresponding uncertainty bias and uncertainty for different $x$ as shown by the x-axis, using the y-axis on the right.
}
\label{fig: Power_Conv_Edge}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Power_Conv_Expansion.pdf} 
\captionof{figure}{
The individual variance term of $1/(1 \pm \delta x)$ vs the different Taylor expansion order $2n$, for different $\delta x$ as shown in the legend. 
The $x$-axis is Taylor expansion order $2n$ Formula \eqref{eqn: Taylor 1d variance}.
The $y$-axis is the value and variance contribution to $1/(1 \pm \delta x)$ at each Taylor expansion order $2n$ according to Formula \eqref{eqn: power mean}, and \eqref{eqn: power precision}, respectively.
}
\label{fig: Power_Conv_Expansion}
\end{figure}

If $c$ is a natural number, Formula \eqref{eqn: power precision} always converges because it only has limited terms, such as Formula \eqref{eqn: square precision} for $c = 2$. 
Even though the normalized value errors for $(0 + \delta x)^2$ is $\chi^2$ distributed, its error deviation is $1$.
This shows that the variance arithmetic allows normalized error distributions to deviate from Normal to a large degree, as what Formula \eqref{eqn: variance momentum UN} has suggested.
...
\begin{itemize}
\item 
When $c < 1$, according to Formula \eqref{eqn: square root precision}, \eqref{eqn: inversion precision}, and \eqref{eqn: inversion square precision}, the coefficient for $P(x)^{2n}$ increases quickly with both $c$ and $2n$, so that the upper bound for $\delta x$ has to decrease with $c$, which is confirmed by Figure \ref{fig: Power_Conv_Edge}.
...
Both formulas have the alternative $+$ and $-$ sign pattern when the Taylor expansion order $n > c + 1$, so the pattern is different by $1$ between $\delta^2 \sqrt{x^3}$ and $\delta^2 \sqrt{x^5}$.
This difference may cancel some terms in higher order expansion, such as the $P(x)^6$ expansion in Formula \eqref{eqn: square root^5 precision} for $\delta^2 \sqrt{x^5}$.
...
\item 
When $c < 0$, Figure \ref{fig: Power_Conv_Edge} shows that the uncertainty grows exponentially with decreasing $c$, so that the result precision becomes worse quickly.
...
Due to the limitation of maximal expansion order of $2n=252$, such determination of convergence is only practical, which may be different from theoretical convergence threshold. 
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Sin_Histo.pdf} 
\captionof{figure}{
The histogram of the error deviation for $\sin(x \pm \delta x)$, for $x=0, +\pi/2, +\pi/4$ as shown in the legend.
The x-axis is for normalized value error. 
...
Gaussian noises are used to produce the input noise $\delta x = 0.001$.
}
\label{fig: Sin_Histo}
\end{figure}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Sin_Conv_Edge_Histo_Quarter.pdf} 
\captionof{figure}{
The histogram for $\sin(\pi/4 \pm \delta x)$ for different $\delta x$ as shown in the legend.
The x-axis is for normalized value error.
The y-axis is for normalized count.
}
\label{fig: Sin_Conv_Edge_Histo_Quarter}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Sin_Conv_Edge_Histo_Middle.pdf} 
\captionof{figure}{
The histogram for $\sin(0 \pm \delta x)$ for different $\delta x$ as shown in the legend.
The x-axis is for normalized value error.
The y-axis is for normalized count.
}
\label{fig: Sin_Conv_Edge_Histo_Middle}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Sin_Conv_Edge_Histo_Edge.pdf} 
\captionof{figure}{
The histogram for $\sin(\pi/2 \pm \delta x)$ for different $\delta x$ as shown in the legend.
The x-axis is for normalized value error.
The y-axis is for normalized count.
}
\label{fig: Sin_Conv_Edge_Histo_Edge}
\end{figure}
...
\captionof{figure}{
The largest measured $\delta x$ for  $\sin(x \pm \delta x)$ for different $x$ as shown by the x-axis, using the y-axis on the left.
Also shown are the corresponding uncertainty bias and uncertainty for different $x$ as shown by the x-axis, using the y-axis on the right.
}
\label{fig: Sin_Conv_Edge}
\end{figure}

Because $2n!$ is much larger than $2n!!$, Formula \eqref{eqn: sin precision} for $sin(x)$ always converges.

$\sin(x)$ has distributional zeros at $x=n\pi +\pi/2$, in which $n$ is an integer.
Figure \ref{fig: Sin_Histo} shows the histogram of normalized errors for $\sin(x \pm 0.001)$ when Gaussian noises are added.
...
In Figure \ref{fig: Sin_Conv_Edge_Histo_Quarter}, the noise $\delta x$ is increased from $0.001$:
\begin{itemize}
\item When $\delta x = 0.001$, the distribution is Normal.

\item When $\delta x=0.2$, the distribution at $x = \pi/4$ starts to be affected by the closest distributional zero at $x = \pi/2$, so that the distribution starts to deviate from Normal distribution.

\item When $\delta x = 0.5$, the distribution at $x = \pi/4$ is affected strongly by the closest distributional zero at $x = \pi/2$, so that the distribution starts to resemble of the distribution at $x = \pi/2$ in Figure \ref{fig: Sin_Histo}. 

\item When $\delta x = 1.024$, the distribution at $x = \pi/4$ starts to be affected by the second closest distributional zero at $x = -\pi/2$, which result in a small peak resembling the distribution at $x = -\pi/2$ in Figure \ref{fig: Sin_Histo}.

\item When $\delta x = 1.025$, the result is invalidated as not monotonic, which is a pleasant side effect of the convergence criteria of variance arithmetic.
In a periodic function $\sin(x + \delta x)$, $\delta x$ conceptually should not be larger than period $2\pi$.
Variance arithmetic just provides a sensible upper bound for $\delta x$ in this case.

\end{itemize}

In some sense, a distribution is like sticky fluid, to be bounced by two enclosing distributional zero walls \footnote{The relation between a distribution and its enclosing distributional zeros resemble the relation between quantum waves and quantum walls. 
} of $\sin(x)$.
Figure \ref{fig: Sin_Conv_Edge_Histo_Quarter} shows the distributions until reaching the maximal holding capacity of the enclosing distributional zeros for $\sin(\pi/4 + \delta x)$.
Figure \ref{fig: Sin_Conv_Edge_Histo_Middle} shows how the distribution lava flows fill evenly between two distributional zero walls for $\sin(0 + \delta x)$ with increasing $\delta x$. 
Figure \ref{fig: Sin_Conv_Edge_Histo_Edge} shows how a distributional zero wall itself is broken with increasing $\delta x$ by its enclosing distributional zero wall. 
For an analytic function with multiple distributional zeros, the monotonic criterion naturally limits the input uncertainty to the distance between two enclosing distributional zeros.
...
The mechanism for the upper bounding of $\delta x$ in $\sin(x + \delta x)$ needs further understanding to quantify the result shown in Figure \ref{fig: Sin_Conv_Edge}.
...
\begin{figure}[p]
\centering
\includegraphics[height=2in]{Exp_Dev.pdf} 
\captionof{figure}{
...
\captionof{figure}{
The calculated uncertainties vs the measured value deviations as well as the measured error deviations for $\ln(x \pm \delta x)$, for different $x$ as shown by the x-axis, and different $\delta x$ as shown in the legend.
The uncertainties and the value deviations are drawn using the logarithmic y scales on the left side, while the error deviations are drawn using the linear y scales on the right side. 
Each color represents a $\delta x$.
Gaussian noises are used to produce the input noise $\delta x$.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.4in]{Sin_Dev_Uniform.pdf} 
\captionof{figure}{
The calculated uncertainties vs the measured value deviations as well as the measured error deviations for $\sin(x \pm \delta x)$, for different $x$ as shown by the x-axis, and different $\delta x$ as shown in the legend.
The uncertainties and the value deviations are drawn using the logarithmic y scales on the left side, while the error deviations are drawn using the linear y scales on the right side. 
Each color represents a $\delta x$.
...
\captionof{figure}{
The values and uncertainties of $\ln(e^x) - x$ vs $x$, as \textit{VarDbl Error} and \textit{VarDbl Uncertainty} in the legend.
...
$\sin(x)$ has distributional zeros at $x=\pm \pi/2$.
...
The combined numerical error of the library function $e^x$ and $\ln(x)$ is calculated as $\ln(e^x) - x$.
...
The numerical error of the library function $x^p$ is calculated as $(x^p)^{1/p} - x$.
...
\begin{figure}[p]
\includegraphics[height=2.5in]{Sin_Error.pdf} 
\captionof{figure}{
...
\begin{figure}
\includegraphics[height=2.5in]{Sin_Cot_Diff.pdf} 
\captionof{figure}{
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_SinCos_1e-16_vs_Order_Lib.pdf} 
\captionof{figure}{
The result error deviation and uncertainty mean of Sin/Cos signals vs. FFT order using the library sine functions for forward, reverse and roundtrip FFT transformations, as shown in the legend.
The y-axis on the left is for error deviation, while the y-axis on the right is for uncertainty mean.
...
\begin{figure}[p]
\includegraphics[height=2.5in]{FFT_Sin_Clean_6_4_Wave_Lib.pdf} 
\captionof{figure}{
...
\begin{figure}[p]
\includegraphics[height=2.5in]{FFT_Sin_Clean_6_1_Wave_Lib.pdf} 
\captionof{figure}{
...
\captionof{figure}{
The result error deviation and uncertainty mean of Linear signal vs. FFT order using the indexed sine functions for forward, reverse and roundtrip FFT transformations, as shown in the legend.
The y-axis on the left is for error deviation, while the y-axis on the right is for uncertainty mean.
}
...
In variance arithmetic, ill-formed problems can be invalidated as result divergence, unreliable result variance, or being practically unstable.
...
\subsection{More Studies are Needed}

This paper just showcases variance arithmetic which is still in its infancy.
Theoretically, some important questions remain:
\begin{itemize}
\item Why can variance arithmetic only give proper coverage for the floating-point rounding errors?
Table \ref{tbl: rounding pattern} shows a possibly mechanism for rounding errors.
When the mechanism for rounding errors is clear, can a separated implementation be optimized for covering floating-point rounding errors?

\item What is the uncertainty upper bounds for each problem? 
Specifically, how to explain quantitatively the measured upper bounds in Figure \ref{fig: Power_Conv_Edge} and \ref{fig: Sin_Conv_Edge}.

\item How to apply variance arithmetic when analytic solution is not available such as solving differential equation?
\end{itemize}
...
Variance arithmetic no longer has dependency problem, and it can reject ill-formed problems.
...
The calculation of the result uncertainty bias and uncertainty contains a lot of sums of constant terms such as Formula \eqref{eqn: Taylor 1d variance} and \eqref{eqn: Taylor 2d variance}, so that it is a excellent candidate for parallel processing.
Because variance arithmetic has neither execution freedoms nor the associated dependency problems, it can be implemented either in software using GPU or directly in hardware.

In variance arithmetic, it is generally an order-of-magnitude more complicated to calculate the result uncertainties than the result values, such as Formula \eqref{eqn: inverse matrix Taylor expansion} for Taylor expansion of matrix inversion.
However, modern software programs for analytic calculation can be great help.
Perhaps it is time to move from numerical programming into analytic programming.
...
This gives clues to engineers on how to improve a measurement effectively.



\subsection{Variable $\sigma$ Variance Arithmetic}

Variance arithmetic rejects a calculation if its distributional pole or distributional zero is within the input ranges.
But there are many use cases in which such intrusion cannot be avoided, so variance arithmetic of smaller $\sigma$ may be needed.
