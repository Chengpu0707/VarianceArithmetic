...
The validation standards are the following:
\begin{enumerate}
\item \emph{tracking power}: If the error deviation is close to $1$.

\item \emph{normality}: If the error distribution is close to normal distribution.
...
\item In the case of \emph{ideal coverage} when each input uncertainty precisely specifies the corresponding input error deviation, the error deviation is $1$, and the normalized errors are normal distributed.
...
The needed minimal noise deviation gives a good estimation of the amount of errors which are not covered by the input uncertainties, such as the numerical errors of library functions.

Gaussian or white noises of specified deviations can also be added to the input to measure the error response for a function.
...
Formula \eqref{eqn: exp precision}, \eqref{eqn: log precision}, \eqref{eqn: sin precision}, and \eqref{eqn: power precision} are tested and verified by the corresponding math library functions \textit{exp}, \textit{log}, \textit{sin}, and \textit{pow}, respectively, in ideal coverage only.
...
\item The value difference of the two results is the value error.
...
After the result uncertainties diverge, the corresponding value deviations still continue on the same trends until the input data start to have significant probability to be less than $0$.
...
A large leakage $\epsilon$ raises the logic questions on the validity of the result, because the leakage means $x \leq 0$ for $\log(x)$ in this case.
In an extreme approach, in another variance arithmetic representation:
\begin{enumerate}

\item Each imprecise value carries its own $\sigma$ or $\epsilon$ to indicate the statistical significance of its uncertainty.

\item The result $\sigma$ or $\epsilon$ involving two imprecise values can be found statistically.

\item The result of an analytic function always converges but at the expense of $\epsilon$, such that $\epsilon \rightarrow 1$ for $\log(x + \delta x)$ when $x \rightarrow 0$. 

\item A calculation has to be stopped if a predefined acceptable leakage level can no longer be achieved, and the major sources of $\epsilon$ are traced for improvements or limitations.  
For example, the acceptable leakage level of $\sigma=5$ dictates the minimal $x$ for a given $\delta x$ in $\log(x + \delta x)$, as the minimal non-divergent $x$ for each $\delta x$ in Figure \ref{fig: Log_Dev}.

\end{enumerate}
The acceptable leakage level should be application specific, so should be $\sigma$.
...
In Figure \ref{fig: Sin_Dev}, Gaussian noises are used, while in Figure \ref{fig: Sin_Dev_Uniform}, ideal white noises are added given the sample size of $10000$.
...
\item 
When the deviation is reduced slightly to below $1/\sigma$ from $0.2$ to $1.9$, Figure \ref{fig: Pow_Dev} shows that the error deviations become much closer to $1$.  
Figure \ref{fig: Pow_Converge} shows reducing $\delta x$ increases the convergence, e.g., $(1 \pm 0.2)^{-1.5}$ diverges while $(1 \pm 0.195)^{-2}$ seems to converge.

\item Figure \ref{fig: Pow_Dev} and \ref{fig: Pow_Converge} shows that the convergence is more sensitive to $\delta x$ than $c$, so the applicable precision threshold is relatively hard, which is estimated as $\delta x = 0.196$ for $c=-2$.

\end{itemize}
Figure \ref{fig: Pow_Converge} shows that the convergence of an analytic function at each input can be judged numerically.
...
Figure \ref{fig: FFT_SinCos_Clean_vs_Order} shows the result uncertainty means for forward, reverse and roundtrip FFT transformations on Sin/Cos signals vs. FFT order $L$.
...
Figure \ref{fig: FFT_SinCos_Clean_vs_Order} also shows that the variance arithmetic is capable of both tracking and bounding the result errors:
\begin{itemize}
...
However, the tracking power of the variance arithmetic is not perfect in Figure \ref{fig: FFT_SinCos_Clean_vs_Order}:
\begin{itemize}
...
The results in the corresponding FFT spectrum in looks no longer structured. 
...
Figure \ref{fig: FFT_Linear_Clean_vs_Order} shows the result uncertainty mean and error maximums and error deviations for forward, reverse and roundtrip FFT transformations on Linear signals vs. FFT order $L$.
...
\item The variance arithmetic has tracking power for FFT transformations, with the error deviations near $1$.
\end{itemize}
...
When the deviation of the input noise is more than $10^{-12}$, the input uncertainties become ideal coverage of the input errors.
\begin{itemize}
\item As expected, the result error deviations for the forward and the reverse transformations are constant $1$.
The result error deviations for the roundtrip transformations are virtually  $0$.
...
The result uncertainty means for both the forward and the reverse transformations is linear to the input uncertainties, because FFT transformations are linear.
...
Smaller FFT order probably means not enough calculations for the error deviations to reach equilibrium.
\end{itemize}
...
Figure \ref{fig:  FFT_Linear_1e-5_vs_Order}  shows the result error deviations and uncertainty means vs. FFT order using Lienar signals with $10^{-5} noise$, when the input uncertainties yet to have idea coverage over the input errors.
The result uncertainty means have the behaviors as if the ideal coverage has been reached.
The result error deviations reach exactly constant $1$ between the FFT order $12$ and $16$ but deviate from $1$ when the FFT order is more than $16$, because more calculations bring more numerical errors from the $\tan$ library function.
...
Figure \ref{fig: FFT_Linear_Forward_ErrorDev_vs_Noise_Order} and \ref{fig: FFT_Linear_Reverse_ErrorDev_vs_Noise_Order} shows the overall dependency of error deviations to both input uncertainties and FFT orders for either the forward or the reverse transform of Linear signals, respectively.
...
However, for the forward transformations, the transition is a cliff toward a flat valley, while it is a cliff toward a rough mountain, showing their  reactions to the numerical errors of the $\tan$ library functions due to their input data, respectively.
...
If both the $\sin$ and the $\tan$ library functions have proper uncertainties, both figures would only contain the ideal coverage except when the FFT orders are too small to let the result statistical properties to become stable.
