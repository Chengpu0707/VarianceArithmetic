...
Such probability is defined as the \emph{bounding leakage}, which relates to the \emph{bounding factor} $\sigma \equiv x / \delta x$ by Formula \eqref{eqn: bounding leakage}, in which $\xi(z)$ is the cumulative density function for normal distribution \cite{Probability_Statistics}.
...
\epsilon = 1 -\xi(\sigma);\eqspace \lim_{\sigma \rightarrow \infty} \epsilon = 0;
\end{equation}
...
-\sigma < \tilde{z} < +\infty:  
&\begin{cases}
\text{$n$ is even}: &(\xi(\sigma) - \varsigma(\sigma, n)) (n-1)!! \\
