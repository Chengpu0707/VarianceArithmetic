...
\item The result variance using Taylor expansion was statistically inaccurate previously. 
The new approach no longer calculate variance around the value if there were no uncertainty.
\end{itemize}
...
m=L-1 \dots 0: \eqspace & H[n,\frac{k}{2^{m}}] = H[n,\frac{k}{2^{m}}] + H[n,\frac{k}{2^{m+1}}]\; \exp{(+i 2\pi \frac{n}{2^{L-m}})}; \\
