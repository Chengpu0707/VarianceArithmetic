...
Analytic functions each with precisely known results are used to validate the result uncertainties from variance arithmetic. 
\begin{itemize}

\item The difference between an arithmetic result and the corresponding known analytic result is defined as the \emph{value error} \cite{Prev_Precision_Arithmetic}.

\item The ratio of the value error to the uncertainty is defined as the \emph{normalized error} \cite{Prev_Precision_Arithmetic}.

\end{itemize}
If the value error is Gaussian distributed with the uncertainty as the distribution deviation, the normalized error is normal distributed.
...
\begin{itemize}
\item the \emph{value deviations}, as the abbreviation for the deviations of the value errors. 
The variance arithmetic matches the value deviations with the corresponding uncertainties.

\item the \emph{error deviations}, as the abbreviation for the deviations of the normalized errors. 
They should be close to $1$, showing the \emph{tracking power} of the variance arithmetic.
They replace the tracking ratios \cite{Prev_Precision_Arithmetic}.
...
They replace the bounding ratios \cite{Prev_Precision_Arithmetic}.
...
The uncertainties and the error deviations are drawn using different y scales on opposite sides of the plot. 
...
The error deviations are drawn using a different y scales which y-axis on the right side of the plot. 
Each color represent a $\delta x$.
}
\label{fig: Log_Dev}
\end{figure}
...
The error deviations are drawn using a different y scales which y-axis on the right side of the plot. 
Each color represent a $\delta x$.
Uniform noises are used instead of Gaussian noised to produce the measured value deviations and the measured error deviations.
}
...
\captionof{figure}{
The individual variance term of $(1 \pm \delta x)^c$ for the different Taylor expansion order $2n$, for different $c$ and $\delta x$ as shown in the legend. 
...
The corresponding value deviation and error deviation are obtained by:
\begin{enumerate}

\item Take 10000 samples from either Gaussian or uniform distribution with $\delta x$ as the deviation, and construct $\tilde{x}$ which is $x$ plus the sampled noise.  

\item Use the corresponding library function to calculate the value as the difference between using $\tilde{x}$ and using $x$ as the input.

\item The deviation of the result values is the value deviation.

\item The difference of each result value with the library function result without noise, as the value error.

\item Each value error is normalized with the result uncertainty, as the normalized error.

\item The deviation of the normalized errors is the error deviation.

\end{enumerate}

%\subsection{Tracking Power}

Figure \ref{fig: Exp_Dev} shows that the calculated uncertainties using Formula \eqref{eqn: exp precision} agree very well with the measured value deviations for $e^{x + \delta x}$, so that the result error deviation is very close to $1$, even both the uncertainties and the value error deviations increase exponentially with $x$ and $\delta x$.
Such strong tracking power holds for all $x$ ranges of $e^{x + \delta x}$. 

%\subsection{Applicable Precision Threshold}

Figure \ref{fig: Log_Dev} shows that the calculated uncertainties using Formula \eqref{eqn: log precision} agree very well with the measured value deviations for $\log(x + \delta x)$, with the result error deviation very close to $1$, until the uncertainties suddenly diverge when the input precision is more than $1/5$ which is the estimated applicable precision threshold.
When the uncertainties diverge, the corresponding value deviations still continue on the same trends, suggesting that such divergence is due to the logic of the variance arithmetic only and the result uncertainties should be rejected.
If the bounding factor $\sigma$ is reduced from $5$ to $4$, the measured applicable precision threshold is also reduced from $1/5$ to $1/4$.
Such reduction increases the bounding leakage $\epsilon$ from $5.7 \times 10^{-7}$ to $6.3 \times 10^{-5}$ according to Formula \eqref{eqn: bounding leakage}, so that the choice of the value for $\sigma$ or $\epsilon$ should be application specific.
Higher value of $\epsilon$ raises logic questions on the validity of the result, because the leakage means $x \leq 0$ for $\log(x)$ in this case.
It is even possible that $\sigma$ or $\epsilon$ be part of another variance arithmetic representation, to make the result uncertainty always converging, such as $\epsilon \rightarrow 1$ for $\log(x + \delta x)$ when $x \rightarrow 0$ for a constant $\delta x$. 
If each imprecise value carries its own $\sigma$ or $\epsilon$, the result $\sigma$ or $\epsilon$ involving two imprecise values can be found statistically.
For the discussion simplicity, the cases of the bounding factor $\sigma$ other than constant $5$ are avoided in this paper, and the input uncertainties should be better than the applicable precision threshold.
...
In Figure \ref{fig: Sin_Dev}, Gaussian noises are used, while in Figure \ref{fig: Sin_Dev_Uniform}, the samples are ideally uniform distributed given the sample size.
...
\item Formula \eqref{eqn: inversion precision} and \eqref{eqn: inversion square precision} show that $(1 \pm 0.2)^{-2}$ diverges faster than $(1 \pm 0.2)^{-1}$, which is confirmed by Figure \ref{fig: Pow_Converge}.
...
It also shows that the convergence is more sensitive to $\delta x$ than $c$, so the applicable precision threshold is relatively hard, which is estimated as $\delta x = 0.196$ for $c=-2$.
...
In each case, the error deviation is very close to 1 when the input precision is better than the corresponding applicable precision threshold.
...
The validation of the variance arithmetic using discrete Fourier transformation will avoid unfaithful frequencies.
...
\captionof{figure}{
The result uncertainty means and uncertainty deviations using Sin/Cos signals vs. FFT order for forward, reverse and roundtrip FFT transformations, as shown in the legend.
}
...
\captionof{figure}{
The result error deviations and error maximums using Sin/Cos signals vs. FFT order for forward, reverse and roundtrip FFT transformations, as shown in the legend.
}
...
\captionof{figure}{
The real and imaginary absolute value errors and uncertainties in the FFT spectrum of a Sin signal of order 6 and index frequency 7 with $10^{-16}$ white noises, as shown in the legend, in which "Error' means value error.
}
...
\captionof{figure}{
The result error deviations and error maximums of Linear signals vs. FFT order for forward, reverse and roundtrip FFT transformations, as shown in the legend.
}
...
\item For the reverse transformation, the input uncertainties come from the peaks while most other values are $0$, so that the result uncertainty deviations are much smaller than the corresponding uncertainty means.
\end{itemize}
...
It may be the reason why the increase of the error deviations of the roundtrip transformation with $L$.
...
The two peaks of the value errors at the index frequencies $\pm 7$ are expected, but Figure \ref{fig: FFT_Sin_Order7_Freq15_Spec_Clean} also shows a few other large peaks, suggesting systematic errors from the library function.
...
For the order $18$ reverse transformations, $10^{-3}$ input noises are necessary for the result error deviation to be 1 and the result value errors to be normal distributed, which is proportional to the $10^{-12}$ input noise necessary for the Sin/Cos signals.
...
\captionof{figure}{
The result uncertainty means using Sin/Cos signals of order 18 vs. input uncertainties for forward, reverse and roundtrip FFT transformations using either Gaussian noise or white noise, as shown in the legend.
}
...
\captionof{figure}{
The result error deviations using Sin/Cos signals of order 18 vs. input uncertainties for forward, reverse and roundtrip FFT transformations using either Gaussian noise or white noise, as shown in the legend.
}
...
\captionof{figure}{
The result error maximums using Sin/Cos signals of order 18 vs. input uncertainties for forward, reverse and roundtrip FFT transformations using either Gaussian noise or white noise, as shown in the legend.
}
...
\captionof{figure}{
The result error maximums using Linear signals of order 18 vs. input uncertainties for forward, reverse and roundtrip FFT transformations using either Gaussian noise or white noise, as shown in the legend.
}
\label{fig: FFT_Linear_ErrorMax_vs_Uncertainty}
\end{figure}


Figure \ref{fig: FFT_Sin_Uncertainty_vs_Uncertainty}  shows the result uncertainty means vs. different input uncertainties using Sin/Cos signals of order 18:
\begin{itemize}
...
\item The roundtrip transform contains most calculations, so it is more sensitive to the numerical errors in the library function.
\end{itemize}
...
Because the $\tan$ library function has much larger least significant values than the $\sin$ library functions, the lower linear limits for the Linear signals are much larger than the corresponding lower linear limits for the Sin/Cos signals: $10^{-11}$ for the forward and the roundtrip transformations, and $10^{-8}$ for the reverse transformation.
...
Figure \ref{fig: FFT_Sin_ErrorDev_vs_Uncertainty} shows the result error deviations vs. different input uncertainties using Sin/Cos of order 18.
...
Figure \ref{fig: FFT_Linear_ErrorDev_vs_Uncertainty} repeats Figure \ref{fig: FFT_Sin_ErrorDev_vs_Uncertainty} for Linear signals instead, which contain much more numerical errors than those for Sin/Cos signals.
...
When the input uncertainties can accurately describe the corresponding errors, the error deviations have a sudden change to $1$ in both Figure \ref{fig: FFT_Sin_ErrorDev_vs_Uncertainty} and \ref{fig: FFT_Linear_ErrorDev_vs_Uncertainty} than the graduate changes to the linear region of the uncertainty means in Figure \ref{fig: FFT_Sin_Uncertainty_vs_Uncertainty} and \ref{fig: FFT_Linear_Uncertainty_vs_Uncertainty}. so checking if the error deviations to be $1$ is a more sensitive check.

Figure \ref{fig: FFT_Sin_ErrorMax_vs_Uncertainty} and \ref{fig: FFT_Linear_ErrorMax_vs_Uncertainty} show the result error maximums vs. different input uncertainties using either Sin/Cos signals, or Linear signals of order 18 are constant $3.1$ except $0$ for the roundtrip transformations when the input uncertainties are large enough so that the numerical errors are no longer effective: $10^{-13}$ for Sin/Cos signals, $10^{-8}$ for Linear signals.
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Linear_Uncertainty_vs_Order.png} 
\captionof{figure}{
The result uncertainty means using Linear signals vs. FFT Orders for forward, reverse and roundtrip FFT transformations with either no noise or $10^{-5}$ Gaussian noise, as shown in the legend.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Linear_Error_vs_Order.png} 
\captionof{figure}{
The result error deviations and error maximums using Linear signals vs. FFT Orders for forward, reverse and roundtrip FFT transformations with $10^{-5}$ Gaussian noise, as shown in the legend.
}
...
When no noise is added, Figure \ref{fig: FFT_Linear_Uncertainty_vs_Order} is different from Figure \ref{fig: FFT_Sin_Uncertainty}: 
\begin{itemize}
...
\item The uncertainty means of the roundtrip transformation increase exponentially with the FFT orderr as $2.09^L$.  
This suggests that the roundtrip transformation can not recover the original uncertainties completely.
\end{itemize}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Linear_ErrorDev_vs_Uncertainty_Order.png} 
\captionof{figure}{
The result error deviations using Linear signals vs. input uncertainties and FFT Orders for roundtrip FFT transformations with $10^{-5}$ Gaussian noise.
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{FFT_Linear_ErrorMax_vs_Uncertainty_Order.png} 
\captionof{figure}{
The result error maximums using Linear signals vs. input uncertainties and FFT Orders for roundtrip FFT transformations with $10^{-5}$ Gaussian noise.
The input uncertainties run from $10^{-16}$ to $10^{-1}$, while the FFT Order runs from $4$ to $18$.
}
...
\item The normalized errors are normal distributed, regardless of the input error distributions, showing the convergence power of the calculation.
\end{itemize}
...
\begin{itemize}
\item For the $18$-order FFT reverse transformations, the measured $10^{-15}$ $\sin$ errors is equivalent to $10^{-12}$ input errors, while the estimated $10^{-10}$ $\tan$ errors is equivalent to $10^{-3}$ input errors. 
...
\end{itemize}
Thus, it is important to rely on the tracking power and the bounding power of the variance arithmetic to contain numerical errors.
