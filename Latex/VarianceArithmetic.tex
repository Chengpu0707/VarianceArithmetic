...
Except the trivial case $\rho(\tilde{z}) = 0, \tilde{z} \in [0, \kappa]$, when $\frac{d \rho(\tilde{z})}{d \tilde{z}}$ is bounded in $\tilde{z} \in [0, \kappa]$, Formula \eqref{eqn: bound moment asymptote deduction left} and \eqref{eqn: bound moment asymptote deduction right} leads to Formula \eqref{eqn: symmetric bound moment asymptote limit}.
...
\forall \kappa > 1&:\;\; 
		\int_{0}^{\kappa} \tilde{z}^n \rho(\tilde{z}) d \tilde{z} \geq \int_{0}^{\kappa} \rho(\tilde{z}) d \tilde{z}\; \frac{\kappa^{n+1}}{n+1}; \\
