...
-\sigma < \tilde{z} < +\infty:  
&\begin{cases}
\text{$n$ is even}: &(\epsilon(\sigma) - \varsigma(\sigma, n)) (n-1)!! \\
...
\begin{figure}%[p]
\centering
\includegraphics[height=2.5in]{Momentum_Residual.png} 
\captionof{figure}{
The momentum residual function $\varsigma(\sigma, n)$ vs the bounding factor $\sigma$, for different $n$ as shown in the legend. 
}
\label{fig: Momentum_Residual}
\end{figure}
...
\delta^2 \ln(x) &\simeq P(x)^2 + \frac{5}{2} P(x)^4 + \frac{14}{3} P(x)^6;
\end{align}
\begin{align}
...
&= \sum_{m=0...\infty, \;n=0...\infty}^{(m,n) \not \in \{(0,0), (0,1), (1,0)\}} M(x, m) M(y, n) 
  \sum_{i=1}^{m-1} \sum_{j=1}^{n-1} \frac{f^{(i,j)}_{(x,y)}}{i!\;j!}\frac{f^{(m-i,n-j)}_{(x,y)}}{(m-i)!\;(n-j)!}; 
\end{align}
