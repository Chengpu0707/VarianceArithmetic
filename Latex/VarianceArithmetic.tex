...
\subsection{Conventional Floating-Point Arithmetic \cite{Prev_Precision_Arithmetic}}
...
Instead of using the threshold probability, the equivalent bounding range for z can be used, such as $|z| \leq 0.67448975$ for the equivalent threshold probability of $50\%$.
...
\item For the variance to converge, in Formula \eqref{eqn: Taylor 1d variance}, the contribution from expansion order $2n$ should decrease with increasing $2n$. 
Otherwise, the result should be invalidated as \emph{not monotonic}.

\end{itemize}
The rejection usually happens near a zero or a pole, such as when the possibility to calculate $1/0$ becomes more than $\epsilon$, so that $1/(1 \pm (0.2 - 1\;10^{-5}))$ is rejected for not monotonic, while $1/(1 \pm (0.2 -2\;10^{-5})) = 1.046 \pm 0.250$.
Figure \ref{fig: Inversion_on_Edge} shows the histograms for $1/(1 \pm \delta x)$ when $\delta x =0.01, 0.1, 0.2 -2\;10^{-5}$.
...
For the worst case of $1/(1 \pm (0.2 -2\;10^{-5}))$, the mean is about $0.4\%$ smaller, while the deviation is about $1.3\%$ smaller.
...
When the input uncertainty is too small, proper coverage cannot be achieved, which is the case when the $\sin(x)$ numerical errors in Figure \ref{fig: Sin_Diff} is combined with the $\cos(x)/\sin(x)$ numerical errors in Figure \ref{fig: Cot_Diff}.
