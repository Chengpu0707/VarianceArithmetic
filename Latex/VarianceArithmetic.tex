...
The reason why Formula \eqref{eqn: power precision} is not applicable needs further discussion:
\begin{itemize}
...
Figure \ref{fig: Pow_Dev} also shows that $(x \pm \delta x)^c$ converges better for smaller $c$ for the same $P(x)$, which is expected from the difference of Formula \eqref{eqn: square precision}, \eqref{eqn: square root precision}, \eqref{eqn: inversion precision} and \eqref{eqn: inversion square precision}.

\item Figure \ref{fig: Pow_Histogram} shows the result distribution deviates significantly from Gaussian for $(1 \pm 0.195)^{-2}$ when $P(x) = 0.2$, because the distribution has too much skewness \cite{Statistical_Methods}.
Figure \ref{fig: Pow_Histogram} also shows that when $P(x)$ becomes finer, the result distribution approaches Gaussian more.
Figure \ref{fig: Inverse_Square_Distribution} predicts the shape of the uncertainty distribution for $(1 \pm 0.195)^{-2}$ in Figure \ref{fig: Pow_Histogram}.
Similarly, Figure \ref{fig: Square_Root_Distribution} predicts the shape of the uncertainty distribution for $(1 \pm 0.195)^{0.5}$ in Figure \ref{fig: Pow_Histogram}.
\end{itemize}
