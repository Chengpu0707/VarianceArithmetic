...
The precision scaling principle chooses to scarify $R$ in favor of $V$, thus introduces the probability for the actual value to be outside the range of $(x - R, x + R)$, e.g., which is $6\;10^{-4}$ When $P_{8/2}$ is replaced by $P_2$.
When $V$ is maximized in its bit confinement in the \emph{normalization} process of the variance representation, such probability is virtually 0 according formula \eqref{eqn: rounding error range vs deviation}.
...
For example, the difference between $1.001 \pm 0.001$ and $1.000 \pm 0.002$ is $0.001 \pm 0.00224$, so that the probability for them to be equal is $2\; \xi(\frac{0.001}{0.00224}) - 1 = 94.6\%$, in which $\xi()$ is the Gaussian error function (which is the cumulative density function of $N()$) \cite{Probability_Statistics}.
If the threshold probability of equality is 95\%, $1.000 \pm 0.002 < 1.001 \pm 0.001$.
...
In addition, while Formula \eqref{eqn: multiplication} is the same regardless of the signs of $x$ and $y$, the result range in Formula \eqref{eqn: interval *} for interval arithmetic changes with the signs of $x$ and $y$. 
Thus, variance arithmetic is more stable than interval arithmetic.

The result of Formula \eqref{eqn: float num calc} calculated by variance arithmetic is $2 \pm 2\sqrt{5}$.
It is $2 \pm 7$ for interval arithmetic \cite{Worst_Case_Error_Bounds}.
...
The $y$-axis is scaled as $\sqrt{\tilde{y}}$ so the distribution is not Gaussian.
}
...
Formula \eqref{eqn: function distribution} can also be deducted using standard probability method \cite{Probability_Statistics}.
If $\rho(\tilde{x}, x, \delta x)$ is given by Formula \eqref{eqn: central limit theorem}, Formula \eqref{eqn: function distribution} can be simplified as Formula \eqref{eqn: Gaussian function distribution}, in which $N(\tilde{z})$ is the probability density function for normal distribution \cite{Probability_Statistics}.
\begin{align}
...
\tilde{z} \equiv \frac{f^{-1}(\tilde{y}) - x}{\delta x}:&\eqspace 
 \tilde{y} = f(x + \tilde{z} \delta x); \eqspace 
 \rho(\tilde{y}, y, \delta y) d \tilde{y} = N(\tilde{z}) d \tilde{z};
\end{align}
...
\end{align}
Figure \ref{fig: Square_Distribution} shows $\rho(\tilde{y}, y, \delta y)$ for $(x \pm 1)^2$.
The $y$-axis is scaled as $\sqrt{\tilde{y}}$ so the distribution is not Gaussian, even $(5 \pm 1)^2$ looks like Gaussian.
When $x = 0$, $\rho(\tilde{y}, y, \delta y)$ is a $\chi^2$ distribution \cite{Probability_Statistics}.

Formula \eqref{eqn: function distribution} can be used trace the uncertainty distribution during calculation.
However, normal scientific and engineering calculations usually do not care about result distribution, but just some statistics of the result.


\subsection{Bounding Factor and Bounding Leakage}

Many functions $f(x)$ are not defined for either $x = 0$ or $x < 0$ or both.
For these functions, the statistical nature of variance arithmetic means that there is non-zero probability for $\tilde{x} \leq 0$ so that the result is not defined.
Such probability is defined as the \emph{bounding leakage}, which relates to the \emph{bounding factor} $\sigma \equiv x / \delta x$ by Formula \eqref{eqn: bounding leakage}, in which $\xi()$ is the Gaussian error function \cite{Probability_Statistics}.
The bounding factor is the inverse of precision of $x \pm \delta x$ for $f(x)$.
Table \ref{tab: bounding leakage} shows the order of magnitude of $\epsilon$ vs $\sigma$.

\begin{equation}
\label{eqn: bounding leakage}
\epsilon = 1 -\xi(\frac{\sigma}{\sqrt{2}});\eqspace \lim_{\sigma \rightarrow \infty} \epsilon = 0;
\end{equation}
...
\hline 
$\sigma$ & 1 & 2 & 3 & 4 & 5 & 6 & 7 & $\frac{255}{32} \simeq 8$ \\ 
\hline 
$\epsilon$ & $10^{-1}$ & $10^{-2}$ & $10^{-3}$ & $10^{-5}$ & $10^{-7}$ & $10^{-9}$ & $10^{-12}$ & $10^{-16}$ \\ 
\hline 
\end{tabular}
\captionof{table}{The bounding leakage $\epsilon$ as a function of bound factor $\sigma$.}
\end{table}

Each imprecise value in the variance representation carries its binding factor, to indicate its bounding leakage.
If a calculation involves more than one imprecise values, the bound factor of the result imprecise value is simply the smallest of all input imprecise values.
...
\overline{f(x)} &\equiv \int \tilde{y} \rho(\tilde{y}, y, \delta y) d \tilde{y}
 ...
 = \sum_{n=0}^{\infty} \frac{f^{(2n)}_x}{2n!} (2n - 1)!! (\delta x)^{2n}; \\
...
&= \sum_{n=0}^{\infty} \sum_{j=0}^{2n} \frac{f^{(j)}_x}{j!} \frac{f^{(2n-j)}_x}{(2n-j)!} (2n - 1)!! (\delta x)^{2n}
 - \sum_{n=0}^{\infty} \sum_{j=0}^{n} \frac{f^{(2j)}_x}{2j!} \frac{f^{(2n-2j)}_x}{(2n-2j)!} (2j-1)!! (2n - 2j - 1)!! (\delta x)^{2n} \\
&= \sum_{n=1}^{\infty} \sum_{j=1}^{2n-1} \frac{f^{(j)}_x}{j!} \frac{f^{(2n-j)}_x}{(2n-j)!} (2n - 1)!! (\delta x)^{2n}
 - \sum_{n=1}^{\infty} \sum_{j=1}^{n-1} \frac{f^{(2j)}_x}{2j!} \frac{f^{(2n-2j)}_x}{(2n-2j)!} (2j-1)!! (2n - 2j - 1)!! (\delta x)^{2n} \\
&\;\; + \sum_{n=1}^{\infty} 2 f_x \frac{f^{(2n)}_x}{(2n)!} (2n - 1)!! (\delta x)^{2n} 
 - \sum_{n=1}^{\infty} 2 f_x \frac{f^{(2n)}_x}{(2n)!} (2n - 1)!! (\delta x)^{2n} \\
&= \sum_{n=1}^{\infty} \sum_{j=1}^{2n-1} \frac{f^{(j)}_x}{j!} \frac{f^{(2n-j)}_x}{(2n-j)!} (2n - 1)!! (\delta x)^{2n} - \widehat{f(x)}^2;
\end{align*}

\begin{align*}
& e^{x + \tilde{x}} - e^x = e^x \sum_{j=1}^{\infty} \frac{\tilde{x}^j}{j!}: \\
\widehat{e^x} / e^x &\simeq \frac{1}{2} \delta^2 x + \frac{1}{8} (\delta^2 x)^2 + \frac{1}{48} (\delta^2 x)^3 + \frac{1}{384} (\delta^2 x)^4; \\
P(e^x)^2 &\simeq \delta^2 x + (\frac{7}{4} - \frac{1}{4}) (\delta^2 x)^2 + (\frac{31}{24} - \frac{1}{8}) (\delta^2 x)^3 + 
         (\frac{127}{192} - \frac{1}{64} - \frac{1}{48}) (\delta^2 x)^4 \\
         &= (\delta^2 x) + \frac{3}{2} (\delta^2 x)^2 + \frac{7}{6} (\delta^2 x)^3 + \frac{5}{8} (\delta^2 x)^3; 
\end{align*}

\begin{align*}
& \ln(x + \tilde{x}) - \ln(x) = \sum_{j=1}^{\infty} \frac{(-1)^{j+1}}{j} \frac{\tilde{x}^j}{x^j}: \\
\widehat{\ln(x)} &\simeq - \frac{1}{2} P(x)^2 - \frac{3}{4} P(x)^4 - \frac{15}{6} P(x)^6 - \frac{105}{8} P(x)^8; \\
\delta^2 \ln(x) &\simeq P(x)^2 + (\frac{11}{4} - \frac{1}{4}) P(x)^4 + (\frac{65}{12} - \frac{3}{4}) P(x)^6 +
   (\frac{105}{16} - \frac{9}{16} - \frac{5}{2}) P(x)^8 \\
 &= P(x)^2 + \frac{5}{2} P(x)^4 + \frac{14}{3} P(x)^6 + \frac{7}{2} P(x)^8;
\end{align*}

\begin{align*}
\frac{(x + \tilde{x})^c}{x^c} - 1 &= \sum_{j=1}^{\infty} P(x)^j \prod_{k=1}^{j} \frac{c + 1 - k}{k} \\
 &\simeq c P + \frac{c(c-1)}{2} P^2 + \frac{c(c-1)(c-2)}{6} P^3 + \frac{c(c-1)(c-2)(c-3)}{24} P^4 + \frac{c(c-1)(c-2)(c-3)(c-4)}{120} P^5; \\
\frac{\widehat{x^c}}{x^c} &\simeq \frac{1}{2} c(c-1) P^2 + \frac{1}{8} c(c-1)(c-2)(c-3) P^4 + \frac{1}{48} c(c-1)(c-2)(c-3)(c-4)(c-5) P(x)^6; \\
P(x^c)^2 &\simeq c^2 P^2 + \left(\frac{3 c^2(c-1)^2}{4} + c^2(c-1)(c-2) - \frac{c^2(c-1)^2}{4} \right) P^4 \\
 &\eqspace + \left(\frac{5 c^2(c-1)^2(c-2)^2}{12} + \frac{5 c^2(c-1)^2(c-2)(c-3)}{8} + \frac{c^2(c-1)(c-2)(c-3)(c-4)}{4} - \frac{1}{8} c^2(c-1)^2(c-2)(c-3) \right) P^6 \\
 & = c^2 P^2 + \frac{3}{2} c^2(c-1)(c-\frac{5}{3}) P^4 + \frac{7}{6} c^2(c-1)(c-2)^2(c-\frac{8}{3}) P^6;
\end{align*}

\fi

The mean $\overline{f(x)}$ of $f(x + \tilde{x})$ can be decomposed as the sum of:
\begin{itemize}
\item The \emph{accurate result} $f(x)$: the result as if there were no input uncertainty; 

\item The \emph{calculation bias} $\widehat{f(x)}$: the result changes due to the input uncertainty.
\end{itemize} 
If $f(x)$ can be Taylor expanded as Formula \eqref{eqn: Taylor 1d}, using Formula \eqref{eqn: Gaussian function distribution}, the calculation bias $\widehat{f(x)}$ and variance $\delta^2 f(x)$ of the result distribution are calculated by Formula \eqref{eqn: calc bias} and \eqref{eqn: Taylor 1d variance}, respectively.  
$\delta^2 f(x) + \widehat{f(x)}^2$ is defined as defined as the \emph{calculation variance}.
As shown in Formula \eqref{eqn: Taylor 1d variance}, like the calculation bias, the calculation variance is centered around $f(x)$ instead of $\overline{f(x)}$.
Usually $\widehat{f(x)}^2 \ll \delta^2 f(x)$, so a calculation variance is a good approximation for the corresponding statistical variance.
The calculation bias in $x$ is transfer to $f(x)$ according to Formula \eqref{eqn: accum calc bias}.
\begin{align}
\label{eqn: Taylor 1d} 
\tilde{y} &= f(x + \tilde{z} \delta x) = \sum_{n=0}^{\infty} \frac{f^{(n)}_x}{n!} \tilde{z}^n (\delta x)^n; \\
\label{eqn: variance momentum}
& M(x, n) \equiv (\delta x)^n \int \tilde{z}^n N(\tilde{z}) d \tilde{z}; \\
\label{eqn: calc bias}
\widehat{f(x)} &\equiv \overline{f(x)} - f(x) =  \int (f(x + \tilde{x}) - f(x)) \rho(\tilde{x}, x, \delta x) d \tilde{x}
 = \sum_{n=1}^{\infty} M(x, n) \frac{f^{(n)}_x}{n!}; \\
\label{eqn: Taylor 1d variance}
\delta^2 f(x) & + \widehat{f(x)}^2 = \int (f(x + \tilde{x}) - f(x))^2 \rho(\tilde{x}, x, \delta x) d \tilde{x} 
= \sum_{n=1}^{\infty} M(x, n) \sum_{j=1}^{n-1} \frac{f^{(j)}_x}{j!} \frac{f^{(n-j)}_x}{(n-j)!}; \\
\label{eqn: accum calc bias}
\widehat{f(\widehat{x})} &= f(x + \widehat{x}) - f(x) + \sum_{n=1}^{\infty} M(x, n) \frac{f^{(n)}_x}{n!};
\end{align}

Formula \eqref{eqn: calc bias} and \eqref{eqn: Taylor 1d variance} both contains \emph{variance momentum} which is defined in Formula \eqref{eqn: variance momentum}.
The integration range of Formula \eqref{eqn: variance momentum} is limited by $f(x)$, such as $0 \le x + \tilde{x}$ in $\sqrt{x}$.  
$M(x, n)$ has different values for different ranges, as show in Formula \eqref{eqn: nth variance momentum} \cite{Gaussian_Integals}, in which $\xi()$ is the Gaussian error function, $\sigma \equiv x /\delta x$ is the bounding factor, and $\epsilon$ is the corresponding bounding leakage for $\sigma$ according to Formula \eqref{eqn: bounding leakage}:
\begin{align}
\label{eqn: nth variance momentum}
& \varsigma(\sigma, n) \equiv N(\sigma)\; \sum_{j=0}^{n/2} \frac{\sigma^{2j+1}}{(2j+1)!!};
	 \eqspace \lim_{\sigma \rightarrow \infty} \varsigma(\sigma, n) = 0;	 \nonumber \\
M(x, n) &= (\delta x)^n 
\begin{cases}
-\infty < \tilde{z} < +\infty:   
&\begin{cases}
...
-\sigma < \tilde{z} < +\infty:  
&\begin{cases}
\text{$n$ is even}: &(1 - \epsilon(\sigma) - \varsigma(\sigma, n)) (n-1)!! \\
\text{$n$ is odd}:  &\varsigma(\sigma, n) (n-1)!! 
\end{cases} 
\end{cases};
\end{align}

When $\delta x \ll x$, Formula \eqref{eqn: exp bias}, \eqref{eqn: exp precision}, \eqref{eqn: log bias}, \eqref{eqn: log precision}, \eqref{eqn: power bias} and \eqref{eqn: power precision} give the result calculation bias and precision for $e^x$, $\ln(x)$, and $x^c$, respectively.
\begin{align}
\label{eqn: exp bias}
\frac{\widehat{e^x}}{e^x} &\simeq \frac{1}{2} (\delta^2 x) + \frac{1}{8} (\delta^2 x)^2 + \frac{1}{48} (\delta^2 x)^3; \\
\label{eqn: exp precision}
P(e^x)^2 &\simeq (\delta^2 x) + \frac{3}{2} (\delta^2 x)^2 + \frac{7}{6} (\delta^2 x)^3 + \frac{5}{8} (\delta^2 x)^3;; \\
\label{eqn: log bias}
\widehat{\ln(x)} &\simeq - \frac{1}{2} P(x)^2 - \frac{3}{4} P(x)^4 - \frac{5}{2} P(x)^6; \\
\label{eqn: log precision}
\delta^2 \ln(x) &\simeq P(x)^2 + \frac{5}{2} P(x)^4 + \frac{14}{3} P(x)^6 + \frac{7}{2} P(x)^8;
\end{align}
\begin{align}
\label{eqn: power bias}
\frac{\widehat{x^c}}{x^c} &\simeq \frac{c(c-1)}{2} P(x)^2 + \frac{c(c-1)(c-2)(c-3)}{8} P(x)^4 + \nonumber \\
 &\eqspace\eqspace \frac{c(c-1)(c-2)(c-3)(c-4)(c-5)}{48} P(x)^6; \\
\label{eqn: power precision}
P(x^c)^2 &\simeq c^2 P(x)^2 + \frac{3}{2} c^2(c-1)(c-\frac{5}{3}) P(x)^4 + \nonumber \\
 &\eqspace\eqspace \frac{7}{6} c^2(c-1)(c-2)^2(c-\frac{8}{3}) P(x)^6;
\end{align}
Some special cases for Formula \eqref{eqn: power precision} are:
\begin{align}
\label{eqn: square precision}
P(x^2)^2 &= 4 P(x)^2 + 2 P(x)^4; \\
\label{eqn: square root precision}
P(\sqrt{x})^2 &\simeq \frac{1}{4} P(x)^2 + \frac{7}{32} P(x)^4 + \frac{91}{128} P(x)^6; \\
\label{eqn: inversion precision}
P(1/x)^2 &\simeq P(x)^2 + 8 P(x)^4 + 77 P(x)^6;
\end{align}
...
&= \sum_{m=0}^{\infty} \sum_{n=0}^{\infty} M(x, m) M(y, n)  \frac{f^{(m,n)}_{(x,y)}}{m! n!} - f(x,y); \\
...
&= \sum_{m=0}^{\infty} \sum_{n=0}^{\infty} M(x, m) M(y, n) 
  \sum_{i=1}^{m-1} \sum_{j=1}^{n-1} \frac{f^{(i,j)}_{(x,y)}}{i!\;j!}\frac{f^{(m-i,n-j)}_{(x,y)}}{(m-i)!\;(n-j)!} - f(x,y)^2; 
\end{align}
...
The variance formula using Taylor expansion shows the nature of the calculation, such as the different nature of variance increases of $e^x$ vs $\ln x$, as show in Formula \eqref{eqn: exp precision} vs \eqref{eqn: log precision}, or the difference speed of variance increases of $x^c$ depending on different $c$, as shown in Formula \eqref{eqn: power precision}.
Formula \eqref{eqn: power precision} also shows that the exponent $c$ determines the convergence range of $P(x)$, as shown in the difference among Formula \eqref{eqn: square precision}, \eqref{eqn: square root precision}, and \eqref{eqn: inversion precision}.
...
\begin{itemize}
\item The \emph{linear Deviation}, which is the square root of the $(\delta x)^2$ term in Formula \eqref{eqn: Taylor 1d variance}. 

\item The \emph{calculation cost}, which is the total deviation minus the linear deviation.
\end{itemize}
The separation of a calculation variance into a linear variance and a calculation cost can only be done for analytic calculations. 


The linear deviation is wrong if the independent uncertainty assumption is applied wrongly.
For example, if the square of an input imprecise value is mistakenly carried out as multiplication of two different input imprecise values, the result linear deviation is only $\frac{1}{\sqrt{2}}$-fold according to Formula \eqref{eqn: square precision} and \eqref{eqn: multiplication precision}.
...
For example, derived from Formula \eqref{eqn: Taylor 1d variance}, Formula \eqref{eqn: sum dependency} calculate the sum of any two functions.
For $f(x) - f(x)$, the linear deviation is $0$ according to Formula \eqref{eqn: sum dependency}.
Formula \eqref{eqn: sum dependency} can be extended to multiple variables, such as to $\delta^2 (f(x, y) + g(x, y))$.
Using Formula \eqref{eqn: sum dependency}, Formula \eqref{eqn: product dependency}  calculate the product of any two functions.
\begin{align}
\label{eqn: sum dependency}
\delta^2 (f(x) + g(x)) &= \delta^2 f(x) + \delta^2 g(x) + 2f^{(1)}_x g^{(1)}_x (\delta^2 x)
     + 2 \sum_{n=2}^{\infty} M(x,n) \sum_{j=1}^{n-1} \frac{f^{(n-j)}_x }{(n-j)!} \frac{g^{(j)}_x}{j!}; \\
\label{eqn: product dependency}
\delta^2 (fg) &= \delta^2 e^{\log(fg)} = \delta^2 e^z |_{e^z = fg,\;\; \delta^2 z = \delta^2 (\log(f) + \log(g))};
\end{align}
The linear deviation becomes $0$ when the calculation results in a constant.

After the $f^{-1}(f(x))$ calculation, the linear deviation can be recovered completely, while the calculation cost always increases.
The increase of the calculation cost depends on the actual steps of calculation, which is called the \emph{dependency problem}.
For example, Formula \eqref{eqn: square root} and \eqref{eqn: root square} shows that the calculation cost of $\sqrt{x}^2$ on the $P(x)^4$ term is greater by 2-fold than that of $\sqrt{x^2}$.
\begin{align}
\label{eqn: square root}
P(\sqrt{x}^2)^2 &\simeq P(x)^2 + 4 P(x)^4; \\
\label{eqn: root square}
P(\sqrt{x^2})^2 &\simeq P(x)^2 + P(x)^4;
\end{align}
...
\item The bounding factor $\sigma$ is stored as as a 8-bit fix-point none-negative value with $\frac{255}{32} \simeq 8$ as the maximum.

\item An optional linear variance is stored using conventional 64-bit floating-point representation.

\item An optional calculation bias is stored using conventional 64-bit floating-point representation.
\end{itemize}
The total bit count of each value in the variance representation is either 128 or 256.
