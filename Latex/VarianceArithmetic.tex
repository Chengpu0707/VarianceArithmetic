...
\begin{figure}
\includegraphics[width=4.5in,height=2.75in]{Sin_Cot_Diff.png} 
\captionof{figure}{
...
\begin{figure}[p]
\includegraphics[width=4.5in,height=2.75in]{Cot_Diff.png} 
\captionof{figure}{
...
\item The obtained indexed sine functions is checked by $\sin(x)^2 + \cos(x)^2 - 1$.
Figure \ref{fig: Sin_Cot_Diff} shows that the indexed sine functions have proper coverage for different FFT orders.
...
Figure \ref{fig: Cot_Diff} shows the value errors between the library $\cos(x)/\sin(x)$ and the indexed $\cos(x)/\sin(x)$ for $x \in (0, \pi)$.
...
Using the maximal absolute value errors, Figure \ref{fig: Sin_Cot_Diff} compares the value error strengths of the library $\sin(x)$ and $\cos(x)/\sin(x)$ for different FFT orders.
Numerically, the library $\sin(x)$ and $\cos(x)/\sin(x)$ do not have proper coverage.
...
The x-axis shows the index time.
The y-axis shows the absolute value errors.
}
...
Using the indexed sine functions, for the waveform of $\sin(2\pi k 1/2^4)$ in which $k$ is the index time:
\begin{itemize}
\item Figure \ref{fig: FFT_Sin_Clean_1_4_Spec_Indexed} shows that for the forward transformation,  the result value errors are comparable to the result uncertainties, with an error deviation of $0$ for the real part, and $1.11$ for the imaginary part.

\item Figure \ref{fig: FFT_Sin_Clean_1_4_Wave_Indexed} shows that for the reverse transformation, the result value errors are comparable to the result uncertainties, with an error deviation of $0$ for the real part, and $0.474$ for the imaginary part.
...
\begin{figure}[p]
\includegraphics[width=4.5in,height=2.75in]{FFT_Sin_Clean_4_1_Spec_Lib.png} 
\captionof{figure}{
The FFT spectrum of $\sin(j 1/2^4 \pi)$ using the library sine functions after the forward transformation calculated by the variance arithmetic, with the uncertainties and the value errors shown in the legend.
The x-axis shows the index frequency.
The y-axis shows the absolute value errors.
}
\label{fig: FFT_Sin_Clean_4_1_Spec_Lib}
\end{figure}

\begin{figure}[p]
\includegraphics[width=4.5in,height=2.75in]{FFT_Sin_Clean_4_1_Wave_Lib.png} 
\captionof{figure}{
The FFT waveform of $\sin(j 4/2^4 \pi)$ using the indexed sine functions after the reverse transformation calculated by the variance arithmetic, with the uncertainties and the value errors shown in the legend.
The x-axis shows the index time.
The y-axis shows the absolute value errors.
No input noise is added to the Sin signal.
}
\label{fig: FFT_Sin_Clean_4_1_Wave_Lib}
\end{figure}
...
\begin{figure}[p]
\includegraphics[width=4.5in,height=2.75in]{FFT_Sin_Clean_6_16_Wave_Lib.png} 
\captionof{figure}{
The FFT waveform of $\sin(j 4/2^4 \pi)$ using the indexed sine functions after the reverse transformation calculated by the variance arithmetic, with the uncertainties and the value errors shown in the legend.
The x-axis shows the index time.
The y-axis shows the absolute value errors.
...
\label{fig: FFT_Sin_Clean_6_16_Wave_Lib}
\end{figure}
...
\begin{itemize}
\item Figure \ref{fig: FFT_Sin_Clean_4_1_Spec_Lib} shows that for the forward transformation of $\sin(2\pi k 1/2^4)$, the result value errors are noticeably larger than the result uncertainties, with an error deviation of $2.61$ for the real part, and $2.51$ for the imaginary part.
As expected, the error deviations are noticeably larger than their counterparts in Figure \ref{fig: FFT_Sin_Clean_1_4_Spec_Indexed}, with the real part being no longer $0$.

\item Figure \ref{fig: FFT_Sin_Clean_4_1_Wave_Lib} shows that for the reverse transformation of $\sin(2\pi k 1/2^4)$, the result value errors are noticeably larger than the result uncertainties, with an error deviation of $2.60$ for the real part, and $	1.87 \; 10^{18}$ for the imaginary part. 
The surprisingly large imaginary error deviation is caused by the $0$ uncertainty at the index time of $8$, at where the value error is no longer $0$ due to the numerical errors of the library sine functions.
...
In Figure \ref{fig: FFT_Sin_Clean_4_1_Wave_Lib}, the value errors tend to increase with the indexes. 
This is actually due to the increase of the numerical errors of $\sin(x)$ with $x$ as shown in Figure \ref{fig: Sin_Diff}.
The periodicity of such increase in Figure \ref{fig: Sin_Diff} can cause a much bigger problem in rare cases: the \emph{error resonance} between the numerical errors of the library sine functions and the input data.
Figure \ref{fig: FFT_Sin_Clean_4_4_Wave_Lib} shows an uniform increase of the real value errors of the reverse transformation with the increasing index time at the Nyquist frequency of FFT order $4$.
Figure \ref{fig: FFT_Sin_Clean_6_16_Wave_Lib} shows an even more elaborated increase pattern  at the Nyquist frequency of FFT order $6$.
Larger noises is needed to completely break such resonance: empirically, $10^{-15}$ for FFT order $4$.
...
When too much numerical errors are not specified by the input uncertainties, proper coverage can not be achieved, which is the case when the $\sin(x)$ numerical errors in Figure \ref{fig: Sin_Diff} is combined with the $\cos(x)/\sin(x)$ numerical errors in Figure \ref{fig: Cot_Diff}.
...
It is quite possible that ideal coverage is not achievable, e.g., visually, when the FFT order is larger than $25$ for the reverse transformation.
...
In other regions, proper coverage are achievable.
...
In rare cases the numerical errors in the library function can resonant with the input data, which could make the result meaningless.
The dependency of the result numerical errors on the amount of calculation and input data means that a small scaled test can not properly qualify the result of a large scaled calculation.
The effect of numerical errors inside math library has not been addressed seriously.

The variance arithmetic should be used, whose uncertainties trace all input errors, and whose result error deviations qualify the calculation quality as either ideal, or proper, or suspicious.
