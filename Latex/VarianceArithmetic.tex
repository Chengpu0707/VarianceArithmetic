...
\captionof{figure}{
The largest $\delta x$ for  $\ln{x \pm \delta x}$ for different $x$ as shown by the x-axis, using the y-axis on the left.
...
It shows that the variance arithmetic allows normalized error distributions to deviate from Normal to a large degree, as what Formula \eqref{eqn: variance momentum UN} has suggested.
...
\captionof{figure}{
The histogram of the error deviation for $\sin(x \pm \delta x)$, for $x=0, +\pi/2, +\pi/4$ and $\delta x = 0.001$ as shown in the legend.
...
\captionof{figure}{
The largest $\delta x$ for  $\sin(x \pm \delta x)$ for different $x$ as shown by the x-axis, using the y-axis on the left.
Also shown are the corresponding uncertainty bias and uncertainty for different $x$ as shown by the x-axis, using the y-axis on the right.
}
