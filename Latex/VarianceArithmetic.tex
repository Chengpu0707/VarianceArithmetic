...
Therefore, the variance arithmetic has no dependency problem.
...
Formula \eqref{eqn: multiplication} can be changed to Formula \ref{eqn: multiplication precision} which is significant statistically: the product becomes less precise.
\begin{align}
...
P(x y)^2 &= P(x)^2 + P(y)^2 + P(x)^2 P(y)^2;
\end{align}
...
It is an approximation to the precision $P(f(x)) =\frac{\delta f(x)}{|\overline{f(x)}|}$ when the bias is ignored.
...
&\eqspace - \sum_{i=0}^{m} \sum_{j=0}^{n} \frac{\zeta(2i) \zeta(2j) f^{(2i,2j)}_{(x,y)}}{(2i)!(2j)!}
...
Table \ref{tbl: matrix rounding errors} shows that the variance arithmetic achieves proper coverage for the calculation rounding errors for adjugate matrix.
...
In Formula \eqref{eqn: inverse matrix}, $\M^{-1}$ is dominated by $1/|\M|$, suggesting that the precision of $\M^{-1}$ is determined by the precision of $|\M|$.
...
Formula \eqref{eqn: determinant variance approx} gives the first approximation of Formula \eqref{eqn: determinant variance}, in which $\tilde{x}_{i,j}$ is an imprecise element.
...
\begin{figure}
\centering
\includegraphics[height=2in]{Moving_Window_Linear_Fit.png} 
\captionof{figure}{ Coefficients of X in Formula \eqref{eqn: time-series linear regression} at current and next position in a time series of the least square linear regression. Except the two end points at $X=-H$ and $X=H+1$, respectively, the coefficient difference between the current and then next position in a time series are all by 1 in the overlapping region from $X=-H+1$ to $X=H$, which results in \eqref{eqn: moving-window linear regression}. }
...
According to Figure \ref{fig: Moving_Window_Linear_Fit}, in which $H$ takes an example value of 4, the calculation of $(\alpha _{j}, \beta _{j})$ can be obtained from the previous values of $(\alpha _{j-1}, \beta _{j-1})$, to reduce the calculation of Formula \eqref{eqn: time-series linear regression} into a progressive moving-window calculation of Formula \eqref{eqn: moving-window linear regression}:
\begin{equation}
\begin{split}
...
Formula \eqref{eqn: moving-window linear regression} uses each input multiple times, so it will have dependency problem for all the three uncertainty-bearing arithmetic.  The question is how the overestimation of uncertainty evolves with time. 
