...
Each probability density function $\rho(\tilde{x})$ in solid line is compared with a Normal distribution $\varrho(\tilde{x})$ of the same mode in the dash line of the same color.
...
Some probability density function $\rho(\tilde{x})$ in solid line is compared with a Normal distribution $\varrho(\tilde{x})$ of the same mode in the dash line of the same color.
The mode of $\rho(\tilde{x})$ is calculated by Formula \eqref{eqn: power mode}.
The difference between $\rho(\tilde{x})$ and $\varrho(\tilde{x})$ is calculated as $\int_{-\infty}^{+\infty} |\rho(\tilde{x}) - \varrho(\tilde{x})| d \tilde{x}$ which is shown as the percentage value of the legend for the normal distribution.
}
...
The probability density function $\rho(\tilde{x})$ in solid line is compared with a Normal distribution $\varrho(\tilde{x})$ of the same mode in the dash line of the same color.
The mode of $\rho(\tilde{x})$ is calculated by Formula \eqref{eqn: power mode}.
The difference between $\rho(\tilde{x})$ and $\varrho(\tilde{x})$ is calculated as $\int_{-\infty}^{+\infty} |\rho(\tilde{x}) - \varrho(\tilde{x})| d \tilde{x}$ which is shown as the percentage value of the legend for the normal distribution.
}
...
Gaussian noise or white noise are added to simulate input data of specified uncertainty deviation, and statistical methods are used to characterize result uncertainty for each calculation.
...
\captionof{figure}{
The measured vs calculated uncertainty deviation for $\sin(x \pm \delta x)$, for different $\delta x$ as shown in the legend. 
The $x$-axis is in radians divided by $\pi$.
...
$\delta^2 \sin(x)$ are shown to have the same periodicity as $\sin(x)$.
}
\label{fig: Sin_Dev}
\end{figure}
...
\captionof{figure}{
The measured vs calculated uncertainty deviation for $(1 \pm \delta x)^c$, for different $\delta x$ as shown in the legend. 
...
Each measured values fits well with the corresponding calculated value except when $c < -1.5$ and $\delta x = 0.2$.
}
\label{fig: Pow_Dev}
\end{figure}
...
\captionof{figure}{
The individual variance term of $(1 \pm \delta x)^{-2}$ for the different Taylor expansion order $2n$, and different $\delta x$ as shown in the legend. 
The $x$-axis is Taylor expansion order $2n$ from 1 to 400 in Formula \eqref{eqn: Taylor 1d variance}.
The $y$-axis is the contribution to $(1 \pm \delta x)^{-2}$ at each Taylor expansion order $2n$ according to Formula \eqref{eqn: power precision}.
It shows that $(1 \pm 0.2)^{-2}$ clearly diverges, while $(1 \pm 0.195)^{-2}$ clearly converges.
}
...
The $x$-axis is the deviation from the mean of $(1 \pm \delta x)^{c}$.
...
Formula \eqref{eqn: exp precision}, \eqref{eqn: log precision}, \eqref{eqn: sin precision}, and \eqref{eqn: power precision} are tested by the corresponding math library functions.
...
\item Figure \ref{fig: Pow_Histogram} shows that when Formula \eqref{eqn: inversion square precision} is no longer applicable due to large $P(x)$, the result distribution deviates significantly from Gaussian, such as for $(1 \pm 0.195)^{-2}$ when $P(x) = 0.2$.
Figure \ref{fig: Inverse_Square_Distribution} is the corresponding theoretical distribution of Figure \ref{fig: Pow_Histogram} but with different $x$ scaling, which correctly predicts the shape of the Gaussian distribution in Figure \ref{fig: Pow_Histogram}.
\end{itemize}
In the case of $(1 \pm \delta x)^{-2}$, the failure of Formula \eqref{eqn: inversion square precision} suggests that the result uncertainty distribution can no longer be approximated by Gaussian, so that the actual result uncertainty distribution according to Formula \eqref{eqn: power distribution} should be used to calculate the result variance.
Weather this observation can be generalized remains a question.

Adding Gaussian noise or white noise does not change the result significantly.
Formula \eqref{eqn: exp precision}, \eqref{eqn: log precision}, \eqref{eqn: sin precision}, and \eqref{eqn: power precision} are all verified by the corresponding math library functions.
