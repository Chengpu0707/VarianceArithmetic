...
In the legend,  
\textit{Absolute Reminder} is calculated by Formula \eqref{eqn: Taylor expansion reminder}, 
\textit{Expansion Uncertainty} is the reminder uncertainty calculated by variance arithmetic,
and \textit{Expansion LSV} is the least significant value for the reminder.
}
...
For $x = \pm 0.6, 0.7$, $r(N)$ reach $LSV$, respectively, and the result $LSV$ equal the corresponding result uncertainties, respectively. 
For $x = -0.7$, $r(N)$ does not reach $LSV$, leaving the least two significant bits incorrect.
Traditional numerical Taylor expansion is less accurate than its theoretical claim.

The difference between the Taylor expansion result and the precise result is the value error for the Taylor expansion.
...
\begin{enumerate}
\item When $x^N$ is added to the sum for Formula \eqref{eqn: Taylor expansion +} or \eqref{eqn: Taylor expansion -}, it is digitized by the $LSV$ of the corresponding sum, to generate rounding error at $n$.
...
\item When $x^N$ reaches below half of $LSV$, it vanishes so that the corresponding expansion terminates effectively.

\item Figure \ref{fig: Taylor_Expansion_Uncertainty} shows that while the accumulated rounding errors fluctuate around $0$, the last accumulated rounding errors when $x^N$ approaches $LSV$ determines the value error of of the expansion, such as the accumulation of 2-bit errors for $1/(1 - 0.7)$.

\item When an integer is repeatedly divided by $2$ and after each division the result is rounded to the nearest, the value of the integer before it becomes $0$ is not randomly distributed, e.g, it is always $1$ in the last step, and predominantly 2 in the step before, and more and more randomly distributed in the steps before, as shown in Table \ref{tbl: rounding pattern}.
...
\item $x^N$ approaches $LSV$ faster for $x = 0.6$ than that for $x = 0.7$, and faster than the pattern in Table \ref{tbl: rounding pattern}, so that the accumulated rounding errors does not cause result value error.

\item When $LSV \ll x^N$, the value pattern in Table \ref{tbl: rounding pattern} is random.
It mean that rounding errors can not accumulate steadily.

\end{enumerate}
The reason for Taylor expansion value error is due to floating-point rounding errors.
To be free from such rounding errors, the convergence input range for an expansion has to be smaller than the theoretical input range without considering  floating-point rounding errors, e.g., for $1/(1v -|x|)$, $|x|$ has to be smaller than $0.7$.

In Figure \ref{fig: Taylor_Expansion_Uncertainty}, although the result uncertainty has the correct trend, it does not reach $r(N)$ at the end.
The reason why Formula \eqref{eqn: Taylor 1d variance} does not succeed completely for $1/(1 - 0.7)$ is due to the digital limitation of the conventional floating-point representation.
The smallest positive floating-point value is about $5\; 10^{-324}$. 
The least significant value for $0.7$ is about $1\; 10^{-16}$.
At $n=100$, applying Formula \eqref{eqn: polynomial Taylor} to Formula \eqref{eqn: Taylor 1d variance} has $100$ terms from $(\delta x)^2$ to  $(\delta x)^{200}$, but only first $10$ terms can be calculated before $(\delta x)^{2n}$ vanishes below $5\; 10^{-324}$.
...
\begin{figure}[p]
\centering
\includegraphics[height=2.25in]{Taylor_Expansion_0.01.png} 
\captionof{figure}{
The absolute reminders for Taylor expansion of $1/(1-x)$, for different Taylor expansion order $N$ as shown in the x-axis, and different imprecise $x$ as shown in the legend.
The y-axis is for expansion uncertainty or absolute reminder value error.
In the legend,  
\textit{Absolute Reminder Value} is the absolute value calculated by Formula \eqref{eqn: Taylor expansion reminder}, 
\textit{Expansion LSV} is the least significant value for the expansion sum value, and
\textit{Expansion Uncertainty} is the calculated expansion sum uncertainty.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.25in]{Taylor_Expansion_0.04.png} 
\captionof{figure}{
The absolute reminders for Taylor expansion of $1/(1-x)$, for different Taylor expansion order $N$ as shown in the x-axis, and different imprecise $x$ as shown in the legend.
The y-axis is for expansion uncertainty or absolute reminder value error.
In the legend,  
...
When input uncertainty increases from $0.01$ to $0.04$, Figure \ref{fig: Taylor_Expansion_0.04} shows that the the absolute reminder values decrease slower than  exponentially with the increasing expansion order, while the expansion uncertainty remains about the same as those of input uncertainty of $0.01$.
For example, the expansion of $1/(1 + 0.7 \pm 0.04)$ now terminates at Taylor expansion order $105$ and it can no longer reach the $LSV$ of the result, which is quite similar to the expansion of $1/(1 - 0.7 \pm 0.01)$ in Figure \ref{fig: Taylor_Expansion_0.01}.
The reason why for the expansion changes is due to the increase of uncertainty bias in Formula \eqref{eqn: polynomial Taylor}.
The result of the Taylor expansion still agrees with the expected result in all the cases, despite slower convergence with larger input uncertainty.

When input uncertainty increases further to $0.06$,the Taylor expansion of $1/(1 + 0.6 \pm 0.06) = 1/(0.4 \pm 0.06) = 2.56 \pm 0.415$ can no longer reach the $LSV$ of the expected result and start to diverge, even though the input precision $0.15$ for the inversion is below the approximate applicable threshold of $0.2$.
It is questionable to use the termination standard for precise input to imprecise inputs.
Specifically, it is questionable to to always calculate the result value to $10^{-16}$ precision while the result precision is only about $0.16$ for the case of $1/(1 + 0.6 \pm 0.06)$.
\begin{equation}
\label{eqn: value precision}
2 \xi(\tau) = \epsilon = 2 - 2 \xi(\frac{1}{2} + \sigma):\eqspace \tau \simeq 1\times 10^{-7};
\end{equation}
Because variance arithmetic allows statistical bounding leakage of $\epsilon = 2.33 \times 10^{-7}$, the lax on the result value should be likewise statistically, with the required precision of the result value $\tau$ given by Formula \eqref{eqn: value precision}, in which $\xi(z)$ is the cumulative density function for normal distribution \cite{Probability_Statistics}.
For imprecise input, an expansion should also terminate when the absolute reminder is less than $\tau \delta y$ in which $\delta y$ is the result uncertainty.
...
\end{tabular}
\captionof{table}{
The rounding pattern of positive integers when the are repeatedly divided by $2$ and after each division the result is rounded to the nearest, with each row containing such a process from left to right, and with each cell in the table contains an integer $P$ with its guard sign bit $g$, as $P^{g}$.
$g$ indicates the sign of the current rounding error, to achieve round to nearest in each step, such as $7^+/2 \rightarrow 4^-$ and $7^-/2 \rightarrow 3^+$.
}
...
The x-axis is for index frequency.
The y-axis is for uncertainty and absolute value errors.
}
...
\captionof{figure}{
The error deviations and means for $\sin(x)^2 + \cos(x)^2 - 1, x \in [0, \pi/4]$, for different regression order as shown by the x-axis.
...
The y-axis on the left is for normalized errors, while the y-axis on the right is for value errors.
}
