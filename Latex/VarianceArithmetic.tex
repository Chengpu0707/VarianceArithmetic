...
\end{align}
When the underlying distribution is Normal, Formula \eqref{eqn: Gaussian theoretical bounding leakage} and \eqref{eqn: Gaussian experimental bounding leakage} give the distributional bounding leakage $\epsilon(\kappa)$ and the sample bounding leakage $\epsilon(\kappa_s, N)$ respectively, where $\xi()$ is the Gaussian error function \cite{Probability_Statistics}.
...
\begin{itemize}
\item
When the underlying uncertainty distribution is Gaussian, Figure \ref{fig: Normal_Function} shows that the resulting variance $\delta^2 f$ for a selected functions rise with $N$ until reaching the corresponding stable values $\widehat{\delta^2} f$ when $N \geq 20$.
...
\item
When the underlying uncertainty distribution is Uniform, Figure \ref{fig: Uniform_Function} shows the same trend, however the stable variances are reached only when $N > 10^5$.
...
Statistical Taylor expansion outputs ideal uncertainty, ideal uncertainty bias, and ideal ratio.
