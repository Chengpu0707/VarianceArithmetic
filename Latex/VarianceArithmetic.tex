...
\subsubsection{Termination of Taylor Expansion}

Both Formula \eqref{eqn: Taylor 1d mean} and \eqref{eqn: Taylor 1d variance} are subject to numerical errors and rounding errors such as from $f^{(j)}_x$, $\zeta(2j)$ and $(\delta x)^{2j}$, so both needs to be calculated using the variance arithmetic as well.
\begin{itemize}
...
\item The conventional floating-point representation has limited bit resolution.
If the new terms for both formula start to be smaller than the corresponding least significant values, the sums can be stopped.
...
\subsubsection{Two-dimensional Expansion}
