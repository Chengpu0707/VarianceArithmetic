...
The limited range of $\tilde{x} \in (-\sigma \delta x, +\sigma \delta x)$ cause a bounding leakage $\epsilon$ according to Formula \eqref{eqn: bounding leakage}.
...
It shows that $\lambda$ is always slightly smaller than $\sigma$, as expected.
It also shows that when $5 \leq \sigma$, $M(\sigma, 2n) \simeq (2n-1)!!$ when $2n < 20$.
...
Thus, $\sigma = 5$ for variance arithmetic.
...
For $y = x^3$, when $P(x)$ is more than the applicable precision threshold, the infinitive peak at $\tilde{y} = 0$, as show in Figure \ref{fig: Cubic_Distribution}, makes Formula \eqref{eqn: power precision} diverges.
On the other hand, for $y = \sqrt[3]{x}$, as show in Figure \ref{fig: Cubic_Root_Distribution}, the integration range in Formula \eqref{eqn: variance momentum} can be reduced to $1/P(x)$ for $\sqrt[3]{x \pm \delta x}$ when $1/\sigma < P(x)$ except for $\sqrt[3]{0 \pm \delta x}$.
The reduction of the integration range in Formula \eqref{eqn: variance momentum} also means larger bounding leakage according to Formula \eqref{eqn: bounding leakage}.
