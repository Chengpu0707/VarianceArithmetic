...
The existence of ideal coverage is a necessary verification if the variance arithmetic correctly tracks the result uncertainty.
...
For a $n$-by-$n$ square matrix $\M$ with the element $x_{i,j}, i,j=1,2\dots n$, let its determinant be defined as Formula \eqref{eqn: determinant} \cite{Linear_Algebra} and let the sub-determinant at index $(i, j)$ be defined as Formula \eqref{eqn: sub-determinant} \cite{Linear_Algebra}.
$M_{i,j}$ is the matrix that results from deleting the row $i$ and column $j$ of $M$ \cite{Linear_Algebra}.  
...
\overline{|\M|} &= - 3 x (\delta x)^2 - 3 y (\delta y)^2 - 3 z (\delta z)^2; \\
...
\delta^2 |\M| &= 3 (5 x^4 - 12 x^2 y z + 2 x y^3 + 2 x z^3 + 3 y^2 z^2)(\delta x)^2 \\
    ...
    &+ 15 (\delta x)^6 + 15 (\delta x)^6 + 15 (\delta x)^6 \nonumber;
\end{align}
...
\captionof{figure}{
The result uncertainty means vs. input uncertainties and matrix sizefor the difference between $\widehat{\M}^A$ and $\M^A$.
The input uncertainties run from $10^{-17}$ to $10^{-1}$, while the matrix size runs from $4$ to $8$.
}
...
\captionof{figure}{
The result error deviations vs. input uncertainties and matrix size for the difference between $\widehat{\M}^A$ and $\M^A$.
The input uncertainties run from $10^{-17}$ to $10^{-1}$, while the matrix size runs from $4$ to $8$.
}
...
\captionof{figure}{
The result error deviations vs. input uncertainties and matrix size for the difference of Formula \eqref{eqn: adjugate matrix}.
The input uncertainties run from $10^{-17}$ to $10^{-1}$, while the matrix size runs from $4$ to $8$.
}
...
\captionof{figure}{
The result error deviations vs. input uncertainties and matrix size for the difference of Formula \eqref{eqn: roundtrip}.
The input uncertainties run from $10^{-17}$ to $10^{-1}$, while the matrix size runs from $4$ to $8$.
}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Adjugate_Histogram_Size_6.png} 
\captionof{figure}{
The result histograms for the normalized errors for the same matrix size $6$ different input uncertainties as shown in the legend.
$\M$ is generated by random integers in the range of $[-2^8, +2^8]$.
}
\label{fig: Adjugate_Histogram_Size_6}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Adjugate_Histogram_Noise_0.png} 
\captionof{figure}{
The result histograms for the normalized errors for different matrix size and the input uncertainty $0$ as shown in the legend.
$\M$ is generated by random integers in the range of $[-2^{11}, +2^{11}]$.
}
\label{fig: Adjugate_Histogram_Noise_0}
\end{figure}
...
Formula \eqref{eqn: adjugate matrix} and \eqref{eqn: roundtrip matrix} show the relation of $\M^A$ and $\M$, which only involve addition, subtraction and multiplication.
\begin{align}
...
\begin{enumerate}
\item A matrix $\M$ is constructed using random integers uniformly distributed in the range of $[-2^8, +2^8]$.
...
The variance arithmetic is used to calculate $|\widehat{\M}|$, $\widehat{\M}^A$, and $|\widehat{\M}^A|$.

\item The difference between $|\widehat{\M}|$ and $\M^A$ defines the \emph{Adjugate} Test.
Figure \ref{fig: Adjugate_Uncertainty_vs_Size_Noise} shows that the result uncertainties increase exponentially with both the input noises and the matrix size, while Figure \ref{fig: Adjugate_Error_vs_Size_Noise} shows that the ideal coverage is achieved for the input noises greater than $10^{-16}$.

\item Formula \eqref{eqn: adjugate matrix} defines the \emph{Forward} Test.
Figure \ref{fig: Forward_Error_vs_Size_Noise} shows that Formula \eqref{eqn: adjugate matrix} is strictly observed for idea coverage because the difference is delta-distributed with zero distribution deviation.

\item Formula \eqref{eqn: roundtrip  matrix} defines the \emph{Roundtrip} Test.
Figure \ref{fig: Roundtrip_Error_vs_Size_Noise} shows that Formula \eqref{eqn: roundtrip matrix} is strictly observed for idea coverage because the difference is delta-distributed with zero distribution deviation.

\end{enumerate}

The significand of the conventional floating-point representation \cite{Floating_Point_Standard} has $53$-bit resolution, or $\epsilon = 10^{-16}$.
Adding noise at or below this level will not change the value of a conventional floating-point value, which is the lower bound for the ideal coverage.
Figure \ref{fig: Adjugate_Histogram_Size_6} shows that the histograms of the normalized errors are normal distributed in ideal coverage.
Below the lower bound for idea coverage, floating-point rounding errors dominate the result uncertainties.
When $\M$ is generated by random integers in the range of $[-2^8, +2^8]$, for the input uncertainty $10^{-17}$:
\begin{itemize}
\item In Figure \ref{fig: Adjugate_Histogram_Size_6}, the adjugate matrix calculation involves about $8 \times 5 = 40$ significand bits for a matrix size $6$, so there is no rounding errors.

\item The histogram for matrix size $7$ is very similar to Figure \ref{fig: Adjugate_Histogram_Size_6}, because the adjugate matrix calculation involves about $8 \times 6 = 48$ significand bits.

\item The histogram for matrix size $8$ is quite different to Figure \ref{fig: Adjugate_Histogram_Size_6}, because the adjugate matrix calculation involves about $8 \times 6 = 56$ significand bits.
\end{itemize}
In Figure \ref{fig: Adjugate_Histogram_Noise_0}, when $\M$ is generated by random integers in the range of $[-2^{11}, +2^{11}]$ without added noises:
\begin{itemize}
\item For matrix size $5$, $11 \times 4 = 44$ significand bits are involved, so there is no rounding errors.

\item For matrix size $6$, $11 \times 5 = 55$ significand bits are involved, so there starts to have rounding errors but the overall distribution is still delta-like.

\item For matrix size $7$, $11 \times 6 = 66$ significand bits are involved, so there are significant rounding errors, and the distribution is no longer delta-like.

\end{itemize}
As shown in Figure \ref{fig: Adjugate_Error_vs_Size_Noise}, \ref{fig: Forward_Error_vs_Size_Noise}, and \ref{fig: Roundtrip_Error_vs_Size_Noise}, the variance arithmetic can only provide proper coverage for the conventional floating-point rounding errors.

For both ideal coverages and proper coverages, the result uncertainties provided by the variance arithmetic trace the result value errors closely, even in the huge range of the value errors as suggested by Figure \ref{fig: Adjugate_Uncertainty_vs_Size_Noise}.
This validates using variance arithmetic to calculate adjugate matrix.


\subsection{Matrix Stability}

\begin{align}
\label{eqn: inverse matrix}
\M^{-1} &\equiv \M^A / |\M|; \\
\label{eqn: matrix condition number}
\left| |\M^{-1}| |\M| \right| &= \frac{|\M^A|}{|\M|^{n-1}};
\end{align}
When all values in $\M$ are precise:
\begin{itemize}
\item The inverse matrix \cite{Linear_Algebra} $\M^{-1}$ defined by Formula \eqref{eqn: inverse matrix} is functionally equivalent to $\M^A$.

\item Formula \eqref{eqn: determinant sum 1} contains different paths, all of which are equivalent.
\end{itemize} 
When the rounding errors of the conventional floating-point arithmetic are no longer ignored, Formula \eqref{eqn: inverse matrix} is avoided, and the different paths of Formula \eqref{eqn: determinant sum 1} is explored to calculate $\M^{-1}$ in the Gaussian elimination algorithm \cite{Numerical_Recipes}.
When the input uncertainties are no longer ignored, the stability of $\M^{-1}$ is measured by the conditional number \cite{Linear_Algebra} as defined in Formula \eqref{eqn: matrix condition number}.
In contrast, the variance arithmetic provides direct quantification of uncertainty, so that the effect of $|\M|$ in Formula \eqref{eqn: inverse matrix} to $\M^{-1}$ can be qualified by the precision of $|\M|$ using Formula \eqref{eqn: inversion precision}.
...
However, when $|\M|$ is imprecise, it introduce globally uncertainties to $\M^{-1}$, and the stability of $\M^{-1}$ is measured by the conditional number  \cite{Linear_Algebra} as defined in Formula \eqref{eqn: matrix condition number}.

When using Formula \eqref{eqn: determinant sum 1} to calculate determinant in conventional floating-point arithmetic:
\begin{itemize}
\item The input uncertainties and the calculation rounding errors can not be accounted for in the results.
...
When calculating the inverse matrix, such dependency is so strong that Formula \eqref{eqn: inverse matrix} is avoided, and the path which is deemed to have smallest rounding error is chosen in the Gaussian elimination algorithm \cite{Numerical_Recipes}.
Even so, the result uncertainty is not clear.
\end{itemize}
In contrast, Formula \eqref{eqn: determinant variance} is no longer path dependent, and it accounts for all input uncertainties and calculation rounding errors.

A matrix $\tilde{M}$ is constructed from $M$ in two different ways:
\begin{itemize}
\item \emph{rounding}: If each element of $M$ is converted to a conventional 64-bit floating-point value to construct $\tilde{M}$, the result errors are due to the rounding errors of the conventional 64-bit floating-point calculations.

\item \emph{noise}: If either Gaussian noise or white noise of a particular level is added to each element of $M$ to construct $\tilde{M}$, the result errors are mainly due to the input noise unless the input noise level is $10^{-16}$ or less.
...
The calculation results using $\tilde{M}$ can be compared with those of $M$ in different tests:
\begin{itemize}
\item \emph{determinant}: to compare $|\M|$ and $|\tilde{M}|$.

\item \emph{adjugate}: to compare $M^A$ and $\tilde{M}^A$.

\item \emph{inverse}: to compare $M^A M = |\M| \I$ and $\tilde{M}^A \tilde{M}$, in which $\I$ is the identity matrix \cite{Linear_Algebra} for $M$.
\end{itemize}
