...
When the inputs obeys the \emph{uncorrelated uncertainty assumption}, in the intermediate steps, variance arithmetic uses statistics to account for correlation between uncertainties through their associated variables, such as $x \pm \delta x$ and $y \pm \delta y$.
Variance arithmetic has no dependency problem for analytic questions, which is different from the claim of its predecessor \cite{Prev_Precision_Arithmetic}.
...
For example, Table 1 shows the dependency problem when applying Formula \eqref{eqn: addition and subtraction}, \eqref{eqn: multiplication}, and \eqref{eqn: square precision} as separated and independent arithmetic operations when the input variance is $(\delta x)^2$.
\begin{table}
\label{eqn: dependency example}
\centering
\begin{tabular}{|l|l|l|} 
\hline 
Formula & Result Variance & Over-estimation \\ 
\hline 
\eqref{eqn: interval depend 1} & $4 x^2 (\delta x)^2 + 2 (\delta x)^4$ & $0$ \\
\hline 
\eqref{eqn: interval depend 2} & $(4 x^2 + 1) (\delta x)^2 + 2 (\delta x)^4$ & $+(\delta x)^2$ \\
\hline 
\eqref{eqn: interval depend 3} & $(2 x^2 - 2 x + 1) (\delta x)^2 + (\delta x)^4$ & $-(2 x^2 - 2x + 1)(\delta x)^2 - (\delta x)^4$ \\
\hline 
\end{tabular}
