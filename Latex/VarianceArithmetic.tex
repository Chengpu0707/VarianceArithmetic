...
\begin{align*}
\zeta(n, \kappa) &= \lim_{\tau \rightarrow +\infty} \left( \int^{\tau} \tilde{z}^{n} \rho(\tilde{z}) d \tilde{z} 
		- \int_{\kappa}^{\tau} \tilde{z}^{n} \rho(\tilde{z}) d \tilde{z} \right); \\
	&= \rho(\kappa) \frac{\kappa^{n+1}}{n+1} + 
			\lim_{\tau \rightarrow +\infty} \left( \int^{\tau} \tilde{z}^{n} \rho(\tilde{z}) d \tilde{z} - \frac{\tau^{n+1}}{n+1} \rho(\tau)
				+ \int_{\kappa}^{\tau} \frac{\tilde{z}^{n + 1}}{n + 1} d \rho(\tilde{z}) \right); \\
\frac{d}{d \kappa} \zeta(n, \kappa) &= \rho(\kappa) \kappa^n;
\end{align*}
...
Empirically, Formula \eqref{eqn: symmetric bound moment asymptote} is confirmed by Formula \eqref{eqn: Gaussian moment}, \eqref{eqn: uniform moment}, and \eqref{eqn: Laplace moment}, and Formula \eqref{eqn: bound moment asymptote} is confirmed by Formula \eqref{eqn: exponential moment} and \eqref{eqn: Gamma asymptotic moment}.
\begin{align}
...
The necessary and sufficient conditions for Formula \eqref{eqn: bound moment asymptote} need to be found, to extend the applicable types of distributions for the statistical Taylor expansion from the most common ones as enumerated in Section \ref{sec: statistical Taylor expansion}.
