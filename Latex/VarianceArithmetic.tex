...
In Formula \eqref{eqn: function distribution}, the same distribution can be expressed in either $\tilde{x}$ or $\tilde{y}$, which are two different representations of the same underlying random variable \cite{Probability_Statistics}.
...
However, normal scientific and engineering calculations usually do not care about result distribution, but just some simple statistics of the result, such as the result deviation.
...
\int_{-\sigma}^{+\sigma} \tilde{z}^{2n} N(\tilde{z}) d \tilde{z} &= 2 N(\sigma) \sigma^{2n} \sum_{j=1}^{\infty} \sigma^{2j-1} \frac{(2n-1)!!}{(2j + 2n-1)!!};
\end{align*}

\begin{align*}
& 0 \le x + \title{x} = x + \title{z} \delta x \Rightarrow - \frac{x}{\delta x} = \sigma < \tilde{z} \\
\int_{-\sigma}^{+\infty} \tilde{z}^{2n+1} N(\tilde{z}) d \tilde{z} 
 &= (2n)!! \;N(\sigma) \sum_{j=0}^{n} \frac{\sigma^{2j}}{(2j)!!} \equiv (2n)!! \varsigma(\sigma, 2n+1); \\
\int_{-\sigma}^{+\infty} \tilde{z}^{2n+2} N(\tilde{z}) d \tilde{z} 
 &= (2n+1)!! \left(\xi(\sigma) - N(\sigma) \sum_{j=0}^{n} \frac{\sigma^{2j+1}}{(2j+1)!!} \right) \equiv (2n+1)!! \varsigma(\sigma, 2n+2); \\
\varsigma(\sigma, 2n+1) 
 &= N(\sigma) \left( e^{\frac{x^2}{2}} - \sum_{j=n+1}^{\infty} \frac{\sigma^{2j}}{(2j)!!} \right) 
 = \frac{1}{\sqrt{2 \pi}} - N(\sigma) \sum_{j=n+1}^{\infty} \frac{\sigma^{2j}}{(2j)!!}; \\
\varsigma(\sigma, 2n+2) 
 &= \xi(\sigma) - N(\sigma)\left(\sqrt{\frac{\pi}{2}} e^{\frac{\sigma^2}{2}} (2 \xi(\sigma) - 1) - \sum_{j=n+1}^{\infty} \frac{\sigma^{2j+1}}{(2j+1)!!} \right)
 = \frac{1}{2} + N(\sigma) \sum_{j=n+1}^{\infty} \frac{\sigma^{2j+1}}{(2j+1)!!}; \\
3 < \sigma < \sqrt{n}:& \eqspace \varsigma(\sigma, 2n+1) = \frac{1}{\sqrt{2 \pi}}; \eqspace \varsigma(\sigma, 2n+2) = \frac{1}{2}; \\
n \rightarrow +\infty:& \eqspace (2n)! \varsigma(\sigma, 2n+1) = (2n + 1)!! \varsigma(\sigma, 2n+2)
  = \sqrt{\frac{n}{2}} \left(\frac{n}{e}\right)^{\frac{n}{2}};
\end{align*}

\begin{align*} 
\int_{+\sigma}^{+\infty} \tilde{z}^{2n + 2} N(\tilde{z}) d \tilde{z} 
 &= (2n+1)!! - (2n+1)!! \left(\xi(\sigma) - N(\sigma) \sum_{j=0}^{n} \frac{\sigma^{2j + 1}}{(2j + 1)!!} \right) \\
 &= (2n+1)!! - \frac{1}{2} (2n+1)!! - (2n+1)!! N(\sigma) \sum_{j=0}^{n} \frac{\sigma^{2j + 1}}{(2j + 1)!!}; \\
\int_{-\sigma}^{+\infty} \tilde{z}^{2n+2} N(\tilde{z}) d \tilde{z} 
 &= \frac{1}{2} (2n+1)!! + (2n+1)!! N(\sigma) \sum_{j=0}^{n} \frac{\sigma^{2j + 1}}{(2j + 1)!!}; \\
\int_{-\sigma}^{+\sigma} \tilde{z}^{2n + 2} N(\tilde{z}) d \tilde{z} 
 &= \int_{-\sigma}^{+\infty} \tilde{z}^{2n + 2} N(\tilde{z}) d \tilde{z} - \int_{+\sigma}^{+\infty} \tilde{z}^{2n + 2} N(\tilde{z}) d \tilde{z} \\
 &=  2 N(\sigma) \sum_{j=n+1}^{\infty} \sigma^{2j+1} \frac{(2n + 1)!!}{(2j+1)!!}
 = 2 N(\sigma) \sigma^{2(n+1)} \sum_{j=1}^{\infty} \sigma^{2j-1} \frac{(2n + 1)!!}{(2j+2n+1)!!}; \\
M(x, 2n) &= x^{2n} 2 N(\sigma) \sum_{j=1}^{\infty} \sigma^{2j-1} \frac{(2n - 1)!!}{(2j+2n-1)!!} \equiv x^{2n} \zeta(2n); \\
\zeta(2n) &= 2 N(\sigma) \frac{\sigma}{2n + 1} + 2 N(\sigma) \frac{\sigma^2}{2n + 1} \sum_{j=1}^{\infty} \sigma^{2j-1} \frac{(2n+2 - 1)!!}{(2j+2n+2-1)!!} \\
 &=2 N(\sigma) \frac{\sigma}{2n + 1} + \frac{\sigma^2}{2n + 1} \varsigma(\sigma, 2n + 2);
\end{align*}

\begin{align*}
\frac{\widehat{e^x}}{e^x} &= \sum_{n=1}^{\infty} (\delta x)^{2n} \zeta(2n) \frac{1}{(2n)!} \\
 &\simeq \frac{1}{2} (\delta x)^2 + \frac{1}{24} \zeta(4) (\delta x)^4 + \frac{1}{720} \zeta(6) (\delta x)^6; \\
P(e^x)^2 &= \sum_{n=1}^{\infty} (\delta x)^{2n} \zeta(2n) \sum_{j=1}^{2n-1} \frac{(2n - 1)!!}{j!\;(2n - j)!} - \left( \frac{\widehat{e^x}}{e^x} \right)^2 \\
 &\simeq (\delta x)^2 + (\frac{7}{12} \zeta(4) - \frac{1}{4}) (\delta x)^4 
  + (\frac{31}{360} \zeta(6) - \frac{1}{24} \zeta(4)) (\delta x)^6
  + (\frac{127}{20160} \widehat{\varsigma(\sigma, 8)} - \frac{1}{720} \zeta(6) - \frac{1}{24^2}) \zeta(4)^2) (\delta x)^8;
\end{align*}

\begin{align*}
\widehat{\ln(x)} &= -\sum_{n=1}^{+\infty} \zeta(2n) (\delta x)^{2n} \frac{1}{2n} 
 \simeq - \frac{1}{2} (\delta x)^2 - \frac{1}{4} \zeta(4) (\delta x)^4 - \frac{1}{6} \zeta(6) (\delta x)^6; \\
...
\delta^2 \ln(x) &= \sum_{n=1}^{+\infty} \zeta(2n) (\delta x)^{2n} \frac{1}{n} \sum_{j=1}^{2n-1} \frac{1}{j} - \left(\frac{\widehat{x^c}}{x^c}\right)^2 \\
 &\simeq (\delta x)^2 + (\frac{1}{2} \frac{11}{12} \zeta(4) - \frac{1}{4}) (\delta x)^4 
  + (\frac{1}{3} \frac{137}{60} \zeta(6) - \frac{1}{4} \zeta(4)) (\delta x)^6 \\
 &\simeq (\delta x)^2 + (\frac{11}{24} \zeta(4) - \frac{1}{4}) (\delta x)^4 
  + (\frac{137}{180} \zeta(6) - \frac{1}{4} \zeta(4)) (\delta x)^6;
\end{align*}

\begin{align*}
(1 + p)^2 - 1 &= 2p + 1p^2; \\
\frac{\widehat{x^2}}{x^2} &= P(x)^2; \\
P(x^2)^2 &= 4 P(x)^2 + (\zeta(4) - 1) P(x)^4; \\
\sqrt{1 + p} - 1 &\simeq \frac{1}{2} p - \frac{1}{8} p^2 + \frac{1}{16} p^3 - \frac{5}{128} p^4 + \frac{7}{256} p^5 - \frac{21}{1024} p^6; \\
\frac{\widehat{\sqrt{x}}}{\sqrt{x}} &\simeq - \frac{1}{8} P(x)^2 - \frac{5}{128} \zeta(4) P(x)^4
 - \frac{21}{1024} \zeta(6) P(x)^6; \\
P(\sqrt{x})^2 &\simeq \frac{1}{4} \widehat{\delta x}^2 P(x)^2
  + ((\frac{1}{16} + \frac{1}{64}) \zeta(4) - \frac{1}{64}) P(x)^4 +
  ((\frac{7}{256} + \frac{5}{512} + \frac{1}{256}) \zeta(6) - \frac{5}{512} \zeta(4)) P(x)^6 \\
 &= \frac{1}{4} \widehat{\delta x}^2 P(x)^2 
  + \frac{1}{64} (\zeta(4) - 1) P(x)^4
  + \frac{1}{512} (21 \zeta(6) - 5 \zeta(4)) P(x)^6; \\
\frac{1}{1 + p} - 1 &\simeq -p + p^2 - p^3 + p^4 - p^5 + p^6; \\
\frac{\widehat{1/x}}{1/x} &\simeq P(x)^2 + \zeta(4) P(x)^4 + \zeta(6) P(x)^6; \\
P(1/x)^2 &\simeq P(x)^2 + (3 \zeta(4) - 1) P(x)^4 + (5 \zeta(6) - 2 \zeta(4)) P(x)^6; 
\end{align*}
...
If $\tilde{z}$ in Formula \eqref{eqn: Taylor 1d} is normal-distributed according to Formula \eqref{eqn: Gaussian function distribution}, Formula \eqref{eqn: calc bias} and \eqref{eqn: Taylor 1d variance} gives the mean $\overline{f(x)}$ and the variance $\delta^2 f(x)$, respectively, in which $M(x, n)$ is the \emph{variance momentum} defined by Formula \eqref{eqn: variance momentum}, and $\sigma$ is the \emph{bounding factor} that determines the $\tilde{x}$ range to be $(-\sigma \delta x, +\sigma \delta x)$.
\begin{align}
\label{eqn: Taylor 1d} 
\tilde{y} &= f(x + \tilde{z} \delta x) = \sum_{n=0}^{\infty} \frac{f^{(n)}_x}{n!} \tilde{z}^n (\delta x)^n; \\
...
The choice of the bounding factor $\sigma$ needs careful scrutiny.
If $\sigma = \infty$, $M(x, 2n) = (2n - 1)!!$, which may cause Formula \eqref{eqn: calc bias} and \eqref{eqn: Taylor 1d variance} to diverge in most cases.
For the limited $x$, the behavior of $\lim_{\tilde{x} \rightarrow \infty} f(x + \tilde{x})$ is not of primary concern, so that $\sigma$ is limited.
When $\sigma$ is limited, Formula \eqref{eqn: momentum even} for $M(x, 2n)$ contains a \emph{variance momentum factor} $\zeta(2n)$ which is defined in Formula \eqref{eqn: momentum factor}.
\begin{align}
\label{eqn: momentum factor} 
\zeta(2n) &\equiv2 N(\sigma) \sigma^{2n} \sum_{j=1}^{\infty} \sigma^{2j-1} \frac{(2n - 1)!!}{(2n-1 + 2j)!!}; \\
\label{eqn: momentum even} 
M(x, 2n) &= (\delta x)^{2n} \zeta(2n); 
\end{align}

Figure \ref{fig: Momentum_Factor_Bounding} shows that the variance momentum factor $\zeta(2n)$ approaches $(2n-1)!!$ with increasing Taylor expansion order $2n$, but for a larger $2n$, $\zeta(2n)$ reaches $(2n-1)!!$ at a larger bounding factor $\sigma$. 
When $\sigma^2 \gg 2n$, $\zeta(2n)$ increases with $2n$ as $(2n-1)!!$ which is faster than $\sigma^{2n}$.
When $\sigma^2 \ll 2n$, before $\zeta(2n)$ reaches $(2n-1)!!$, $\zeta(2n)$ increases with $2n$ as $\sigma^{2n} /2n$ which is slower than $\sigma^{2n}$.
Thus it is possible to choose $\sigma$ so that $\zeta(2n) \sim \sigma^{2n}$.

Figure \ref{fig: Momentum_Factor_Expansion} shows that when $3 \leq \sigma \leq 6$, $\zeta(2n) \sim \lambda^{2n}$ by exponential fitting of $\zeta(2n)$.
Table 1 contains the $\lambda$ and $\zeta(2)$ for different $\sigma$, in which the fitted value $\lambda$ is always slightly smaller than $\sigma$, which is expected.

\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Momentum_Factor_Bounding.png} 
\captionof{figure}{
The momentum factor $\zeta(2n)$ in Formula \eqref{eqn: momentum even} vs the Taylor expansion order $2n$ according to Formula \eqref{eqn: momentum factor}, for different $\sigma$ as shown in the legend. 
}
\label{fig: Momentum_Factor_Bounding}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Momentum_Factor_Expansion.png} 
\captionof{figure}{
The momentum factor $\zeta(2n)$ in Formula \eqref{eqn: momentum even} vs the bounding factor $\sigma$ according to Formula \eqref{eqn: momentum factor}, for different Taylor expansion order $2n$ as shown in the legend. 
Also shown is the $(2n - 1)!!$ vs $2n$.
}
\label{fig: Momentum_Factor_Expansion}
\end{figure}

The limited range of $\tilde{x}$ cause the bounding leakage $\epsilon = 2 - 2 \xi(\frac{1}{2} + \sigma)$.
Table 1 shows $\epsilon$ vs $\sigma$.
...
Formula \eqref{eqn: exp bias} and \eqref{eqn: exp precision} give the result for $e^x$:
\begin{align}
...
\frac{\widehat{e^x}}{e^x} &= \sum_{n=1}^{\infty} \zeta(2n) (\delta x)^{2n} \frac{1}{(2n)!} \\
 &\simeq \frac{1}{2} (\delta x)^2 + \frac{1}{8} (\delta x)^4 + \frac{1}{48} (\delta x)^; \\
...
P(e^x)^2 &= \sum_{n=1}^{\infty} \zeta(2n) (\delta x)^{2n} \sum_{j=1}^{2n-1} \frac{1}{j!\;(2n - j)!} - \left( \frac{\widehat{e^x}}{e^x} \right)^2 \\
 &\simeq (\delta x)^2 + \frac{3}{2} (\delta x)^4 + \frac{7}{6} (\delta x)^6;
\end{align}
...
\widehat{\ln(x)} &= -\sum_{n=1}^{+\infty} \zeta(2n) P(x)^{2n} \frac{1}{2n} \\
 &\simeq - \frac{1}{2} P(x)^2 - \frac{3}{4} P(x)^4 - \frac{5}{2} P(x)^6; \\
...
\delta^2 \ln(x) &= \sum_{n=1}^{+\infty} \zeta(2n) P(x)^{2n} \frac{1}{n} \sum_{j=1}^{2n-1} \frac{1}{j} - \left(\frac{\widehat{x^c}}{x^c}\right)^2 \\
 &\simeq P(x)^2 + \frac{9}{8} P(x)^4 + \frac{32}{3} P(x)^6; 
\end{align}
...
\end{align}
As the special cases for Formula \eqref{eqn: power bias} and \eqref{eqn: power precision}, Formula \eqref{eqn: square bias} and \eqref{eqn: square precision} give the result for $x^2$, Formula \eqref{eqn: square root bias} and \eqref{eqn: square root precision} give the result for $\sqrt{x}$, and Formula \eqref{eqn: inversion bias} and \eqref{eqn: inversion precision} give the result for $1/x$: 
\begin{align}
\label{eqn: square bias}
\frac{\widehat{x^2}}{x^2} &= P(x)^2; \\
\label{eqn: square precision}
P(x^2)^2 &= 4 P(x)^2 + 2 P(x)^4; \\
\label{eqn: square root bias}
\frac{\widehat{\sqrt{x}}}{\sqrt{x}} &\simeq - \frac{1}{8} P(x)^2 - \frac{15}{128} P(x)^4 - \frac{315}{1024} P(x)^6; \\
\label{eqn: square root precision}
P(\sqrt{x})^2 &\simeq \frac{1}{4} P(x)^2 + \frac{7}{32} P(x)^4 + \frac{75}{128} P(x)^6; \\
\label{eqn: inversion bias}
\frac{\widehat{x^c}}{x^c} &= \sum_{n=1}^{\infty} \zeta(2n) P(x)^{2n} \begin{pmatrix} c \\ 2n \end{pmatrix}; \\
\label{eqn: inversion precision}
P(1/x)^2 &\simeq P(x)^2 + 8 P(x)^4 + 69 P(x)^6; 
\end{align}

The variance formula using Taylor expansion shows the nature of the calculation, such as the different nature of variance increases of $e^x$ vs $\ln x$ vs $x^c$, or the difference speed of variance increases of $x^c$ depending on different $c$.
The result for $e^x$ always converges.
For other cases, Formula \eqref{eqn: calc bias} and \eqref{eqn: Taylor 1d variance} are expressed in $P(x)$ instead of $\delta x$.
When $P(x) < 1/\sigma$, Formula \eqref{eqn: calc bias} and \eqref{eqn: Taylor 1d variance} converge.
The $\sigma$ is a default constant in variance arithmetic.
...
For example, if the square of an input imprecise value is mistakenly carried out as multiplication of two different input imprecise values, the result linear deviation is only $\frac{1}{\sqrt{2}}$-fold according to the result precision for $x^2$ Table 1 and \eqref{eqn: multiplication precision}.
...
For example, Formula \eqref{eqn: square root} and \eqref{eqn: root square} shows that the calculation cost of $\sqrt{x}^2$ is greater by 4-fold than that of $\sqrt{x^2}$.
\begin{align}
...
The variance bounding factor $\sigma$ is default to 5.
