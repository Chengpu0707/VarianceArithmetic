...
When $\rho(\tilde{z}) > 0$ and $\frac{d \rho(\tilde{z})}{d \tilde{z}}$ is bounded in $\tilde{z} \in [0, \kappa]$, Formula \eqref{eqn: bound moment asymptote deduction left} and \eqref{eqn: bound moment asymptote deduction right} leads to Formula \eqref{eqn: symmetric bound moment asymptote limit}.
 Formula \eqref{eqn: bound moment asymptote deduction} and \eqref{eqn: symmetric bound moment asymptote limit} conclude Formula \eqref{eqn: symmetric bound moment asymptote} and \eqref{eqn: bound moment asymptote}.
\begin{align}
\label{eqn: bound moment asymptote deduction}
\int_{0}^{\kappa} \tilde{z}^n \rho(\tilde{z}) d \tilde{z} &= \rho(\kappa) \frac{\kappa^{n+1}}{n+1} 
			- \frac{1}{n+1} \int_{0}^{\kappa} \tilde{z}^{n+1} \frac{d \rho(\tilde{z})}{d \tilde{z}} d \tilde{z}; \\
\label{eqn: bound moment asymptote deduction left}
\rho = \min\{ \rho(\tilde{z}) \}&: \tilde{z} \in [0, \kappa] \}:\;\; 
		\int_{0}^{\kappa} \tilde{z}^n \rho(\tilde{z}) d \tilde{z} \geq \rho \frac{\kappa^{n+1}}{n+1}; \\
\label{eqn: bound moment asymptote deduction right}
\tau = \max\{ |\frac{d \rho(\tilde{z})}{d \tilde{z}}|&: \tilde{z} \in [0, \kappa] \}:\;\; 
\frac{1}{n+1} \int_{0}^{\kappa} \tilde{z}^{n+1} |\frac{d \rho(\tilde{z})}{d \tilde{z}}| d \tilde{z} <
		\tau \frac{\kappa^{n+1}}{n+1}\frac{\kappa}{n+2}; \\
\label{eqn: symmetric bound moment asymptote limit}
\lim_{n \rightarrow +\infty}& \int_{0}^{\kappa} \tilde{z}^n \rho(\tilde{z}) d \tilde{z} = \rho(\kappa) \frac{\kappa^{n+1}}{n+1}; \\
\label{eqn: symmetric bound moment asymptote}
|\varrho| = \kappa &:\eqspace \lim_{n \rightarrow +\infty} \zeta(2n, \kappa) = 2 \rho(\kappa) \frac{\kappa^{2n+1}}{2n+1}; \\
\label{eqn: bound moment asymptote}
|\varrho| < \kappa&:\eqspace \lim_{n \rightarrow +\infty} \zeta(n, \kappa) = \rho(\varrho) \frac{\varrho^{n+1}}{n+1} + \rho(\kappa) \frac{\kappa^{n+1}}{n+1} 
		\simeq \rho(\kappa) \frac{\kappa^{n+1}}{n+1};
\end{align}
...
\lim_{n \rightarrow +\infty}& \zeta(2n, \kappa) = 2 N(\kappa) \frac{\kappa^{2n+1}}{2n+1};
\end{align}
...
\zeta(2n) = \int_{-\kappa}^{+\kappa} \tilde{z}^{2n} \frac{1}{2 \sqrt{3}} d \tilde{z} = 2 \rho(\kappa) \frac{\kappa^{2n+1}}{2n + 1}, \;\; 0 \leq \kappa \leq \sqrt{3}; 
\end{equation}
...
For input distribution without bounding, Formula \eqref{eqn: Taylor 1d variance} may not converge, e.g., when the input uncertainty is Laplace \cite{Probability_Statistics} so that $\zeta(2n) = (2n)!$.
Thus, it is necessary that the input uncertainty is bounded.
...
\item Formula \eqref{eqn: log precision} for $\log(x \pm \delta x)$ can be approximated by Formula \eqref{eqn: log convergence} as $n \rightarrow \infty$,
which converges when $P(x) < 1/\kappa$.

\item Formula \eqref{eqn: power precision} for $(x \pm \delta x)^c$ can be approximated by Formula \eqref{eqn: pow convergence} after applying Vandermonde's identity $\sum_{k=0}^{r} \begin{pmatrix} m \\ k \end{pmatrix} \begin{pmatrix} n \\ r - k \end{pmatrix} = \begin{pmatrix} m + n \\ r \end{pmatrix}$ .  
...
\item
When the underlying uncertainty distribution is Uniform, $\kappa_s = \sqrt{3}$, Figure \ref{fig: Uniform_Function} shows the same trend, however the stable variances are reached only when $N > 10^4$.
\end{itemize}
...
These observations suggest that bounding leakage is the reason for $\delta^2 f < \widehat{\delta^2} f$ and it is justified to infer the confidential interval range $\kappa$ from bounding leakage $\epsilon(N)$.
...
The choice of $\kappa_s$ to calculate  $\widehat{\delta^2} f$ and $\alpha$ is defined as the ideal bounding range $\hat{\kappa}$.
For Uniform distribution, by definition $\hat{\kappa} = \sqrt{3}$.
When the input distribution is unbounded, the choice of $\hat{\kappa}$ is a trade-off: a larger $\hat{\kappa}$ means a better reliability for the result but a narrower convergence range for the inputs.
For Gaussian distribution, according to the 5-$\sigma$ rule for determining the statistical significance of experimental result \cite{Statistical_Methods}\cite{Precisions_Physical_Measurements},  $\hat{\kappa} = 5$.

In statistical Taylor expansion, every calculation should be carried out twice:
\begin{enumerate}
\item With the ideal $\zeta(n, \hat{\kappa})$ of each input, to obtain the ideal variances and ideal uncertainty offset.
Ideal variance is consistent with traditional variance \cite{Probability_Statistics}.

\item With the real $\zeta(n, \kappa)$ of each input given the sample count and input uncertainty distribution, to obtain the ideal ratio to quantify the reliability of the ideal variance and ideal uncertainty bias as a value in the range of $[0, 1)$.
\end{enumerate}
...
Variance arithmetic rejects the distributional zero of $\log(x)$ or $(x \pm \delta x)^c$ in the range of $P(x) \lesssim 1/\hat{\kappa}$ statistically due to the divergence of Formula \eqref{eqn: log precision} mathematically, with $\zeta(2n, \kappa)$ providing the connection between these two perspectives.

For Gaussian input uncertainty with $\hat{\kappa} = 5$:
\begin{itemize}
\item 
For $e^{x \pm \delta x}$, $\delta x \lesssim 19.864$ and $(e^{x \pm \delta x}) \lesssim 1681.767$ regardless of $x$.
...
\end{itemize}
For $(x \pm \delta x)^c$, except when $c$ is a natural number, the upper bound $P(x)$ is close to $\hat{\kappa}$ but increasing with $c$. 
This trend is confirmed in Figure \ref{fig: Pow_Conv_Edge} for Gaussian input distribution and Figure \ref{fig: Pow_Conv_Edge_Uniform} for uniform input distribution.
The bounded moment $\zeta(2n, \hat{\kappa})$ for Uniform distribution is much less than that for Normal distribution, such that the expansion contains $652$ terms instead of $448$ terms.
When normalized by the respective $\kappa$, the upper bounds of input uncertainty in Figure \ref{fig: Pow_Conv_Edge} and \ref{fig: Pow_Conv_Edge_Uniform} looks identical, however, the resulting uncertainty biases and uncertainties in Figure \ref{fig: Pow_Conv_Edge} are $3$ order-of-magnitude smaller than those in Figure \ref{fig: Pow_Conv_Edge_Uniform} when the exponent $c$ is less than $0$, showing the effect of input uncertainty distribution.
...
In Figure \ref{fig: Sin_Conv_Edge_Uniform} for Uniform input uncertainty, the input uncertainty upper bound is $4$ times larger than that in Figure \ref{fig: Sin_Conv_Edge} but is still periodic.
...
Figure \ref{fig: Pow_Exp_Dev} reveals that the error deviations of $(x + \delta x)^c$ remain close to $1$ for all input exponents $c$ and input uncertainties $\delta x > 10^{15}$.

Figure \ref{fig: Pow_Exp_Dev} shows a typical desired relation of error deviation versus input noise deviation $\delta x$ and another parameter, with the error deviations always close to $1$ when the result variance of Formula \ref{eqn: Taylor 1d variance} and \ref{eqn: Taylor 2d variance} tracks the corresponding value errors closely, except when $\delta x$ is close to $0$ so that input noise can no longer overpower floating-point rounding errors.
...
For example, Figure \ref{fig: FFT_Cos_Clean_6_6_Wave_Lib} shows that the FFT value error waveform of $\cos(6 \frac{2 \pi}{2^6} j)$ has  much stronger resonant pattern than that of $\sin(3 \frac{2 \pi}{2^6} j)$ in Figure \ref{fig: FFT_Sin_Clean_6_3_Wave_Lib}.
...
Bound moment $\zeta(2n)$ needs to be extended to all probability distributions.
...
The choice of ideal bounding range $\hat{\kappa}$ needs further study to extend to other distributions.
The condition for Formula \eqref{eqn: variance asymptotic} seems to be as generic as the definition in Formula \eqref{eqn: bound moment} which needs further proof.

The convergence of statistical Taylor expansion is quite similar to the re-normalization in quantum field theory: 
The result diverges for a theoretical distribution, but converges after introducing sampling.
This similarity needs further study.
