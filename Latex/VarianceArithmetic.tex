...
\captionof{figure}{
The Taylor expansion for $\frac{1}{1 + x} = \sum_{n = 0}^{N} x^n$, for different Taylor expansion order $N$ as shown in the x-axis, and different $x$ as shown in the legend.
The y-axis shows either the uncertainty and the reminder value error.
}
...
\item In all cases, the reminder value errors reach the corresponding least significant values but they do not vanish.
...
\item For $\frac{1}{1 + |x|}$, the reminder value errors oscillate near the Taylor expansion order, which means difficulty in obtaining a reliable estimator.
On the other hand, the statistical tracking by variance arithmetic using Formula \eqref{eqn: polynomial Taylor} is straight-forward.

\end{itemize}

Using variance arithmetic for Taylor expansion, the result uncertainty tracks the reminder value error properly.
The stability of the uncertainties adds an affirmation for the stability test method to obtain the Taylor expansion order.
