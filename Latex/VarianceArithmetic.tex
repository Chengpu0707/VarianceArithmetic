...
For the limited $x$, the behavior of $\lim_{\tilde{x} \rightarrow \infty} f(x + \tilde{x})$ is not of primary concern.
When $\sigma$ is limited, $M(x, 2n)$ is calculated by Formula \eqref{eqn: momentum even}, which contains a \emph{variance momentum factor} $\zeta(2n)$ defined in Formula \eqref{eqn: momentum factor}.
$\zeta(2n)$ is normalized for $\zeta(2) = 1$ so that $\delta^2 x = (\delta x)^2$ for any choice of $\sigma$.
\begin{align}
