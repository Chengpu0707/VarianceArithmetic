...
It is based on central limit theorem.
...
\item The Taylor expansion in variance arithmetic is now centered around statistical means instead of the calculation results assuming no uncertainty, so variance arithmetic is strict statistically, and when the input uncertainties specify the input error deviation precisely, the result uncertainties specify the result error deviation precisely.
...
When the calculation is too close to a pole or zero, variance arithmetic will reject the result automatically, such as rejecting an inversion near $0$.
...
A \emph{distributional zero} of the uncertainty distribution happens when $f^{(1)}_x=\infty \rightarrow \rho(\tilde{y}, y, \delta y) = 0$, while a \emph{distributional pole} happens when $f^{(1)}_x=0 \rightarrow \rho(\tilde{y}, y, \delta y) = \infty$.
...
Figure \ref{fig: Square_Root_Distribution} shows the probability density function for $\sqrt{x \pm 1}$ according to Formula \eqref{eqn: power distribution}, which has a distributional zero at $x=0$ that separates the defined region $x \ge 0$ vs the undefined region $x < 0$.
...
Figure \ref{fig: Cubic_Root_Distribution} shows the probability density function for $\sqrt[3]{x \pm 1}$ according to Formula \eqref{eqn: power distribution}, which also has a distributional zero at $x=0$ that separates two defined region $x \ge 0$ and $x \leq 0$.
...
Figure \ref{fig: Inverse_Square_Distribution} shows the probability density function for $1/(x \pm 1)^2$ according to Formula \eqref{eqn: power distribution}, which has a distributional zero at $x=0$ that separates two defined region $x > 0$ and $x < 0$.
...
The linear fit equation is: $8.284726600172205\;10^{-7} \times 4.9405914462757465^{2n}$.
}
...
Formula \eqref{eqn: sin precision} gives the result variance for $\sin(x)$:
\begin{align}
...
Formula \eqref{eqn: power precision} gives the normalized variance for $x^c$, in which $\begin{pmatrix} c \\ n \end{pmatrix} = \frac{\prod_{j=0}^{n-1} (c -j)}{n!}$:
\begin{align}
...
\end{align}
As some special cases for Formula \eqref{eqn: power precision}, Formula \eqref{eqn: square precision}, \eqref{eqn: square root precision}, \eqref{eqn: square root precision}, and \eqref{eqn: square root precision} give the normalized variance for $x^2$, $\sqrt{x}$, $1/x$, and $1/x^2$, respectively: 
\begin{align}
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Inversion_on_Edge_Histo.pdf} 
\captionof{figure}{
The histogram for $1/(1 \pm \delta x)$ using Gaussian input noises, with $\delta x$ shown in the legend. 
The histogram has been normalized by the result mean and uncertainty calculated by variance arithmetic, so that $0$ on the x-axis marks the result mean, and $\pm 1$ on the x-axis marks the result uncertainty.
The $y$-axis is scaled as the normalized count of the histogram.
}
\label{fig: Inversion_on_Edge_Histo}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Inversion_on_Edge.pdf} 
\captionof{figure}{
The result of variance arithmetic vs the sampled statistics for $1/(1 \pm 0.2 - \delta x)$, for different $\delta x$ as shown in the x-axis.
In the legend, \textit{value} and \textit{uncertainty} are calculated by variance arithmetic, while \textit{mean} and \textit{deviation} are sampled statistics from $10,000$ floating-point calculation of $1/\tilde{x}$ in which $\tilde{x}$ is Gaussian noise with mean $1$ and deviation $0.2 - \delta x$.
The y-axis on the left compares value vs mean, while the y-axis on the right compares uncertainty vs standard deviation.
}
\label{fig: Inversion_on_Edge}
\end{figure}


When Formula \eqref{eqn: momentum factor higher} contains infinitive expansion terms, the necessary condition for it to converge is $P(x) < 1/\sigma$.
The sufficient $P(x)$ for or Formula \eqref{eqn: power precision} to converge is defined as \emph{applicable precision threshold}, which can be estimated as $1/\sigma$.
Through $\zeta(2n)$, the convergence condition also rejects distributional zero or distributional pole inside the range of $[x - \sigma \delta x, x + \sigma \delta x]$, and guarantees that the underlying uncertainty distribution is Normal-like.

Variance arithmetic may reject a calculation for the following reasons:
\begin{itemize}
\item For Formula \eqref{eqn: Taylor 1d variance} to converge, the contribution from expansion order $2n$ should decrease with increasing $2n$ when $2n$ is sufficiently large \footnote{The lower limit for $2n$ is set empirically as $20$.}. 
Otherwise, the result should be invalidated as \emph{not monotonic}.
Section \ref{sec: Math Library} will describe this mechanism in more detail.
...
If at the limit, the expansion is still not stable, the result should be invalidated as \emph{practical unstable}.
Section \ref{sec: polynomial} will define the stability standard for truncating an expansion.

\end{itemize}

For example, $1/(1 \pm (0.2 - 1.28\;10^{-5}))$ is rejected, while $1/(1 \pm (0.2 -1.29\;10^{-5})) = 1.046 \pm 0.250$.
Figure \ref{fig: Inversion_on_Edge_Histo} shows the histograms for $1/(1 \pm \delta x)$ when $\delta x =0.01, 0.1, 0.2 - 1.29\;10^{-5}$.
When $\delta x =0.01$, the result is normal distributed.
When $\delta x$ approaches $0.2$, the result distribution deviates from normal more and more.
The rejection also keeps the result distribution from deviating too much from Normal distribution, so that the result value and uncertainty do not deviate too much from those of variance arithmetic.
Figure \ref{fig: Inversion_on_Edge} shows that the result of variance arithmetic fits the result from statistical sampling for $1/(1 \pm (0.2 - \delta x))$ reasonably well when $\delta x \rightarrow 0$.
...
When an integer is repeatedly divided by $2$ and after each division the result is rounded to the nearest, the value of the integer before it becomes $0$ is not randomly distributed, e.g., it is always $1$ in the last step, predominantly 2 in the step before, and more randomly distributed in the step before, as shown in Table \ref{tbl: rounding pattern}.
...
At $n=100$, applying Formula \eqref{eqn: polynomial Taylor} to Formula \eqref{eqn: Taylor 1d variance} has $100$ terms from $\sim (\delta x)^2$ to $\sim (\delta x)^{200}$, but only the first $10$ terms can be calculated before $(\delta x)^{2n}$ becomes $0$.
...
\captionof{figure}{
The histogram of the error deviation for $\sin(x \pm \delta x)$, for $x=0, +\pi/2$ and $\delta x = 10^{-7}, 10^{-16}$ as shown in the legend.
...
\begin{figure}[p]
\centering
\includegraphics[height=2.5in]{Pow_Converge.pdf} 
\captionof{figure}{
...
$\log(x)$ has a distributional zero at $x=0$ so that all $log(x \pm \delta x)$ are rejected when $1/5 < P(x)$.
Figure \ref{fig: Log_Dev} shows that the calculated uncertainties using Formula \eqref{eqn: log precision} agree very well with the measured value deviations for $\log(x + \delta x)$, so that the result error deviations are very close to $1$
...
$\sin(x)$ has a distribution pole at $x = \pm \pi/2$.
Figure \ref{fig: Sin_X_Dev} shows that the error deviation for $\sin(x + \delta x)$ is $1$ except when $x=\pm \pi/4$ and $\delta x < 10^{-8}$, which agrees with the expected Delta distribution at $x = \pm \pi/2$ without added noise.
...
\begin{itemize}
\item When $\delta x=10^{-16}$, the distribution is Normal at $x = 0$, while it is Delta at $x = \pm \pi/2$.

\item When $\delta x=10^{-7}$, the distribution is Normal at $x = 0$, while it is a folded Normal at $x = \pm \pi/2$ when the 2nd derivative of $\sin(\pi/2)$ folds all added noises to one side only.

\end{itemize}

Variance arithmetic rejects $\sin(0 \pm \delta x), \delta x \ge \pi/4$ and $\sin(\pi/2 \pm \delta x), \delta x \ge 3\pi/16$ for not monotonic, but can calculate smaller $\delta x$ without a problem.
Because the second derivatives at the distributional pole is not zero, this case may not be generally applicable to calculations near a distributional pole.
...
It shows that $(1 \pm 0.2)^{-2}$ clearly diverges, while $(1 \pm 0.2)^{-1}$ marginally diverges, so that variance arithmetic invalidate both as not monotonic.
...
\item The values of the indexed sine functions are extended from $j = 0,1,\dots N/8$ to $j = 0,1,\dots N$ using the symmetry of $\sin(2\pi j/N)$ and $\cos(2\pi j/N)$.

\item The values of the indexed sine functions are extended from $j = 0,1,\dots N$ to the whole integer region using the periodicity of $\sin(2\pi j/N)$ and $\cos(2\pi j/N)$.
...
Variance arithmetic further reveals that the amount of noise to add small noises is to achieve proper coverage.
...
\begin{itemize}
\item Figure \ref{fig: FFT_Linear_vs_Order_Lib} shows that proper coverage cannot be achieved for reverse transformation for all FFT orders, because the value errors outpace the uncertainties with increasing FFT order.
...
When the input uncertainty is too small compared to the real input error, proper coverage cannot be achieved, which is the case when the $\sin(x)$ numerical errors in Figure \ref{fig: Sin_Diff} is combined with the $\cos(x)/\sin(x)$ numerical errors in Figure \ref{fig: Cot_Diff}.
...
Adding enough noise to the input can overpower the numerical calculation errors, to achieve ideal coverage.
...
Due to digital limitation of 64-bit standard floating-point representation, variance arithmetic can only provide proper coverage for floating-point rounding errors, with the empirical error deviations in the range of $[1/5, 5]$
...
Due to its dependency tracing, variance arithmetic allows no numerical execution freedom, and it has no dependency on such freedoms.
...
How to treat the conceptual difference between variance arithmetic and conventional floating-point arithmetic is a general question.
The uncertainty bias can be potentially significant near a distributional zero or a distributional pole, such as $1/(1 \pm (0.2 -2\;10^{-5})) = 1.046 \pm 0.250$.  
...
A distributional pole may be a problem for variance arithmetic, but it is never a problem in conventional floating-point arithmetic.
...
Variance arithmetic rejects a calculation if its distributional pole or distributional zero is in the range of $[x - \sigma \delta x, x + \sigma \delta x]$ with $\sigma = 5$.
But there are many use cases in which calculations need to be performed at $x$ closer to either a distributional pole or a distributional zero, so variance arithmetic of smaller $\sigma$ may be needed.
...
Such approach needs theoretical foundation in both mathematics and statistics.
...
This limits the effective range for $x \pm \delta x$ to be much smaller than the full range of the standard 64-bit floating-point representation, such as when applying Formula \eqref{eqn: Taylor 1d mean} and \eqref{eqn: Taylor 1d variance} to floating-point rounding errors in Section \ref{sec: polynomial}.
Ideally, $(\delta x)^2$ should be calculated and stored in a different 64-bit floating-point representation with no sign bit, more exponent bits, and less significant bits, because  $(\delta x)^2$ needs not to be as accurate as $x$.
Let the new 
