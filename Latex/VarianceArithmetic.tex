...
Viewed in the $f^{-1}(\tilde{y})$ coordinate, $\rho(\tilde{y}, y, \delta y)$ is Gaussian modulated by $\frac{d\tilde{x}}{d\tilde{y}} = 1/f^{(1)}_x$, so that $\rho(\tilde{y}, y, \delta y) = 0$ when $f^{(1)}_x=\infty$, and $\rho(\tilde{y}, y, \delta y) = \infty$ when $f^{(1)}_x=0$.
\begin{align}
...
Figure \ref{fig: Square_Root_Distribution} shows the probability density function for $\sqrt{x \pm 1}$ according to Formula \eqref{eqn: power distribution}.
...
Figure \ref{fig: Cubic_Root_Distribution} shows the probability density function for $\sqrt[3]{x \pm 1}$ according to Formula \eqref{eqn: power distribution}.
Compared with $\sqrt{x \pm 1}$, $\sqrt[3]{x \pm 1}$ distributes on $\tilde{y} < 0$ also, so that the uncertainty distributions for $\sqrt[3]{0 \pm 1}$ which has two equal peaks..
...
Figure \ref{fig: Cubic_Distribution} shows the probability density function for $(x \pm 1)^3$ according to Formula \eqref{eqn: power distribution}.
...
%If the underlying result distribution for $f(x)$ needs to be Gaussian, $f^{(1)}_x = \infty$ or $f^{(1)}_x = 0$ serves as separation boundaries.
...
\widehat{\sin(x)} &= \sum_{n=1}^{\infty} \sin(x) (-1)^{n} \frac{(\delta x)^{2n}}{(2n)!} \\
...
\delta^2 \sin(x) &=  - \widehat{\sin(x)}^2 + \sum_{n=1}^{\infty} (\delta x)^{2n} \zeta(2n) \\
 ...
 &\simeq \cos(x)^2 (\delta x)^2 + (\frac{1}{2}\sin(x)^2 - \cos(x)^2) (\delta x)^4 - (\frac{1}{3}\sin(x)^2 + \frac{1}{6} \cos(x)^2) (\delta x)^6;
\end{align*}
...
\begin{align*}
\zeta(\sigma, 2n + 2) &= (2n + 1) \zeta(\sigma, 2n) - 2 N(\sigma) \sigma^{2n + 1}; \\ 
\alpha \lambda^{2n+2} &= (2n + 1) \alpha \lambda^{2n} - 2 N(\sigma) \sigma \sigma^{2n}; \\
\lambda^2 &= (2n + 1) - \frac{2 N(\sigma) \sigma}{\alpha} (\frac{\sigma}{\lambda})^{2n} 
 \simeq (2n + 1) - \frac{2 N(\sigma) \sigma}{\alpha}(1 + 2n \frac{\sigma - \lambda}{\lambda}); \\
\frac{2 N(\sigma) \sigma}{\alpha} &= 1 - \lambda^2; \\
\lambda = (1 - \lambda^2) (\sigma - \lambda);
\end{align*}
...
$\zeta(2n)$ is normalized so that $\delta^2 x = (\delta x)^2$ for any choice of $\sigma$.
\begin{align}
&\widehat{\zeta(2n)} = \int_{-\sigma}^{+\sigma} \tilde{z}^n N(\tilde{z}) d \tilde{z}
  = 2 N(\sigma) \sigma^{2n} \sum_{j=1}^{\infty} \sigma^{2j-1} \frac{(2n - 1)!!}{(2n-1 + 2j)!!}; \nonumber \\
\label{eqn: momentum factor} 
\zeta(2n) &\equiv \widehat{\zeta(2n)} / \widehat{\zeta(2)}; \\
...
\end{align}
Formula \eqref{eqn: exp precision} always converges.

Formula \eqref{eqn: sin bias} and \eqref{eqn: sin precision} give the result for $\sin(x)$:
\begin{align}
\label{eqn: sin Taylor}
\sin(x + \tilde{x}) &= \sin(x) + \sum_{n=0}^{\infty} \cos(x) (-1)^n \frac{\tilde{x}^{2n+1}}{(2n + 1)!} + \sin(x) (-1)^{n+1} \frac{\tilde{x}^{2n+2}}{(2n + 2)!}; \\
\label{eqn: sin bias}
\widehat{\sin(x)} &= \sin(x) \sum_{n=1}^{\infty} (-1)^{n} \frac{(\delta x)^{2n}}{(2n)!} \\
 &\simeq  \sin(x) \left( \frac{1}{2}(\delta x)^2 - \frac{1}{24} (\delta x)^4 + \frac{1}{720} (\delta x)^6 \right); \\
...
\delta^2 \sin(x) &= - \widehat{\sin(x)}^2 + \sum_{n=1}^{\infty} (\delta x)^{2n} \zeta(2n) \nonumber \\
 ...
 &\simeq \cos(x)^2 (\delta x)^2 + (\frac{1}{2}\sin(x)^2 - \cos(x)^2) (\delta x)^4 - (\frac{1}{3}\sin(x)^2 + \frac{1}{6} \cos(x)^2) (\delta x)^6;
\end{align}
Formula \eqref{eqn: sin precision} may diverge if $\delta x$ is too large.
...
For each function, the maximal $P(x)$ for Formula \eqref{eqn: Taylor 1d variance} to be applicable is defined as \emph{applicable precision threshold}.
...
For $x^c$, when $P(x)$ is more than the applicable precision threshold, the underlying uncertainty distribution deviates significantly from Gaussian, as show in Figure \ref{fig: Cubic_Distribution} for $x^3$ and Figure \ref{fig: Cubic_Root_Distribution} for $\sqrt[3]{x}$.

For $(x+a)^c$ in which $a$ is another constant, $P(x) = \delta x / |x - a|$ according to Formula \eqref{eqn: power bias} and \eqref{eqn: power precision}.
