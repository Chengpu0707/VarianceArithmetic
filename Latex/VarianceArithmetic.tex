...
\subsection{Bound Moment Symmetry}
...
\subsection{Asymptotic Behavior for Bound Moment}

\iffalse

\begin{align*}
\zeta(n, \kappa) &= \zeta(n, +\infty) - \int_{\kappa}^{+\infty} \tilde{z}^{2n} \rho(\tilde{z}) d \tilde{z}
		=  \rho(\kappa)\frac{\kappa^{n+1}}{n+1} + 
			\lim_{\tau \rightarrow +\infty} \left( \int^{\tau} \tilde{z}^{2n} \rho(\tilde{z}) d \tilde{z} - \frac{\tau^{n+1}}{n+1} \rho(\tau) \right); \\
n \rightarrow +\infty &:\eqspace \frac{\kappa^{n+1}}{n+1} \ll \kappa^{2n}; \\
\kappa \rightarrow +\infty &:\eqspace \rho(\kappa) \simeq \frac{d}{d \kappa} \rho(\kappa); \\
n \rightarrow +\infty, \; \kappa \rightarrow +\infty &: \eqspace
	\frac{d}{d \kappa} \frac{\kappa^{n+1}}{n+1} \rho(\kappa) = \kappa^{2n} \rho(\kappa) + \frac{\kappa^{n+1}}{n+1} \frac{d}{d \kappa} \rho(\kappa)
		\simeq \kappa^{2n} \rho(\kappa) ; \\
\end{align*}

\fi

Formula \eqref{eqn: bound asymptote} and \eqref{eqn: order asymptote} suggests Formula \eqref{eqn: bound moment asymptote}.
Formula \eqref{eqn: symmetric bound moment asymptote} is a special case of Formula \eqref{eqn: bound moment asymptote} for symmetric probability distribution.
Empirically:
\begin{itemize}
\item Formula \eqref{eqn: symmetric bound moment asymptote} is confirmed by Formula \eqref{eqn: Gaussian moment}, \eqref{eqn: uniform moment}, and \eqref{eqn: Laplace moment}.

\item Formula \eqref{eqn: bound moment asymptote} is confirmed by Formula \eqref{eqn: exponential moment} and \eqref{eqn: Gamma asymptotic moment}.
\end{itemize}
\begin{align}
\label{eqn: bound asymptote}
\zeta(n, \kappa) &= \rho(\kappa)\frac{\kappa^{n+1}}{n+1} + 
			\lim_{\tau \rightarrow +\infty} \left( \int^{\tau} \tilde{z}^{n} \rho(\tilde{z}) d \tilde{z} - \frac{\tau^{n+1}}{n+1} \rho(\tau) \right); \\
\label{eqn: order asymptote}
&\lim_{n \rightarrow +\infty} \frac{d}{d \tau} \left( \int^{\tau} \tilde{z}^{n} \rho(\tilde{z}) d \tilde{z} - \frac{\tau^{n+1}}{n+1} \rho(\tau) \right) = 0; \\
\label{eqn: bound moment asymptote}
\kappa \ll n &:\eqspace \zeta(n, \kappa) \simeq \rho(\kappa) \frac{\kappa^{n}}{n}; \\
\label{eqn: symmetric bound moment asymptote}
\rho(-\tilde{z}) = -\rho(\tilde{z}) &:\eqspace \zeta(2n, \kappa) = 2 \rho(\kappa) \frac{\kappa^{2n+1}}{2n+1};
\end{align}
...
\captionof{figure}{
When $\kappa = 5$, the value of bound moment  $\zeta(2n, \kappa)$ (left y-axis) for Gaussian distribution with order $2n$ (x-axis) versus Formula \eqref{eqn: Gaussian asymptotic moment} (left y-axis), and their ratio (right y-axis).
}
...
With Formula \eqref{eqn: double factorial} \cite{Double_Factorial}, for Gaussian distribution, Formula \eqref{eqn: Gaussian moment} provides $\zeta(2n, \kappa)$ while $\zeta(2n + 1, \kappa) = 0$, while Formula \eqref{eqn: Gaussian asymptotic moment} demonstrates its asymptotic behavior.
Figure \ref{fig: Gaussian_5_Moment} shows the transition to Formula \eqref{eqn: Gaussian asymptotic moment} with increasing order $2n$, in which the ratio is the ratio of Formula \eqref{eqn: Gaussian asymptotic moment} when $\kappa = 5$ to $\zeta(2n, 5)$.
\begin{align}
\label{eqn: double factorial} 
& \sum_{j=0}^{+\infty} \frac{\kappa^{2j + 1}}{(2j + 1)!!} = \sqrt{\frac{\pi}{2}} \xi (\frac{\kappa}{\sqrt{2}}) e^{\frac{1}{2} \kappa^2}; \\
\label{eqn: Gaussian moment} 
\zeta(2n, \kappa) &= \int_{-\kappa}^{+\kappa} \tilde{x}^{2n} N(\tilde{x}) d \tilde{x}
		= 2 N(\kappa) \kappa^{2n} \sum_{j=1}^{\infty} \kappa^{2j-1} \frac{(2n - 1)!!}{(2n-1 + 2j)!!} \\
\label{eqn: Gaussian asymptotic moment} 
& 1 < \kappa^2 \ll 2n:\eqspace \zeta(2n, \kappa) = 2 N(\kappa) \frac{\kappa^{2n+1}}{2n+1};
\end{align}
...
As shown in Figure \ref{fig: Sin_Error_Prec_ULP}, rounding errors can be uniformly distributed \cite{Prev_Precision_Arithmetic}.

For uniform distribution, the bounding range $\kappa$ is less than the maximal range $\sqrt{3}$.
Formula \eqref{eqn: uniform moment} provides $\zeta(2n)$ while $\zeta(2n + 1) = 0$.
\begin{equation}
\label{eqn: uniform moment}
\zeta(2n) = \int_{-\kappa}^{+\kappa} \tilde{x}^{2n} \frac{1}{2 \sqrt{3}} d \tilde{x} = 2 \frac{1}{2 \sqrt{3}} \frac{\kappa^{2n+1}}{2n + 1}, \;\; 0 \leq \kappa \leq \sqrt{3}; 
\end{equation}




\subsubsection{Laplace Input Uncertainty}

\iffalse

\begin{align*}
\rho(\tilde{x}) &= \frac{1}{2} e^{-|\tilde{x}|}; \\
\zeta(0) &= 1 - e^{-\kappa}; \\
\zeta(2n) &= \int_{0}^{\kappa} \tilde{x}^{2n}  e^{-\tilde{x}} d \tilde{x} = 2n \int_{0}^{\kappa} \tilde{x}^{2n - 1}  e^{-\tilde{x}} d \tilde{x} - \kappa^{2n}  e^{-\kappa} \\
	&= 2n (2n-1) \zeta(2n-2) -  e^{-\kappa} (2n \kappa^{2n -1} + \kappa^{2n}) \\
	&= 2n (2n-1) \left( (2n-2) (2n-3) \zeta(2n-4) -  e^{-\kappa} ((2n-2) \kappa^{2n -3} + \kappa^{2n - 2}) \right) -  e^{-\kappa} (2n \kappa^{2n -1} + \kappa^{2n}) \\
	& = 2n (2n-1)(2n-2) (2n-3) \zeta(2n-4) - e^{-\kappa} \left(2n (2n-1)(2n-2) \kappa^{2n -3} + 2n (2n-1) \kappa^{2n - 2} + 2n \kappa^{2n -1} + \kappa^{2n} \right) \\
	&= (2n)! \left( 1 - e^{-\kappa} \sum_{j=0}^{2n} \kappa^j \frac{1}{j!} \right) = e^{-\kappa} (2n)! \sum_{2n+1}^{\infty} \frac{\kappa^j}{j!}
		= e^{-\kappa} \sum_{j = 1}^{\infty} \kappa^{2n + j} \frac{(2n)!}{(2n + j)!};
\end{align*}

\fi

For Laplace distribution \cite{Probability_Statistics}, Formula \eqref{eqn: Laplace moment} provides $\zeta(2n, \kappa)$ while $\zeta(2n + 1, \kappa) = 0$, and Formula \eqref{eqn: Laplace asymptotic moment} demonstrates its asymptotic behavior.
\begin{align}
\label{eqn: Laplace moment} 
\zeta(2n, \kappa) &= \int_{-\kappa}^{+\kappa} \tilde{x}^{2n} \frac{1}{2} e^{-|\tilde{x}|} d \tilde{x}
		= e^{-\kappa} \kappa^{2n} \sum_{\kappa = 1}^{\infty} \kappa^j \frac{(2n)!}{(2n + j)!}; \\
\label{eqn: Laplace asymptotic moment} 
& 1 < \kappa^2 \ll 2n:\eqspace \zeta(2n, \kappa) = 2 \frac{1}{2} e^{-\kappa} \frac{\kappa^{2n+1}}{2n+1};
\end{align}



\subsubsection{Exponential Input Uncertainty}

\iffalse

\begin{align*}
\mu &= \int_{0}^{\infty} \tilde{x} e^{-\tilde{x}} d \tilde{x} = - \int_{0}^{\infty} \tilde{x} d e^{-\tilde{x}} = \int_{0}^{\infty} e^{-\tilde{x}} d \tilde{x} = 1; \\
\sigma^2 &= \int_{0}^{\infty} \tilde{x}^2 e^{-\tilde{x}} d \tilde{x} - 1 = 2 \int_{0}^{\infty} \tilde{x} d e^{-\tilde{x}} - 1 = 1; \\
\rho(\tilde{z}) &= e^{-\tilde{z} + 1}, \tilde{z} \in [-1, +\infty); \\
\zeta(n) &= \int_{\varrho}^{\kappa} (\tilde{x} - 1)^{n} e^{-\tilde{x}} d\tilde{x} 
		=  n \zeta(n - 1) + (\varrho - 1)^{n}  e^{-\varrho} - (\kappa - 1)^{n} e^{-\kappa} \\
	& =  n (n - 1) \zeta(n - 2) + n (\varrho - 1)^{n - 1}  e^{-\varrho} - n (\kappa - 1)^{n - 1} e^{-\kappa} + (\varrho - 1)^{n}  e^{-\varrho} - (\kappa - 1)^{n} e^{-\kappa} \\
	&= (n)! \left( e^{-\varrho} \sum_{j=0}^{n} \frac{(\varrho - 1)^j}{j!} - e^{-\kappa} \sum_{j=0}^{n} \frac{(\kappa - 1)^j}{j!} \right) \\
	&=  (2n)! e^{-\varrho} \left(e^{-\varrho + 1} - \sum_{j = n+1}^{\infty} \frac{(\varrho - 1)^j}{j!} \right)
		- (2n)! e^{-\kappa} \left(e^{-\kappa + 1} - \sum_{j = n+1}^{\infty} \frac{(\kappa - 1)^j}{j!} \right) \\
	&= e^{-\kappa} (\kappa - 1)^{n} \sum_{j = 1}^{\infty} (\kappa - 1)^j \frac{(n)!}{(n + j)!} 
		- e^{-\varrho} (\varrho - 1)^{n} \sum_{j = 1}^{\infty} (\varrho - 1)^j \frac{(n)!}{(n + j)!}; \\
&0 = \zeta(1) = e^{-\varrho} + e^{-\varrho} (\varrho - 1) - e^{-\kappa} - e^{-\kappa} (\kappa - 1) =  e^{-\varrho} \varrho - e^{-\kappa} \kappa; \\
&\eqspace \Longrightarrow \varrho e^{-\varrho} = \kappa e^{-\kappa}; \Longrightarrow \varrho < 1 < \kappa; \\
&\kappa^2 \ll 2n:\eqspace \zeta(n) \simeq e^{-\kappa} \frac{(\kappa - 1)^{n + 1}}{n + 1};  
\end{align*}

\fi

An exponential distribution \cite{Probability_Statistics} is no longer symmetric.
Formula \eqref{eqn: exponential moment} provides $\zeta(2n, \kappa)$, Formula \eqref{eqn: exponential mean preserving} shows the equation for the mean preserving condition, and Formula \eqref{eqn: exponential asymptotic moment} demonstrates its asymptotic behavior.
\begin{align}
\label{eqn: exponential moment} 
\zeta(n, \kappa) &= \int_{\varrho}^{\kappa} (\tilde{x} - 1)^{n} e^{-\tilde{x}} d \tilde{x} \\
	&= e^{-\kappa} (\kappa - 1)^{n} \sum_{j = 1}^{\infty} (\kappa - 1)^j \frac{n!}{(n + j)!} 
		- e^{-\varrho} (\varrho - 1)^{n} \sum_{j = 1}^{\infty} (\varrho - 1)^j \frac{n!}{(n + j)!}; \nonumber \\
\label{eqn: exponential mean preserving}
& \varrho e^{-\varrho} = \kappa e^{-\kappa} \Longrightarrow 0 < \varrho < 1 < \kappa; \\
\label{eqn: exponential asymptotic moment} 
& 1 < \kappa \ll n:\eqspace \zeta(n, \kappa) = e^{-\kappa} \frac{\kappa^{n + 1}}{n + 1}; 
\end{align}



\subsubsection{Simplified Gamma Input Uncertainty}

\iffalse

\begin{align*}
\rho(\tilde{x}) &\equiv \frac{1}{m!} \tilde{x}^{m} e^{-\tilde{x}},\eqspace x \in [0, +\infty); \\
	&\int_{0}^{+\infty} \rho(\tilde{x}) d \tilde{x} = 1; \\
	&\int_{0}^{+\infty} \tilde{x} \rho(\tilde{x}) d \tilde{x} = m + 1; \\
	&\int_{0}^{+\infty} \tilde{x}^2 \rho(\tilde{x}) d \tilde{x} - \left(\int_{0}^{+\infty} \tilde{x} \rho(\tilde{x}) d \tilde{x} \right)^2 = (m + 2)(m+1) - (m + 1)^2 = m + 1; \\
\tilde{z} &= \frac{\tilde{x} - (m + 1)}{\sqrt{m + 1}}, \tilde{z} \in [-\sqrt{m + 1}, +\infty); 
\end{align*}

\begin{align*}
\eta(m) &\equiv \int_{\varrho}^{\kappa} \tilde{z}^{m} e^{-\tilde{z}} d \tilde{z} = m \eta(m - 1) + e^{-\varrho} \varrho^{m} - e^{-\kappa} \kappa^{m} \\
	&= m (m - 1) \eta(m - 2) + e^{-\varrho} m \varrho^{m - 1} - e^{-\kappa} m \kappa^{m - 1} + e^{-\varrho} \varrho^{m} - e^{-\kappa} \kappa^{m} \\
	&= m! \left( e^{-\varrho} \sum_{j=0}^{m} \frac{\varrho^j}{j!} - e^{-\kappa} \sum_{j=0}^{m} \frac{\kappa^j}{j!} \right) 
		= m! \left( e^{-\kappa} \sum_{j=m+1}^{+\infty} \frac{\kappa^j}{j!} - e^{-\varrho} \sum_{j=m+1}^{+\infty} \frac{\varrho^j}{j!} \right) \\
	&=  e^{-\kappa} \kappa^{m} \sum_{j=1}^{+\infty} \frac{m!}{(m+j)!} \kappa^j
		- e^{-\varrho} \varrho^{m} \sum_{j=1}^{+\infty} \frac{m!}{(m+j)!} \varrho^j;
\end{align*}

\begin{align*}
\zeta(n)_m &= \int_{\varrho}^{\kappa} \frac{1}{m!} \left( \frac{\tilde{x} - (m+1)}{\sqrt{m+1}} \right)^n \tilde{x}^{m} e^{-\tilde{x}} d \tilde{x} \\
	&= \frac{1}{m!} \sum_{i = 0}^{n} \frac{n!}{i! (n-i)!} \sqrt{(m+1)^{n - 2i}} \int_{\varrho}^{\kappa} \tilde{x}^{m + i} e^{-\tilde{x}} d \tilde{x} \\
	&= e^{-\varrho} \sum_{i = 0}^{n} \frac{(-1)^i n!}{i! (n-i)!} \sqrt{(m+1)^{n - 2i}} \frac{(m + i)!}{m!} \sum_{j=0}^{m+i} \frac{\varrho^j}{j!} 
	 	- e^{-\kappa} \sum_{i = 0}^{n} \frac{(-1)^i n!}{i! (n-i)!} \sqrt{(m+1)^{n - 2i}} \frac{(m + i)!}{m!} \sum_{j=0}^{m+i} \frac{\kappa^j}{j!}; \\
& 0 = \zeta(1)_m = e^{-\varrho} \sqrt{m + 1} \sum_{j=0}^{m} \frac{\varrho^j}{j!} - e^{-\varrho} \sqrt{m + 1} \sum_{j=0}^{m + 1} \frac{\varrho^j}{j!} 
		- e^{-\kappa} \sqrt{m + 1} \sum_{j=0}^{m} \frac{\kappa^j}{j!} + e^{-\kappa} \sqrt{m + 1} \sum_{j=0}^{m + 1} \frac{\kappa^j}{j!} \\
	&\eqspace = \sqrt{m + 1} e^{-\kappa} \frac{\kappa^{m+1}}{(m+1)!} - \sqrt{m + 1} e^{-\varrho} \frac{\varrho^{m+1}}{(m+1)!};
		\Longrightarrow e^{-\varrho} \varrho^{m+1} = e^{-\kappa} \kappa^{m+1}; \Longrightarrow 0 < \varrho < m+ 1 < \kappa; \\
\zeta(n)_m &= \frac{1}{m!} \sum_{i = 0}^{n} \frac{n!}{i! (n-i)!} \sqrt{(m+1)^{n - 2i}} e^{-\kappa} \kappa^{m+i} \sum_{j=1}^{+\infty} \frac{(m+i)!}{(m+i+j)!} \kappa^j \\
	&\eqspace - \frac{1}{m!} \sum_{i = 0}^{n} \frac{n!}{i! (n-i)!} \sqrt{(m+1)^{n - 2i}} e^{-\varrho} \varrho^{m+i} \sum_{j=1}^{+\infty} \frac{(m+i)!}{(m+i+j)!} \varrho^j\\
& \lim_{n \rightarrow +\infty} \zeta(n)_m = e^{-\kappa} \frac{1}{m!} \sum_{i = 0}^{n} 
		\frac{n!}{i! (n-i)!} \sqrt{(m+1)^{n - 2i}} \kappa^{m+i} \frac{(m+i)!}{(m+n+1)!} \kappa^{n + 1 - i} \\
&\eqspace = \frac{1}{m!} e^{-\kappa} \frac{\kappa^{m+n+1}}{(m+n+1)} \frac{1}{\sqrt{(m+1)^n}};
\end{align*}

\fi

The gamma distribution \cite{Probability_Statistics} $\rho(\tilde{x}) = x^{\alpha - 1} e^{- x /\lambda} \lambda^{\alpha} /\Gamma(\alpha), \tilde{x}, \alpha, \lambda > 0$ is simplified as Formula \eqref{eqn: Gamma probability density}, whose mean and deviation are $m + 1$ and $\sqrt{m + 1}$, respectively.
Formula \eqref{eqn: Gamma moment} provides $\zeta(2n, \kappa)$, Formula \eqref{eqn: Gamma mean preserving} shows the equation for the mean preserving condition, and Formula \eqref{eqn: Gamma asymptotic moment} demonstrates its asymptotic behavior.
\begin{align}
...
\label{eqn: Gamma mean preserving}
& e^{-\varrho} \varrho^{m+1} = e^{-\kappa} \kappa^{m+1}; \Longrightarrow 0 < \varrho < m+1 < \kappa; \\
\label{eqn: Gamma asymptotic moment} 
& 1 < \kappa \ll n:\eqspace \zeta(n, \kappa) = \frac{1}{m!} e^{-\kappa} \frac{\kappa^{m+n+1}}{m+n+1}; 
\end{align}
...
When $n \rightarrow \infty$, $\zeta(j, \kappa) \zeta(n - j, \kappa)$ will be ignored in Formula \eqref{eqn: Taylor 1d variance} because $\zeta(j, \kappa) \zeta(n - j, \kappa) \ll \zeta(n, \kappa)$.
The asymptotic behavior of $\zeta(n, \kappa)$ when $n \rightarrow \infty$ determines if Formula \eqref{eqn: Taylor 1d variance} will converge.
...
$\kappa_s = \sqrt{3}$ and $\kappa_s = 5$ are defined as the ideal bounding ranges $\hat{\kappa}$ for the uniform and Gaussian distributions respectively.
...
The stable variance $\widehat{\delta^2} f$ is computed using Formula \eqref{eqn: uniform moment}.
\end{itemize}
...
\subsection{Refine Quart Sine Function}
...
\begin{figure}[p]
\includegraphics[height=2.5in]{Sin_Error_Prec_ULP.pdf} 
\captionof{figure}{
The value error histogram for the Quart sine function when it is calculated using 128-bit floating-point arithmetic and cast to 64-bit floating-point representation.
}
\label{fig: Sin_Error_Prec_ULP}
\end{figure}


If the Quart sine function is constructed using the 128-bit floating-point library of gcc and then converted to 64-bit values, the resulting indexed sine function is referred to as the \emph{Prec} sine function.
\begin{itemize}
\item 
...
As suggested in Figure \ref{fig: Sin_Error_Quart_Prec}, the Prec sine function has about $5\%$ more non-zero $\sin(x)^2 + \cos(x)^2 - 1$ value errors, and  $14\%$ non-zero $\sin(x)^2 + \cos(x)^2 - 1$ value errors exceeding the corresponding uncertainties.
These difference indicate that casting a 128-bit floating-point value to a 64-bit floating-point value is less accurate in the 64-bit floating-point arithmetic.

\item Figure \ref{fig: Sin_Error_Prec_ULP} demonstrates that the value errors of the Prec sine function after normalized by the corresponding value ULP is uniform distributed with in the half bit of ULP.
Because variance arithmetic assumes that the value error is uniformly distributed within ULP of each value, the Quart sine function overestimates value errors by one times.
\end{itemize}
...
Such increase results in larger error deviations: At FFT order $18$, $0.89 > 0.65$ for the forward transformation, $1.08 > 0.90$ for the reverse transformation, $1.67 > 0.90$ for the roundtrip transformation, and wider distributions of the normalized errors in Figure \ref{fig: FFT_Linear_Clean_Histo_Indexed} versus the corresponding histograms in Figure \ref{fig: FFT_SinCos_Clean_Histo_Indexed}.
...
Figure \ref{fig: FFT_Linear_Indexed_Reverse_ErrorDev_vs_Noise_Order} shows that the rounding errors balances the over-estimation of value errors in the Quart sine function in Figure \ref{fig: Sin_Error_Prec_ULP}, so that the error deviations within the proper coverage region differ from $1$ only marginally.
...
At first, it is surprising that in Figure \ref{fig: FFT_Performance} the Java implementation is 2 times faster than the C++ implementation which has been fully optimized for speed.
...
The necessary and sufficient conditions for Formula \eqref{eqn: bound moment asymptote} need to be found.
The statistical bounding to find the bounding range $\kappa$ for the bounded moment needs to be extended to discrete distributions.

The choice of ideal bounding range $\hat{\kappa}$ needs further study.
For example, the choice of $\hat{\kappa} = 5$ for Normal distribution is based on the $5-\sigma$ rule in practice only.
Such choice certainly can not be applied to other distributions.
Also, the choice of ideal bounding range $\hat{\kappa}$ determines the convergence of both $\log(x)$ and $x^c$ as $P(x) \lesssim 1/\hat{\kappa}$, which needs better mathematical understanding.
...
The dependence of the convergence of $\log(x \pm \delta x)$ and $(x \pm \delta x)^c$ on the chose of ideal bounding range $\kappa$ needs further discussion.
