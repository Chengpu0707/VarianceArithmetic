...
\end{itemize} 
If $f(x)$ can be Taylor expanded as Formula \eqref{eqn: Taylor 1d}, using Formula \eqref{eqn: Gaussian function distribution}, the variance bias $\widehat{f(x)}$ and variance $\delta^2 f(x)$ of the result distribution are calculated by Formula \eqref{eqn: calc bias} and \eqref{eqn: Taylor 1d variance}, respectively.  
$\delta^2 f(x) + \widehat{f(x)}^2$ is defined as defined as the \emph{calculation variance}.
As shown in Formula \eqref{eqn: Taylor 1d variance}, like the variance bias, the calculation variance is centered around $f(x)$ instead of $\overline{f(x)}$.
...
The variance bias in $x$ is transfer to $f(x)$ according to Formula \eqref{eqn: accum calc bias}.
\begin{align}
\label{eqn: Taylor 1d} 
\tilde{y} &= f(x + \tilde{z} \delta x) = \sum_{n=0}^{\infty} \frac{f^{(n)}_x}{n!} \tilde{z}^n (\delta x)^n; \\
...
When $\delta x \ll x$, Formula \eqref{eqn: exp bias}, \eqref{eqn: exp precision}, \eqref{eqn: log bias}, \eqref{eqn: log precision}, \eqref{eqn: power bias} and \eqref{eqn: power precision} give the result variance bias and precision for $e^x$, $\ln(x)$, and $x^c$, respectively.
\begin{align}
\label{eqn: exp bias}
...
\item An optional variance bias is stored using conventional 64-bit floating-point representation.
\end{itemize}
