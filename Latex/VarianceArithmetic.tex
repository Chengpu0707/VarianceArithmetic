...
\zeta(2n) &= 2 N(\sigma) \frac{\sigma^{2n+1}}{2n + 1} + 2 N(\sigma) \frac{\sigma^{2n+2}}{2n + 1} \sum_{j=1}^{\infty} \sigma^{2j-1} \frac{(2n+2 - 1)!!}{(2j+2n+2-1)!!} \\
...
P(e^x)^2 &= \sum_{n=1}^{\infty} (\delta x)^{2n} \zeta(2n) \sum_{j=1}^{2n-1} \frac{(2n - 1)!!}{j!\;(2n - j)!}
 - \left( \frac{\widehat{e^x}}{e^x} \right)^2 \\
...
For the limited $x$, the behavior of $\lim_{\tilde{x} \rightarrow \infty} f(x + \tilde{x})$ is not of primary concern, so that $\sigma$ should be limited, and $M(x, 2n)$ is calculated by Formula \eqref{eqn: momentum even}, which contains a \emph{variance momentum factor} $\zeta(2n)$ defined in Formula \eqref{eqn: momentum factor}.
\begin{align}
...
\end{align}
Figure \ref{fig: Momentum_Factor_Bounding} shows that the variance momentum factor $\zeta(2n)$ approaches $(2n-1)!!$ with increasing Taylor expansion order $2n$, but for a larger $2n$, $\zeta(2n)$ reaches $(2n-1)!!$ at a larger $\sigma$. 
...
Thus it is possible to choose $\sigma$ so that $\zeta(2n) \sim \sigma^{2n}$ for $2n < 200$ which is the practical range for Taylor expansion.

Figure \ref{fig: Momentum_Factor_Expansion} shows that when $3 \leq \sigma \leq 6$, $\zeta(2n) \sim \lambda^{2n}$ in which $\lambda$ is a fitting parameter.
Table 1 contains the $\lambda$ for different $\sigma$, in which $\lambda$ is always slightly smaller than $\sigma$, as expected.
...
The smaller diagram shows the starting portion of $\zeta(2n)$ vs $2n$ for different $\sigma$.
}
...
Thus, $\sigma \leq 5$ for variance arithmetic.
...
\label{eqn: log bias}
\widehat{\ln(x)} &= -\sum_{n=1}^{+\infty} \zeta(2n) P(x)^{2n} \frac{1}{2n}; \\
 & P(x) \leq 1/5:\eqspace \widehat{\ln(x)} \simeq - \frac{1}{2} P(x)^2 - \frac{3}{4} P(x)^4 - \frac{5}{2} P(x)^6 - \frac{105}{8} P(x)^8; \\
\label{eqn: log precision}
\delta^2 \ln(x) &= \sum_{n=1}^{+\infty} \zeta(2n) P(x)^{2n} \frac{1}{n} \sum_{j=1}^{2n-1} \frac{1}{j} - \left(\frac{\widehat{x^c}}{x^c}\right)^2 \\
 & P(x) \leq 1/5:\eqspace \delta^2 \ln(x) \simeq P(x)^2 + \frac{5}{2} P(x)^4 + \frac{32}{3} P(x)^6 + 65 P(x)^8; 
\end{align}
...
\end{align}
As the special cases for Formula \eqref{eqn: power bias} and \eqref{eqn: power precision}, when $P(x) \leq 1/5$, Formula \eqref{eqn: square bias} and \eqref{eqn: square precision} give the result for $x^2$, Formula \eqref{eqn: square root bias} and \eqref{eqn: square root precision} give the result for $\sqrt{x}$, and Formula \eqref{eqn: inversion bias} and \eqref{eqn: inversion precision} give the result for $1/x$: 
\begin{align}
...
\frac{\widehat{x^c}}{x^c} &\simeq P(x)^2 + 3 P(x)^4 + 15 P(x)^6 + 105 P(x)^8; \\
\label{eqn: inversion precision}
P(1/x)^2 &\simeq P(x)^2 + 8 P(x)^4 + 69 P(x)^6 + 696 P(x)^8; 
\end{align}

The variance formula using Taylor expansion shows the nature of the calculation, such as $\delta x \rightarrow P(e^x)$, $P(x) \rightarrow P(x^c)$, and $P(x) \rightarrow \delta \ln(x)$. or the difference speed of variance increases of $x^c$ depending on different $c$.

If Formula \eqref{eqn: Taylor 1d variance} can be expressed as $\sum_{n=1}^{\infty} \upsilon(2n) \zeta(2n) P(x)^{2n}$, such as Formula \eqref{eqn: log precision} and \eqref{eqn: power precision}, and if $|\upsilon(2n)| \sim \lambda^{2n}$, then Formula \eqref{eqn: Taylor 1d variance} converges if $P(x) < 1/(\sigma + \lambda)$.
$\upsilon(2n)$ depends on $f(x)$, and so does $\lambda$.
From Formula \eqref{eqn: square root precision} and \eqref{eqn: inversion precision}, $\lambda$ is larger for $1/x$ than for $\sqrt{x}$.
For each function, the maximal $P(x)$ for Formula \eqref{eqn: Taylor 1d variance} to be applicable is defined as \emph{applicable precision threshold}, which could be smaller than $1/\sigma$ because according to Table 1, $\zeta(2n)$ increases with $2n$ slightly slower than $\sigma^{2n}$.
When the applicable precision threshold is larger than $1/\sigma$, the integration range in Formula \eqref{eqn: variance momentum} needs to be reduced to $1/P(x)$. 
Generally, $1/\sigma$ provides a rough estimation for the applicable precision threshold.
...
\subsection{The Boundary Condition at Zero}
...
The difference between $\rho(\tilde{x})$ and $\varrho(\tilde{x})$ is $\int_{-\infty}^{+\infty} |\rho(\tilde{x}) - \varrho(\tilde{x})| d \tilde{x}$ which is shown as the percentage value of the legend for the normal distribution.
}
...
When Formula \eqref{eqn: calc bias} and \eqref{eqn: Taylor 1d variance} has $P(x)$ as the input, such as for $\ln(x)$ or $x^c$, both break down at $0 \pm \delta x$, even the function may be defined across 0, such as $x^2$ or $x^3$.
...
\item $(5 \pm 1)^2$ looks definitely Gaussian.
...
It shows that $\sqrt{3 \pm 1}^2$ is as close to Gaussian as $(5 \pm 1)^2$.
Even though $x^2$ is defined across 0, the uncertainty distribution of $(x + \delta x)^2$ deviates from Gaussian more than that of $\sqrt{x + \delta x}$ when $x \rightarrow 0$.
...
For example, if the square of an input imprecise value is mistakenly carried out as multiplication of two different input imprecise values, the result linear deviation is only $1/2$-fold according to Formula \eqref{eqn: square precision} and \eqref{eqn: multiplication precision}.
...
For example, Formula \eqref{eqn: square root} and \eqref{eqn: root square} shows that the calculation cost of $\sqrt{x}^2$ is more than double of the calculation cost of $\sqrt{x^2}$.
\begin{align}
...
\end{align}
The dependency problem also exists in interval arithmetic, and to a much larger extend, because variance arithmetic has no dependency problem due to different analytic expression.
