...
void test_poly_0() {
    VarDbl res;
    try {
        res = VarDbl().polynominal(std::vector<int>{1});
        test::assertAlmostEqual(res.value(), 1, 0);
        test::assertAlmostEqual(res.uncertainty(), 0, 0);

        res = VarDbl(1).polynominal(std::vector<int>{1});
        test::assertAlmostEqual(res.value(), 1, 1);
        test::assertAlmostEqual(res.uncertainty(), 0, 0);

        res = VarDbl(1).polynominal(std::vector<int>{-2});
        test::assertAlmostEqual(res.value(), -2, 0);
        test::assertAlmostEqual(res.uncertainty(), 0, 0);

        res = VarDbl(2).polynominal(std::vector<double>{1.0});
        test::assertAlmostEqual(res.value(), 1, 0);
        test::assertAlmostEqual(res.uncertainty(), 0, 0);
...
void test_poly_1() {
    VarDbl res;
    try {
        res = VarDbl(0, 1/8).polynominal(std::vector<int>{0, 1});
        test::assertAlmostEqual(res.value(), 0, 0);
        test::assertAlmostEqual(res.uncertainty(), 1/8, 1e-6);

        res = VarDbl(0, 1/8).polynominal(std::vector<int>{1,1});
        test::assertAlmostEqual(res.value(), 1, 0);
        test::assertAlmostEqual(res.uncertainty(), 1/8, 1e-6);

        res = VarDbl(-2, 1/8).polynominal(std::vector<int>{1,1});
        test::assertAlmostEqual(res.value(), -1, 0);
        test::assertAlmostEqual(res.uncertainty(), 1/8, 1e-6);
    } catch (std::runtime_error e) {
        test::fail(e.what());
...
void test_poly_2() {
    ...
    try {
        for (double value: {0., 1., -1., 2., -2., 0.25, -0.25}) {
            res = VarDbl(value, 0.5).polynominal(std::vector<int>{0,0,1});
            ...
            res = VarDbl(value, 0.5).polynominal(std::vector<int>{1,2,1});
...
void test_poly_3() {
    ...
    try {
        for (double value: {1., -1., 2., -2., 0.25, -0.25}) {
            res = VarDbl(value, 0.5).polynominal(std::vector<int>{0,0,0,1});
            ...
            res = VarDbl(value, 0.5).polynominal(std::vector<int>{1,3,3,1});
