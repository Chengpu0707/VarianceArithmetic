...
#ifndef __TestTaylor_h__
#define __TestTaylor_h__
namespace var_dbl 
{
...
{
    std::cout << "Start searching edge for " << dumpPath << std::endl;
    std::ostringstream oss;
    oss << "x in [";
    for (double x: sX)
        oss << x << ", ";
    ...
    for (double x: sX) {
        ...
        bias = func(x, 0);
        ofs << std::setprecision(15) << x << "\t" << edge << "\t" << res.value() - bias.value() 
            << "\t" << res.value() << "\t" << res.uncertainty() << "\t" << except << "\n";
        ofs.flush();
    }
    std::cout << "Finish searching edge for " << dumpPath << std::endl;
}
